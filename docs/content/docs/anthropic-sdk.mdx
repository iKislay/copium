---
title: Anthropic SDK
description: Auto-compress messages in the Anthropic TypeScript SDK with a single withCopium() wrapper.
---

Copium wraps the Anthropic TypeScript SDK to automatically compress messages before every `messages.create()` call. All other methods pass through unchanged.

## Installation

```bash
npm install copium-ai @anthropic-ai/sdk
```
...
`withCopium()` returns a proxy around your Anthropic client that intercepts `messages.create()`:
