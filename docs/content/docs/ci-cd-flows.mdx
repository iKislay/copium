...
This page is the quick visual map for Copium automation. Use it when opening a PR, reviewing a PR, cutting a release, or deciding which workflow owns a failure.
...
flowchart TD
    ...
    C --> D[Smoke-test image imports pydantic_core and copium._core]
...
| `.github/workflows/init-e2e.yml` | Dockerized `copium init` behavior |
| `.github/workflows/wrap-e2e.yml` | Dockerized `copium wrap` behavior |
| `.github/workflows/init-native-e2e.yml` | Host-specific `copium init -g` smoke tests |
