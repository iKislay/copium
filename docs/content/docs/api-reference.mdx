---
title: API Reference
description: Complete API reference for the Copium Python and TypeScript SDKs. Core client, configuration types, result types, errors, and utilities.
---

Complete API reference for the Copium Python and TypeScript SDKs.

## Core

### CopiumClient

The main entry point for the Copium SDK.

<Tabs groupId="lang" items={['TypeScript', 'Python']}>
<Tab value="TypeScript">

<TypeTable type={{
  baseUrl: { type: 'string', description: 'Base URL for the Copium proxy' },
...
Accepts all standard OpenAI/Anthropic parameters plus Copium-specific overrides:

<TypeTable type={{
  copium_mode: { type: '"audit" | "optimize" | "simulate"', description: 'Override mode for this request', default: 'Client default' },
  copium_query: { type: 'str', description: 'Query for relevance scoring', default: 'None' },
  copium_output_buffer_tokens: { type: 'int', description: 'Reserve tokens for output', default: '4000' },
  copium_keep_turns: { type: 'int', description: 'Keep last N turns uncompressed', default: '2' },
  copium_tool_profiles: { type: 'dict', description: 'Per-tool compression overrides', default: '{}' },
}} />
...
Top-level function to compress messages via the Copium proxy.
...
<TypeTable type={{
  ...
  fallback: { type: 'boolean', description: 'Return original messages on failure instead of throwing' },
  retries: { type: 'number', description: 'Number of retry attempts on failure' },
  client: { type: 'CopiumClientInterface', description: 'Pre-configured client instance to use' },
...
Context management is now automatic. Copium handles rolling window behavior internally based on the model's context limit and the `copium_keep_turns` / `copium_output_buffer_tokens` per-request overrides. The `RollingWindowConfig`, `IntelligentContextConfig`, and `ScoringWeights` classes no longer exist as public API.
...
<TypeTable type={{
  ...
  defaultMode: { type: 'CopiumMode', description: 'Default compression mode' },
...
# Note: rolling_window was removed in 0.9.x — use copium_keep_turns per-request instead
```
...
Semantic similarity scoring (requires `copium-ai[relevance]`):
...
| Exception | Meaning |
|-----------|---------|
| `CopiumError` | Base class for all errors |
| `CopiumConnectionError` | Cannot reach proxy |
| `CopiumAuthError` | 401 from proxy |
| `CopiumCompressError` | Compression failed (includes `statusCode`, `errorType`) |
...
| Exception | Meaning |
|-----------|---------|
| `CopiumError` | Base class for all Copium errors |
