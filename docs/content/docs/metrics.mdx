...
description: Monitor compression performance, cost savings, and system health with Copium's built-in metrics, Prometheus endpoint, and SDK APIs.
---

Copium provides comprehensive metrics for monitoring compression performance, cost savings, and system health through both the proxy server and the SDK.
...
Persistent savings are stored at `~/.copium/proxy_savings.json` and survive proxy restarts. Override the path with `COPIUM_SAVINGS_PATH`.
...
```
# HELP copium_requests_total Total requests processed
copium_requests_total{mode="optimize"} 1234

# HELP copium_tokens_saved_total Total tokens saved
copium_tokens_saved_total 5678900

# HELP copium_compression_ratio Compression ratio histogram
copium_compression_ratio_bucket{le="0.5"} 890
copium_compression_ratio_bucket{le="0.7"} 1100
copium_compression_ratio_bucket{le="0.9"} 1200

# HELP copium_latency_seconds Request latency histogram
copium_latency_seconds_bucket{le="0.01"} 800
copium_latency_seconds_bucket{le="0.1"} 1150

# HELP copium_cache_hits_total Cache hit counter
...
| Panel | PromQL |
|-------|--------|
| Tokens Saved | `copium_tokens_saved_total` |
| Compression Ratio (median) | `histogram_quantile(0.5, copium_compression_ratio_bucket)` |
| Request Latency (p99) | `histogram_quantile(0.99, copium_latency_seconds_bucket)` |
| Cache Hit Rate | `copium_cache_hits_total / (copium_cache_hits_total + copium_cache_misses_total)` |
