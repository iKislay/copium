...
description: Understand Copium's three-stage compression pipeline, automatic content routing, and how different content types are compressed.
---

Copium automatically detects what kind of content you're sending and routes it to the right compressor. You don't need to configure anything -- just call `compress()` and the pipeline handles the rest.
...
<Tab value="Python">
```python
# Advanced / Internal API -- prefer `from copium import compress` for typical use
...
Copium doesn't blindly truncate. It identifies what matters in each content type and preserves it:
