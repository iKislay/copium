...
description: Run Claude Code against Claude models on Google Vertex AI, with Copium compressing your prompts — fewer input tokens, same answers, your own GCP login.
---

If your Claude models live on **Google Vertex AI**, you can still get Copium's
prompt compression. Copium sits between Claude Code and Vertex: it shrinks the
big stuff in each request (file reads, logs, tool output) and forwards the rest to
Vertex using **your own Google credentials**. You keep your GCP setup; Copium
...
- **No new secrets** — Copium never holds your Google credentials. Claude Code
  keeps authenticating to Vertex with its own ADC token; Copium passes it through.

## Before you start

You should already have Claude Code working against Vertex **without** Copium.
...
That's it. Because `CLAUDE_CODE_USE_VERTEX=1` is set, `copium wrap claude`
automatically:

1. starts the Copium proxy,
...
```
Claude Code ──(Vertex request)──▶ Copium ──(compressed)──▶ Vertex AI (Claude)
...
Copium. Copium compresses the messages (keeping the Vertex request shape
...
2. Or look at the response headers on a request: `x-copium-tokens-before`,
...
- **It still goes straight to Google (no savings).** Make sure `CLAUDE_CODE_USE_VERTEX=1`
  is exported *in the same shell* before `copium wrap claude`. The wrapper only
...
- **Wrong region / 404 from Vertex.** Confirm `CLOUD_ML_REGION` matches a region
  ...
  non-regional host.
- **Auth errors.** Copium forwards your token as-is — if `gcloud auth
  application-default login` (or `GOOGLE_APPLICATION_CREDENTIALS`) works for Claude
  Code without Copium, it works with it.

## Alternative: let Copium talk to Vertex for you

If you'd rather **not** run Claude Code in Vertex mode, you can have Copium be the
translator instead: Claude Code speaks plain Anthropic to Copium, and Copium
calls Vertex on your behalf.

```bash
export COPIUM_BACKEND=litellm-vertex_ai     # note the _ai suffix
export COPIUM_REGION=us-east5
...
copium wrap claude --backend litellm-vertex_ai --region us-east5
```
...
- Streaming, tool use, and prompt caching all work through Copium.
- Want to point at a private Vertex gateway instead of Google's host? Start the
  proxy with `--vertex-api-url https://your-gateway` and Copium will forward there.
