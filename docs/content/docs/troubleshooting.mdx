...
description: Solutions for common Copium issues including proxy startup, connection errors, no token savings, high latency, and installation problems.
---
...
**Symptom**: After pointing Claude Code at Copium (`ANTHROPIC_BASE_URL`), `/context all`
...
```bash
# Easiest: `copium wrap claude` sets ENABLE_TOOL_SEARCH=true automatically.
copium wrap claude
...
copium wrap claude --tool-search auto
...
See [issue #746](https://github.com/chopratejas/copium/issues/746) for the full analysis.
...
### pipx installs an older Copium version

**Symptom**: PyPI shows a newer `copium-ai` release, but `pipx install` or
...
environment uses a Python version that Copium does not publish wheels for yet,
...
If you already have Copium installed under `pipx`, uninstall it first or
...
4. File an issue at [github.com/chopratejas/copium](https://github.com/chopratejas/copium/issues) with your Copium version, Python version, provider, debug log output, and minimal reproduction code
