...
Copium's CCR (Compress-Cache-Retrieve) architecture makes compression **reversible**. When content is compressed, the original data is cached locally. If the LLM needs the full data, it retrieves it instantly.
...
The client never sees CCR tool calls -- they are handled transparently by Copium.
...
import type { CCRConfig } from "copium-ai";
...
const ccrConfig: CCRConfig = {
  enabled: true,
  injectTool: true,             // Inject copium_retrieve tool
...
# The LLM calls copium_retrieve when it needs more data.
...
agent runs, set `COPIUM_CCR_TTL_SECONDS` before starting the proxy:

```bash
COPIUM_CCR_TTL_SECONDS=7200 copium proxy
```
