...
When conversations grow beyond a model's context window, Copium decides which messages to keep and which to drop. Instead of naively removing the oldest messages, the pipeline scores every message by learned importance and drops the least valuable ones first.
...
If IntelligentContext is disabled or scoring data is unavailable, Copium falls back to a **rolling window** strategy:
...
Copium enforces several protections to ensure model output quality:
...
response = client.chat.completions.create(
    model="gpt-4o",
    messages=messages,
    copium_output_buffer_tokens=8000,  # More room for long responses
    copium_keep_turns=5,               # Protect last 5 turns
)
```
</Tab>
</Tabs>
