...
When agents hand off to each other, context gets replayed in full. SharedContext compresses what moves between agents using Copium's compression pipeline, typically saving **~80% of tokens** on agent handoffs.
...
```ts twoslash
import { SharedContext } from "copium";
...
Under the hood, `put()` calls `copium.compress()` -- the same pipeline used by the Copium proxy -- and stores the original in memory. `get()` returns the compressed version. `get(full=True)` returns the original.
