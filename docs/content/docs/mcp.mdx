...
Copium's MCP server exposes compression, retrieval, and observability as tools that any MCP-compatible AI coding tool can call -- Claude Code, Cursor, Codex, and more. No proxy required.
...
The original is stored locally for 1 hour. If the LLM needs the full content later, it calls `copium_retrieve`.
...
Sub-agent stats are aggregated via a shared stats file at `~/.copium/session_stats.jsonl`.
...
For MCP hosts that let you configure a local stdio server, point them at `copium mcp serve`. If you also run the proxy, pass the proxy URL explicitly so retrieval and stats come from the intended proxy instance.
...
The LLM calls `copium_compress` on demand. Compression happens locally in the MCP process. Originals are stored in a local `CompressionStore` with 1-hour TTL.
...
`copium_retrieve` checks the local store first, then falls back to the proxy's store.

## Troubleshooting

**"MCP SDK not installed"** -- Run `pip install "copium-ai[mcp]"`.

**"Proxy not running"** -- Start the proxy with `copium proxy` in another terminal. Only needed for proxy-backed retrieval.

**"Entry not found or expired"** -- Local content expires after 1 hour, proxy content after 5 minutes.

**Claude doesn't see copium tools** -- Run `copium mcp status`, restart Claude Code, and verify with `/mcp` inside Claude Code.

### Claude Code `/usage` attributes a large share to `copium` MCP

Claude Code counts MCP tool calls and MCP tool results as session context. If a
long-running workflow or a subagent-heavy command calls `copium_compress`,
`copium_retrieve`, or `copium_stats` many times, `/usage` can show a visible
share under the `copium` MCP server even when Copium is saving tokens inside
individual tool results.

That number is not a direct "Copium overhead" bill. It means Claude Code kept
Copium MCP interactions in the conversation context. Deep research workflows
...
- Run `copium_stats` and compare `tokens_saved` with the number of MCP calls.
...
- Prefer the proxy path for automatic compression of normal Claude Code traffic:
  `copium proxy` plus `ANTHROPIC_BASE_URL=http://127.0.0.1:8787 claude`.
- Disable the MCP server for sessions where you only want proxy-level
  compression and do not need on-demand `copium_compress` or
  `copium_retrieve`.
