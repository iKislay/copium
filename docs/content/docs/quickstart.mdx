---
title: Quickstart
description: Get Copium running in 5 minutes. Install, compress, and send to your LLM with fewer tokens.
---
...
The TypeScript SDK sends messages to a local Copium proxy for compression. Start the proxy before using the TS SDK:
...
If you do not want to change any code, run Copium as a proxy and point your existing client at it:
...
All requests flow through Copium automatically. Check savings at any time:
...
The biggest savings come from tool outputs -- search results, database rows, log files, API responses. Copium auto-detects the content type and routes it to the best compressor. No configuration needed.
