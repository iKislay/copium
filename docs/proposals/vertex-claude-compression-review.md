# Claude Code + Vertex AI + Copium compression — does it work?
...
copy-paste runbook: [`docs/claude-code-vertex-copium.md`](../claude-code-vertex-copium.md).**
...
>   proxy never receives a byte. Not a Copium bug — the native `:rawPredict`
...
>   path.** Verified: Claude Code → Copium → LiteLLM → Vertex (`global`), real
...
There is still **no `copium wrap claude` turnkey** for Vertex, and the one backend
...
client is pointed at Copium, one mis-named backend, a missing pip extra, a
default-off compressor, and a few env vars Copium never sets for you.
...
Vertex AI** (not the direct Anthropic API). They want **Copium** in the middle so
...
1. **The client's traffic must actually reach Copium** (the proxy must be in the path).
2. **Copium must compress it.**
3. **Copium must forward it to Vertex correctly** (right URL, right auth, right body shape) and translate the answer back so Claude Code understands it.
...
|---|---|---|---|
| **Python proxy, native Vertex `:rawPredict`** (publisher = `anthropic`) | Client sends a real Vertex request to Copium | ✅ **Yes** | Runs the full Anthropic compression pipeline, keeps the Vertex body shape, forwards the client's own Google token. `proxy_routes.py:648` |
| **Python proxy, `--backend litellm-vertex_ai`** | Client speaks plain Anthropic; Copium translates to Vertex | ✅ **Yes** (correct string only) | Full Anthropic↔Vertex translation incl. streaming + tools. **`litellm-vertex` is broken — must use `litellm-vertex_ai`.** |
| **Rust proxy, native Vertex `:rawPredict`** | A separate `copium-proxy` binary | ✅ **Yes** | Correct and well-built — **but never run by `copium proxy`/`wrap`.** Dead code for normal users. |
...
**Through `copium wrap claude` with zero extra setup: no.** `wrap claude` only
sets `ANTHROPIC_BASE_URL`. If Claude Code is in Vertex mode it ignores that and
talks straight to Google — Copium is never in the path. And `wrap claude` has no
...
You just tell it "send Vertex requests to Copium instead of straight to Google,"
and you tell Copium where the real Vertex endpoint is.

```bash
# 1) Run Copium, telling it the real Vertex endpoint (match your region!)
copium proxy --port 8787 \
  --vertex-api-url https://us-east5-aiplatform.googleapis.com   # use YOUR region

# 2) Run Claude Code in Vertex mode, but point its Vertex base URL at Copium
...
What happens: Claude Code → `ANTHROPIC_VERTEX_BASE_URL` (Copium) → Copium
...
## Setup B — Claude Code in normal Anthropic mode; Copium talks to Vertex

Idea: Claude Code thinks it's talking to plain Anthropic. Copium holds the Google
credentials and is the one that actually talks to Vertex.

```bash
# Copium does the Vertex talking — note the backend name carefully
export COPIUM_BACKEND=litellm-vertex_ai        # NOT "litellm-vertex" (that's broken — Issue #1)
export COPIUM_REGION=us-east5                  # becomes the Vertex location
export VERTEXAI_PROJECT=<your-gcp-project>        # Copium does NOT set this for you (Issue #4)
...
What happens: Claude Code → Copium (plain Anthropic `/v1/messages`) →
...
2. Or check response headers on a request: `x-copium-tokens-before`,
   `x-copium-tokens-after`, `x-copium-tokens-saved`.
...
on `vertex_ai`, not `vertex`. So Copium falls back to a generic "unknown
...
### 2. GAP — `copium wrap claude` has no Vertex support
...
### 3. GAP — Vertex-mode Claude Code bypasses the proxy, and Copium never sets the fix
...
— that points Claude Code's Vertex traffic at a gateway. But Copium never sets it
...
For Setup B, Copium passes the region to LiteLLM but never the project. You must
...
Copium pins the Vertex host to one region (default `us-central1`) but throws away
the region in the client's request path. If your client targets, say,
`europe-west1` while Copium is on the default host, the request goes to the wrong
...
There's a second, Rust proxy (`crates/copium-proxy/src/vertex/`) that compresses
Vertex traffic correctly. But `copium proxy` and `copium wrap` run the **Python**
...
*Where:* `crates/copium-proxy/Cargo.toml` (`[[bin]]`), no Python→Rust bridge to it.
...
`crates/copium-proxy/src/vertex/mod.rs:42-47` describes a "synthetic model
...
After #1–#3, the honest customer message becomes: *"`copium wrap claude` works
