# Claude Code + Google Vertex AI, with Copium compression

*Validated end-to-end on 2026-06-19 (Claude Code 2.1.181, Copium 0.27.0).*

This is the **working, tested** way to run **Claude Code** against **Claude models on
Google Vertex AI** with **Copium compressing the context** in the middle.

## TL;DR

Run Claude Code in **normal Anthropic mode** (NOT Vertex mode) pointed at a local
Copium proxy, and let **Copium** be the thing that talks to Vertex:

```
Claude Code  ──ANTHROPIC_BASE_URL──▶  Copium proxy  ──LiteLLM (vertex_ai)──▶  Vertex AI
...
## Why not "just point Claude Code's Vertex URL at Copium"?
...
receives a byte. This is a Claude Code limitation, not a Copium bug. The native
`:rawPredict` passthrough in Copium is correct and compresses (verified by direct
...
- **Copium ML extra** for compression: `pip install "google-cloud-aiplatform>=1.38"`
  plus the Kompress ML stack (`torch`, `transformers`, `onnxruntime` — the
  `copium-ai[ml]` extra). The `kompress-v2-base` model downloads from Hugging Face
  on first use.

## Terminal 1 — start the Copium proxy (Vertex backend)
...
export ANTHROPIC_MODEL=claude-sonnet-4-6             # sent to Copium, mapped to vertex_ai/claude-sonnet-4-6
...
Claude Code now talks plain Anthropic `/v1/messages` to Copium; Copium compresses
...
if the model needs the exact bytes it can pull them back via the `copium_retrieve`
tool — compression is reversible, not destructive.

**What it does NOT touch:**

- **Your files on disk.** Copium is a network proxy: it only rewrites the *request
...
- Copium maps clean ids to Vertex publisher ids: `claude-sonnet-4-6` →
  `vertex_ai/claude-sonnet-4-6` (see `copium/backends/litellm.py`). Newer models use
...
| `tokens_saved: 0` everywhere, `/debug/warmup` shows `kompress: not installed` | ML extra missing | install `copium-ai[ml]` (torch/transformers/onnxruntime) |
