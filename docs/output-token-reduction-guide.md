...
Copium normally shrinks the prompt you **send**. This feature shrinks what the
...
You don't change any code. It runs in the Copium proxy.

## Turn it on

```bash
export COPIUM_OUTPUT_SHAPER=1     # off by default
copium proxy --port 8787
```

> **If a proxy is already running** (e.g. `copium wrap claude` attaches to one
...
> it. `copium wrap` handles this for you: it hot-syncs your current output
...
2. **Effort routing** — if a turn is just the model continuing after a tool ran
   (e.g. it read a file and there were no errors), Copium turns the model's
...
```bash
export COPIUM_VERBOSITY_LEVEL=3
```

Or — better — let Copium learn it from your habits (next section).

## Let Copium pick the level for you

People rarely *say* "be brief." They *show* it: they interrupt long answers, or
reply so fast they couldn't have read the whole thing. `copium learn
--verbosity` reads your past sessions and picks a level from those signals.

```bash
# Preview what it found (doesn't change anything)
copium learn --verbosity

# Save it — the proxy uses this level from now on
copium learn --verbosity --apply
```
...
see what the model *would* have written without our nudge. So Copium reports an
**estimate with a confidence range**, never a fake exact number:

```bash
copium output-savings
```
...
```bash
export COPIUM_OUTPUT_HOLDOUT=0.1     # 10% of conversations stay unshaped
```

Now `copium output-savings` compares shaped vs unshaped directly and reports a
...
Unset `COPIUM_OUTPUT_SHAPER` (or set it to `0`) and restart the proxy. You can
also send `x-copium-bypass: true` on a request to skip it for that call.
