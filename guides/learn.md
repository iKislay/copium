...
copium learn --project ~/my-project --apply

# Analyze all projects
copium learn --all --apply
```
...
The core innovation. Instead of cataloging failures ("Read failed 5 times"), Copium finds what the model did to fix each failure:
...
Copium manages a clearly-delimited section in each file:

```markdown
<!-- copium:learn:start -->
## Copium Learned Patterns
*Auto-generated by `copium learn` — do not edit manually*
...
<!-- copium:learn:end -->
```
...
Copium Learn uses a plugin architecture where each agent is a self-contained plugin:
...
**Plugins** bundle scanning, detection, and writing for one agent. Built-in plugins are auto-discovered from `copium.learn.plugins.*`. External plugins register via the `copium.learn_plugin` entry point.

**The Analyzer** is shared — it uses an LLM (Sonnet, GPT-4o, or Gemini Flash) to find patterns. Same analysis for any agent.

### Adding Support for a New Agent

1. Create `copium/learn/plugins/myagent.py`
2. Implement `LearnPlugin` + `ConversationScanner` (scanner + writer + detection)
3. Add `plugin = MyAgentPlugin()` at module scope
4. Done — `copium learn --agent myagent` works automatically

Or install an external plugin: `pip install copium-learn-cursor` (registers via entry point).
...
`copium learn` needs an LLM to analyze your sessions. It picks one automatically using this priority:

| Priority | Source | Example |
|----------|--------|---------|
| 1 | `--model` flag | `copium learn --model gpt-4o` |
| 2 | API key env var | `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY` |
| 3 | `COPIUM_LEARN_CLI` env var | `export COPIUM_LEARN_CLI=gemini` |
| 4 | Auto-detect installed CLIs | Checks PATH for `claude`, `gemini`, `codex` |

### Using without an API key

If you use Claude Code, Gemini CLI, or Codex via subscription (no raw API key), `copium learn` can call them directly:
...
copium learn --model gemini-cli

# Pin a CLI via environment variable
export COPIUM_LEARN_CLI=codex
copium learn
```

Valid values for `COPIUM_LEARN_CLI`: `claude`, `gemini`, `codex`.
