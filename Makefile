# Copium Rust build targets. `just` is not installed on dev boxes; this
...
help:
	@echo "Copium Rust targets:"
	@echo "  make test               - cargo test --workspace"
	@echo "  make test-parity        - maturin develop + parity-run against fixtures"
	@echo "  make bench              - cargo bench --workspace"
	@echo "  make build-proxy        - release build + strip copium-proxy, print size"
	@echo "  make build-wheel        - release wheel for copium-py"
	@echo "  make verify-rust-core   - build + install + import-verify copium._core"
...
test-parity:
	...
	$(MATURIN) develop -m crates/copium-py/Cargo.toml
	$(CARGO) run -p copium-parity -- run --fixtures $(FIXTURES)

bench:
	$(CARGO) bench --workspace

build-proxy:
	$(CARGO) build --release -p copium-proxy
	@BIN=target/release/copium-proxy; \
	if command -v strip >/dev/null 2>&1; then strip "$$BIN" || true; fi; \
	SIZE=$$(wc -c < "$$BIN"); \
	printf 'copium-proxy: %s bytes (%.1f MiB)\n' "$$SIZE" "$$(echo "$$SIZE / 1048576" | bc -l)"

build-wheel:
	$(MATURIN) build --release -m crates/copium-py/Cargo.toml
...
# mode (Finding #2 in COPIUM_PROXY_LOG_FINDINGS_2026_05_03.md). The
...
# darwin wheel, copium._core not built in test-extras + smoke-test,
...
# instantiate `SmartCrusher`, which hard-imports `copium._core`.
ci-precheck-python:
