"""Copium MCP Server — Context engineering toolkit for AI coding tools.

Exposes Copium's compression, retrieval, and observability as MCP tools
that any MCP-compatible host (Claude Code, Cursor, Codex, etc.) can use.

Tools:
    copium_compress   — Compress content on demand (no proxy needed)
    copium_retrieve   — Retrieve original uncompressed content by hash
    copium_stats      — Session compression statistics
...
CCR_TOOL_NAME = "copium_retrieve"
COMPRESS_TOOL_NAME = "copium_compress"
STATS_TOOL_NAME = "copium_stats"
READ_TOOL_NAME = "copium_read"

logger = logging.getLogger("copium.ccr.mcp")

# Feature flag: enable copium_read tool (file read caching via CCR)
# Set COPIUM_MCP_READ=on to enable
_READ_ENABLED = os.environ.get("COPIUM_MCP_READ", "off").lower().strip() in (
    "on",
    "true",
    "1",
    "yes",
...
DEFAULT_PROXY_URL = os.environ.get("COPIUM_PROXY_URL", "http://127.0.0.1:8787")
...
def _format_session_summary(summary: dict[str, Any], local_stats: dict[str, Any]) -> str:
    ...
    lines: list[str] = []
    lines.append("Copium Session Summary")
    lines.append("=" * 40)
    ...
    without = cost.get("without_copium_usd", 0)
    with_hr = cost.get("with_copium_usd", 0)
    saved = cost.get("total_saved_usd", 0)
    pct = cost.get("savings_pct", 0)
    if without > 0:
        lines.append("Cost Impact:")
        lines.append(f"  Without Copium:  ${without:.2f}")
        lines.append(f"  With Copium:     ${with_hr:.2f}")
...
# copium_stats aggregates across all instances within the session window.
# Respects COPIUM_WORKSPACE_DIR.
...
class CopiumMCPServer:
    """MCP Server exposing Copium's context engineering toolkit.

    Tools:
        copium_compress — Compress content on demand. Stores original for
                           retrieval. Works without a proxy.
        copium_retrieve — Retrieve original uncompressed content by hash.
                           Checks local store first, then proxy if configured.
        copium_stats    — Session statistics: compressions, savings, cost.
    ...
    def __init__(
        ...
        self.server: Server = Server("copium")
    ...
    def _compress_content(self, content: str) -> dict[str, Any]:
        """Compress content using Copium's pipeline.
        ...
        return {
            ...
            "note": f"Original stored with hash={hash_key}. Use mcp__copium__{CCR_TOOL_NAME} to get full content later.",
        }
    ...
    async def _retrieve_content(
        ...
        return {
            ...
            "compressed via copium_compress is stored for the session; content "
    ...
    def _setup_handlers(self) -> None:
        ...
        async def list_tools() -> list[Tool]:
            tools = [
                Tool(
                    ...
                    description=(
                        ...
                        f"The original is stored and can be retrieved later via mcp__copium__{CCR_TOOL_NAME}. "
                ...
                Tool(
                    ...
                    description=(
                        ...
                        "The hash comes from copium_compress results or from compression "
            ...
            # Conditionally add copium_read (behind feature flag)
            if _READ_ENABLED:
                tools.append(
                    Tool(
                        name=READ_TOOL_NAME,
                        description=(
                            ...
                            f"Use mcp__copium__{CCR_TOOL_NAME} with the hash to get full content if needed. "
    ...
    async def _handle_compress(self, arguments: dict[str, Any]) -> list[TextContent]:
        """Handle copium_compress tool call."""
    ...
    async def _handle_retrieve(self, arguments: dict[str, Any]) -> list[TextContent]:
        """Handle copium_retrieve tool call."""
    ...
    async def _handle_read(self, arguments: dict[str, Any]) -> list[TextContent]:
        """Handle copium_read tool call — file read with session caching."""
        import hashlib
        ...
        # in copium/ccr/ — use the centralized safe-log decoder so
                            ...
                            text=json.dumps(
                                {
                                    ...
                                    "note": (
                                        ...
                                        f"from the first read. Call mcp__copium__{CCR_TOOL_NAME}(hash='{ccr_hash}') "
    ...
    async def run_stdio(self) -> None:
        ...
        async with stdio_server() as (read_stream, write_stream):
            logger.info(f"Copium MCP Server starting (proxy: {self.proxy_url})")
...
def create_ccr_mcp_server(
    ...
    """Create a Copium MCP server instance.

    Args:
        proxy_url: URL of the Copium proxy server (for retrieval fallback).
        direct_mode: Ignored (kept for backward compatibility).

    Returns:
        CopiumMCPServer instance.
    """
    return CopiumMCPServer(proxy_url=proxy_url)


async def main() -> None:
    """Run the Copium MCP server."""
    parser = argparse.ArgumentParser(
        description="Copium MCP Server — Context engineering toolkit"
    )
    parser.add_argument(
        "--proxy-url",
        default=DEFAULT_PROXY_URL,
        help=f"Copium proxy URL for retrieval fallback (default: {DEFAULT_PROXY_URL})",
    )
    parser.add_argument(
        "--direct",
        action="store_true",
    ...
    server = CopiumMCPServer(proxy_url=args.proxy_url)
