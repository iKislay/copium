"""Copium Proxy Server - Production Ready.
...
Usage:
    python -m copium.proxy.server --port 8787
...
from copium.cache.compression_feedback import get_compression_feedback
from copium.cache.compression_store import format_retrieval_miss_detail, get_compression_store
...
from copium.config import (
    DEFAULT_EXCLUDE_TOOLS,
    CacheAlignerConfig,
    ReadLifecycleConfig,
)
from copium.dashboard import get_dashboard_html
from copium.observability import (
...
from copium.pipeline import PipelineExtensionManager, PipelineStage
from copium.providers.proxy_routes import register_provider_routes
...
from copium.proxy import runtime_env
from copium.proxy.auth_mode import should_stamp_codex_client

# =============================================================================
# Extracted modules (re-exported for backward compatibility)
# =============================================================================
from copium.proxy.cost import (
...
from copium.proxy.memory_handler import MemoryConfig, MemoryHandler

# Data models (extracted to copium/proxy/models.py for maintainability)
from copium.proxy.models import CacheEntry, ProxyConfig, RateLimitState, RequestLog  # noqa: F401
...
from copium.proxy.probe_recorder import probe_recorder_from_env
from copium.proxy.project_context import (
    classify_project,
    set_current_project,
    strip_project_path_prefix,
)
from copium.proxy.prometheus_metrics import PrometheusMetrics  # noqa: F401
from copium.proxy.rate_limiter import TokenBucketRateLimiter  # noqa: F401
from copium.proxy.request_logger import RequestLogger  # noqa: F401
from copium.proxy.semantic_cache import SemanticCache  # noqa: F401
from copium.proxy.ssl_context import find_ca_bundle
from copium.proxy.warmup import WarmupRegistry
from copium.proxy.ws_session_registry import WebSocketSessionRegistry
from copium.subscription.base import get_quota_registry, reset_quota_registry
from copium.subscription.codex_rate_limits import get_codex_rate_limit_state
from copium.subscription.copilot_quota import get_copilot_quota_tracker
from copium.subscription.tracker import (
    configure_subscription_tracker,
    get_subscription_tracker,
)
from copium.telemetry import get_telemetry_collector
from copium.telemetry.beacon import is_telemetry_enabled
from copium.telemetry.toin import get_toin
from copium.transforms import (
    CacheAligner,
    CodeAwareCompressor,
...
def _classify_agent_from_log(entry: dict[str, Any]) -> tuple[str, str, str]:
    ...
    for source, candidate in (
        ...
        ("stack", tags.get("stack") or tags.get("copium-stack")),
    ):
...
# PyTorch is optional in copium; the warning is not actionable for operators.
...
_MULTI_WORKER_CONFIG_ENV = "COPIUM_PROXY_CONFIG_JSON"

# Env var that opts out of the Rust core deployment smoke test (Hotfix-A0).
# Default behavior: hard-fail at startup if `copium._core` is unimportable
# (Finding #2 in COPIUM_PROXY_LOG_FINDINGS_2026_05_03.md — production
...
# fail-loud behavior.
_RUST_CORE_REQUIRED_ENV = "COPIUM_REQUIRE_RUST_CORE"
...
def _check_rust_core() -> tuple[str, str | None]:
    """Verify the Rust extension `copium._core` is loadable at startup.

    Returns a `(status, error)` tuple:
      - ``("loaded", None)``     — `copium._core.hello()` returned the
        expected sentinel.
    ...
    Behavior is gated by the ``COPIUM_REQUIRE_RUST_CORE`` env var:
    ...
    try:
        from copium._core import hello as _rust_hello
        ...
        msg = (
            f"FATAL: Rust extension `copium._core` not loadable.\n"
            f"    error: {reason}\n"
            f"    fix:   `make build-wheel && pip install --force-reinstall "
            f"target/wheels/copium_*.whl`\n"
    ...
    if marker != "copium-core":
        ...
        if not require:
            ...
            return ("disabled", reason)
        msg = (
            f"FATAL: Rust extension `copium._core` is loaded but the "
            f"marker function returned {marker!r}; expected 'copium-core'.\n"
            f"    fix:   rebuild: `make build-wheel && pip install "
            f"--force-reinstall target/wheels/copium_*.whl`\n"
        )
...
from copium.proxy.handlers import (  # noqa: E402
...
class CopiumProxy(
    ...
    """Production-ready Copium optimization proxy."""
    ...
    def __init__(self, config: ProxyConfig):
        ...
        CopiumProxy.ANTHROPIC_API_URL = api_targets.anthropic
        CopiumProxy.OPENAI_API_URL = api_targets.openai
        ...
        from copium.cache.prefix_tracker import PrefixFreezeConfig, SessionTrackerStore
        ...
        self._compression_executor = concurrent.futures.ThreadPoolExecutor(
            ...
            thread_name_prefix="copium-compress",
        )
        ...
        if config.memory_enabled:
            ...
            from copium.proxy.memory_handler import MemoryMode
            ...
            from copium.memory.storage_router import MemoryStorageMode
        ...
        if config.license_key:
            from copium.telemetry.reporter import UsageReporter
        ...
        if config.traffic_learning_enabled:
            from copium.memory.traffic_learner import TrafficLearner

            self.traffic_learner = TrafficLearner(
                user_id=os.environ.get("COPIUM_USER_ID", os.environ.get("USER", "default")),
                agent_type=config.traffic_learning_agent_type,
        ...
        if config.code_graph_watcher:
            from copium.graph.watcher import CodeGraphWatcher
    ...
    async def startup(self):
        ...
        logger.info("Copium Proxy started")
        ...
        if self._kompress_status == "enabled":
            ...
            logger.info("Kompress: not installed (pip install copium-ai[ml] for ML compression)")
        ...
        if self._code_aware_status == "enabled":
            ...
            logger.info("Code-Aware: not installed (pip install copium-ai[code])")
        ...
        if is_telemetry_enabled():
            logger.info(
                ...
                "Opt out: COPIUM_TELEMETRY=off or --no-telemetry"
            )
        else:
    ...
    def _print_summary(self):
        ...
        m = self.metrics
        logger.info("=" * 70)
        logger.info("COPIUM PROXY SESSION SUMMARY")
        logger.info("=" * 70)
    ...
    async def _record_request_outcome(self, outcome: RequestOutcome) -> None:
        ...
        Thin wrapper around :func:`copium.proxy.outcome.emit_request_outcome`
    ...
    def _extract_tags(self, headers: dict) -> dict[str, str]:
        ...
        """
        from copium.proxy.helpers import extract_tags
    ...
    async def _retry_request(
        ...
        *,
        ...
        Byte-faithful forwarding (PR-A3, fixes P0-2):
          ...
          * ``COPIUM_PROXY_PYTHON_FORWARDER_MODE=legacy_json_kwarg`` is an
...
def _register_memory_components(proxy: CopiumProxy, tracker: MemoryTracker) -> None:
    ...
    Args:
        proxy: The CopiumProxy instance.
...
def create_app(config: ProxyConfig | None = None) -> FastAPI:
    ...
    # Always-on file logging to ~/.copium/logs/ for `copium perf` analysis.
    # Installed here (not at module import) so importing copium.proxy.server
    ...
    # cc-switch reconciler (opt-in: COPIUM_CC_SWITCH_RECONCILE=1).
    # Keeps Copium in the request path while cc-switch overwrites
    # ~/.claude/settings.json on every provider switch. See
    # copium/proxy/cc_switch_reconciler.py for the full rationale.
    from copium.proxy.cc_switch_reconciler import (
        CCSwitchReconciler,
        reconciler_enabled,
    )
    ...
    if reconciler_enabled():
        ...
        def _set_anthropic_upstream(url: str) -> None:
            from copium.providers.registry import _normalize_api_url

            CopiumProxy.ANTHROPIC_API_URL = _normalize_api_url(
    ...
    from copium.telemetry.beacon import TelemetryBeacon

    _beacon = TelemetryBeacon(
        port=config.port if hasattr(config, "port") else 8787,
        sdk=os.environ.get("COPIUM_SDK", "proxy").strip() or "proxy",
        backend=config.backend if hasattr(config, "backend") else "anthropic",
    )
    from copium import paths as _hr_paths
    ...
    async def lifespan(app: FastAPI):  # type: ignore[no-untyped-def]
        ...
        # explicitly opted out with COPIUM_REQUIRE_RUST_CORE=false. See
        # Finding #2 in COPIUM_PROXY_LOG_FINDINGS_2026_05_03.md.
        ...
        configure_otel_metrics(OTelMetricsConfig.from_env(default_service_name="copium-proxy"))
        configure_langfuse_tracing(
            LangfuseTracingConfig.from_env(default_service_name="copium-proxy")
        )
        ...
        try:
            try:
                ...
                # CopiumProxy.ANTHROPIC_API_URL, so workers could disagree on
    ...
    def _health_checks() -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "upstream": _component_health(
                enabled=os.environ.get("COPIUM_SKIP_UPSTREAM_CHECK", "").strip() != "1",
    ...
    def _health_payload(*, include_config: bool) -> dict[str, Any]:
        ...
        deployment_profile = os.environ.get("COPIUM_DEPLOYMENT_PROFILE")
        if deployment_profile:
            payload["deployment"] = {
                "profile": deployment_profile,
                "preset": os.environ.get("COPIUM_DEPLOYMENT_PRESET"),
                "runtime": os.environ.get("COPIUM_DEPLOYMENT_RUNTIME"),
                "supervisor": os.environ.get("COPIUM_DEPLOYMENT_SUPERVISOR"),
                "scope": os.environ.get("COPIUM_DEPLOYMENT_SCOPE"),
            }
        if include_config:
            profile_kwargs = proxy_pipeline_kwargs(config)
            ...
            payload["config"] = {
                ...
                # Surfaced so `copium wrap` can see what a reused proxy is
    ...
    # every /readyz poll.  Set COPIUM_SKIP_UPSTREAM_CHECK=1 to opt out (e.g.
    ...
    async def _check_upstream() -> None:
        ...
        """
        if os.environ.get("COPIUM_SKIP_UPSTREAM_CHECK", "").strip() == "1":
    ...
    # X-Copium-Stack: SDK adapters (TS openai/anthropic/etc.) tag their
    ...
    @app.middleware("http")
    async def _record_copium_stack(request, call_next):
        started = time.perf_counter()
        inbound_id = f"inbound-{time.time_ns()}"
        # Project attribution: an explicit X-Copium-Project header wins
        ...
        try:
            from copium.proxy.helpers import redact_for_wire_debug
        ...
        if request.url.path.startswith("/v1/"):
            stack = request.headers.get("x-copium-stack")
            if stack:
                try:
    ...
    # the `copium.proxy_extension` entry-point group, but **opt-in only**:
    # only names listed in config.proxy_extensions (CLI: --proxy-extension,
    # env: COPIUM_PROXY_EXTENSIONS) actually get installed. Discovery alone
    ...
    from copium.proxy.extensions import install_all as _install_extensions
    ...
    from copium.proxy.debug_introspection import (
        collect_tasks as _collect_tasks,
    )
    from copium.proxy.loopback_guard import require_loopback as _require_loopback
    ...
    async def get_upstream():
        ...
        """
        return {
            "anthropic": CopiumProxy.ANTHROPIC_API_URL,
    ...
    async def admin_runtime_env(request: Request):
        ...
        that ``copium wrap`` reused — rather than started — never sees values
    ...
    async def _build_stats_payload() -> dict[str, Any]:
        ...
        async with _throughput_cache_lock:
            ...
            if _throughput_cache["expires_at"] < now or _throughput_cache["value"] is None:
                ...
                def _compute_throughput():
                    from copium.perf.analyzer import build_perf_summary, parse_log_files
        ...
        # Calculate Copium overhead (optimization time only, excludes pass-through requests)
        avg_overhead_ms = (
        ...
        # Calculate total tokens before Copium-side reduction. Proxy
        ...
        try:
            from copium.proxy.output_savings import get_recorder
        ...
        return {
            ...
            "savings": {
                ...
                "by_layer": {
                    ...
                    "compression": {
                        ...
                        "description": (
                            "Tokens removed by Copium proxy compression. "
                    ...
                    "prefix_cache": {
                        ...
                        "description": (
                            ...
                            "Copium's CacheAligner improves hit rates; "
            ...
            **recent_request_payload,
            "request_logging_enabled": proxy.logger is not None,
            "compression_enabled": bool(getattr(proxy.config, "optimize", False)),
    ...
    async def stats_history(
        ...
        if format == "csv":
            filename = f"copium-stats-history-{series}.csv"
            return Response(
    ...
    # CCR Tool Call Handler - for agent frameworks to call when LLM uses copium_retrieve
...
def _proxy_config_from_env() -> ProxyConfig:
    ...
    if raw_config:
        try:
            ...
            logger.warning(
                "Invalid %s; falling back to COPIUM_* env vars", _MULTI_WORKER_CONFIG_ENV
            )

    return ProxyConfig(
        host=_get_env_str("COPIUM_HOST", "127.0.0.1"),
        port=_get_env_int("COPIUM_PORT", 8787),
        ...
        backend=_get_env_str("COPIUM_BACKEND", "anthropic"),
        bedrock_region=_get_env_str("COPIUM_BEDROCK_REGION", "us-west-2"),
        bedrock_profile=os.environ.get("AWS_PROFILE"),
        bedrock_api_url=os.environ.get("BEDROCK_TARGET_API_URL"),
        anyllm_provider=_get_env_str("COPIUM_ANYLLM_PROVIDER", "openai"),
        disable_kompress=_get_env_bool("COPIUM_DISABLE_KOMPRESS", False),
        max_connections=_get_env_int("COPIUM_MAX_CONNECTIONS", 500),
        max_keepalive_connections=_get_env_int("COPIUM_MAX_KEEPALIVE", 100),
        http2=_get_env_bool("COPIUM_HTTP2", True),
        mode=normalize_proxy_mode(_get_env_str("COPIUM_MODE", PROXY_MODE_TOKEN)),
    )
...
def _get_code_aware_banner_status(config: ProxyConfig) -> str:
    ...
    if config.code_aware_enabled:
        if is_tree_sitter_available():
            ...
            return "NOT INSTALLED (pip install copium-ai[code])"
    else:
        if is_tree_sitter_available():
            return "DISABLED (--code-aware or COPIUM_CODE_AWARE_ENABLED=1 to enable)"
        return "DISABLED  (install copium-ai[code] to enable)"
...
def run_server(
    ...
    Args:
        ...
        print_banner: When False, skip the legacy ASCII banner. The
            Click CLI (`copium proxy`) prints its own startup banner
            before calling this — printing a second banner here is the
            "dual banner" UX issue. Direct `python -m copium.proxy.server`
...
""")
    ...
    if workers > 1:
        ...
        # cross-worker backend is configured via COPIUM_CCR_BACKEND.
        # See RUST_DEV.md -> "Multi-worker deployment -- CCR fragmentation".
        if os.environ.get("COPIUM_CCR_BACKEND", "").strip():
            logger.warning(
                "Copium is running with workers=%d. Compression cache, "
            ...
            logger.warning(
                "Copium is running with workers=%d. The in-memory CCR store, "
                ...
                "sessions land on different workers. Set COPIUM_CCR_BACKEND=sqlite for a "
        ...
        app_target = "copium.proxy.server:create_app_from_env"
...
def _parse_exclude_tools(cli_excludes: str | None) -> set[str]:
    ...
    Both --exclude-tools and COPIUM_EXCLUDE_TOOLS are comma-separated
    ...
    used unchanged).
    """
    raw = ",".join(s for s in (cli_excludes, os.environ.get("COPIUM_EXCLUDE_TOOLS")) if s)
    names: set[str] = set()
...
def _parse_tool_profiles(cli_profiles: list[str]) -> dict[str, Any]:
    """Parse tool profiles from CLI args and COPIUM_TOOL_PROFILES env var.
    ...
    """
    from copium.config import PROFILE_PRESETS, CompressionProfile

    profiles: dict[str, CompressionProfile] = {}
    raw_entries: list[str] = list(cli_profiles)

    # Also check env var
    env_val = os.environ.get("COPIUM_TOOL_PROFILES", "")
    if env_val:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Copium Proxy Server")
    ...
    parser.add_argument(
        ...
        "Can be specified multiple times. Also settable via COPIUM_TOOL_PROFILES env var.",
    )
    parser.add_argument(
        ...
        help=(
            ...
            "Also settable via COPIUM_COMPRESS_USER_MESSAGES=1."
        ),
    )
    parser.add_argument(
        "--disable-kompress",
        ...
        help=(
            ...
            "Also settable via COPIUM_DISABLE_KOMPRESS=1."
        ),
    )
    parser.add_argument(
        "--exclude-tools",
        ...
        "Also settable via COPIUM_EXCLUDE_TOOLS env var.",
    )
    ...
    parser.add_argument(
        "--code-aware",
        action="store_true",
        help="Enable AST-based code compression (requires: pip install copium-ai[code])",
    )
    parser.add_argument(
        "--no-code-aware",
        action="store_true",
    ...
    # Environment variable defaults (COPIUM_* prefix)
    # CLI args override env vars, env vars override ProxyConfig defaults
    env_code_aware = _get_env_bool("COPIUM_CODE_AWARE_ENABLED", True)
    env_optimize = _get_env_bool("COPIUM_OPTIMIZE", True)
    env_cache = _get_env_bool("COPIUM_CACHE_ENABLED", True)
    env_rate_limit = _get_env_bool("COPIUM_RATE_LIMIT_ENABLED", True)
    ...
    disable_kompress = args.disable_kompress or _get_env_bool("COPIUM_DISABLE_KOMPRESS", False)
    ...
    config = ProxyConfig(
        host=_get_env_str("COPIUM_HOST", args.host),
        port=_get_env_int("COPIUM_PORT", args.port),
        ...
        anyllm_provider=_get_env_str("COPIUM_ANYLLM_PROVIDER", args.anyllm_provider),
        optimize=optimize,
        min_tokens_to_crush=_get_env_int("COPIUM_MIN_TOKENS", args.min_tokens),
        max_items_after_crush=_get_env_int("COPIUM_MAX_ITEMS", args.max_items),
        smart_crusher_with_compaction=(
            _get_env_bool("COPIUM_SMART_CRUSHER_COMPACTION", False)
            if "COPIUM_SMART_CRUSHER_COMPACTION" in os.environ
            else None
        ),
        cache_enabled=cache_enabled,
        cache_ttl_seconds=_get_env_int("COPIUM_CACHE_TTL", args.cache_ttl),
        rate_limit_enabled=rate_limit_enabled,
        rate_limit_requests_per_minute=_get_env_int("COPIUM_RPM", args.rpm),
        rate_limit_tokens_per_minute=_get_env_int("COPIUM_TPM", args.tpm),
        budget_limit_usd=args.budget,
        budget_period=args.budget_period,
        log_file=_get_env_str("COPIUM_LOG_FILE", args.log_file)
        if args.log_file
        else os.environ.get("COPIUM_LOG_FILE"),
        log_full_messages=args.log_messages or _get_env_bool("COPIUM_LOG_MESSAGES", False),
        code_aware_enabled=code_aware_enabled,
        disable_kompress=disable_kompress,
        # Connection pool settings
        max_connections=_get_env_int("COPIUM_MAX_CONNECTIONS", args.max_connections),
        max_keepalive_connections=_get_env_int("COPIUM_MAX_KEEPALIVE", args.max_keepalive),
        http2=not args.no_http2 and _get_env_bool("COPIUM_HTTP2", True),
        ...
        or _get_env_bool("COPIUM_COMPRESS_USER_MESSAGES", False),
        savings_profile=os.environ.get("COPIUM_SAVINGS_PROFILE") or None,
        # Default 0.4 keep-ratio so the Kompress text (prose/code) path compresses
        # meaningfully out of the box; COPIUM_TARGET_RATIO overrides.
        target_ratio=(
            float(os.environ["COPIUM_TARGET_RATIO"])
            if os.environ.get("COPIUM_TARGET_RATIO")
            else 0.4
        ),
        compress_system_messages=(
            _get_env_bool("COPIUM_COMPRESS_SYSTEM_MESSAGES", False)
            if "COPIUM_COMPRESS_SYSTEM_MESSAGES" in os.environ
            else None
        ),
        protect_recent=(
            int(os.environ["COPIUM_PROTECT_RECENT"])
            if os.environ.get("COPIUM_PROTECT_RECENT")
            else None
        ),
        protect_analysis_context=(
            _get_env_bool("COPIUM_PROTECT_ANALYSIS_CONTEXT", False)
            if "COPIUM_PROTECT_ANALYSIS_CONTEXT" in os.environ
            else None
        ),
        accuracy_guard=os.environ.get("COPIUM_ACCURACY_GUARD") or None,
    )

    # Get worker and concurrency settings
    workers = _get_env_int("COPIUM_WORKERS", args.workers)
    limit_concurrency = _get_env_int("COPIUM_LIMIT_CONCURRENCY", args.limit_concurrency)
