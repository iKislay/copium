"""Cost tracking and budget management for the Copium proxy.
...
from copium.proxy.modes import PROXY_MODE_CACHE
...
def header_safe_transforms(transforms: list[str]) -> list[str]:
    ...
    ``x-copium-transforms`` is built as ``",".join(transforms_applied)``, so a
...
def build_prefix_cache_stats(
    ...
    for provider, pc in metrics.cache_by_provider.items():
        ...
        # client (e.g. Claude Code) pays regardless of Copium. We track it
    ...
    return {
        ...
        "attribution": (
            ...
            "Copium reports cache stats as observed from API responses. "
            "CacheAligner and prefix freeze improve cache hit rates by stabilizing "
            "the message prefix, but baseline caching happens without Copium. "
...
def _aggregate_mcp_events() -> dict[str, int]:
    """Aggregate compression / retrieval events written by Copium MCP
    server instances to the cross-process shared events file.

    The Copium MCP server (``copium mcp serve``) records every
    ``copium_compress`` and ``copium_retrieve`` invocation to a
    file-locked shared log (see :func:`copium.ccr.mcp_server._append_shared_event`).
    ...
    Keys: ``compressions`` (count of copium_compress calls),
    ``tokens_removed`` (sum of input_tokens-output_tokens across
    compress events), ``retrievals`` (count of copium_retrieve
    calls — the load-bearing over-compression signal).
    """
    zero = {"compressions": 0, "tokens_removed": 0, "retrievals": 0}
    try:
        from copium.ccr.mcp_server import _read_shared_events
    except ImportError:
        return zero
...
def build_session_summary(
    ...
    cost_with = cost_stats.get("cost_with_copium_usd", 0.0)
    compression_savings = cost_stats.get("savings_usd", 0.0)
    ...
    summary: dict[str, Any] = {
        ...
        "cost": {
            "without_copium_usd": round(cost_without, 2),
            "with_copium_usd": round(cost_with, 2),
    ...
    # MCP-side compression: events written by `copium mcp serve`
    ...
    if proxy.config.mode == PROXY_MODE_CACHE and uncompressed_reasons["prefix_frozen"] > 10:
        summary["tip"] = (
            "Most requests are prefix-frozen. Set COPIUM_MODE=token "
...
class CostTracker:
    ...
    def estimate_cost(
        ...
        try:
            from copium.pricing.litellm_pricing import resolve_litellm_model
    ...
    def record_tokens(
        ...
        Args:
            model: Model name.
            tokens_saved: Tokens removed by compression (Copium's count).
            tokens_sent: Compressed message tokens sent (Copium's count).
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            "total_input_cost_usd": round(cost_with_copium, 4),
            ...
            "per_model": per_model,
            "cost_with_copium_usd": round(cost_with_copium, 4),
            "savings_usd": round(savings_usd, 4),
            # Budget config passthrough — surfaces in /stats["cost"] so
            # `copium doctor` can report whether a budget is set.
