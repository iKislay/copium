...
# reused proxy re-synced by ``copium wrap`` takes effect without a restart.
def _min_chars_to_rewrite() -> int:
    try:
        return int(runtime_env.getenv("COPIUM_INTERCEPT_READ_MIN_CHARS", "500"))
...
OUTLINE_MARKER = "    # ... (body elided by Copium; Read a specific line range to see it)\n"
...
def _run_ast_grep(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="copium-sg-"))
    try:
