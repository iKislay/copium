...
:meth:`CopiumProxy._record_request_outcome`), which owns the four
...
class RequestOutcome:
    ...
    # Response-cache hit (Copium's own semantic cache served the
    ...
    #     via :func:`copium.proxy.auth_mode.classify_client`. The
    ...
    def cache_hit(self) -> bool:
        ...
        was served from Copium's own response cache.
...
async def emit_request_outcome(handler: Any, outcome: RequestOutcome) -> None:
    ...
    Owns the four downstream effects in canonical order:
      ...
      4. structured PERF log line — consumed by ``copium perf``

    Takes the handler as a free argument rather than ``self`` so this
    function is callable from:
    * ``CopiumProxy._record_request_outcome`` (production)
    ...
    * any provider handler mixin
    ...
    """
    from copium.proxy.cost import _summarize_transforms
    from copium.proxy.models import RequestLog
    from copium.proxy.project_context import get_current_project
    ...
    # HTTP middleware / WS accept captured from ``X-Copium-Project``.
    ...
    #    line unchanged, and gives ``copium perf --client X``
