"""Data models for the Copium proxy.
...
class RequestLog:
    ...
    # See ``copium.proxy.helpers.compute_turn_id`` for the derivation. None
...
class ProxyConfig:
    ...
    # CLI: --disable-kompress; env: COPIUM_DISABLE_KOMPRESS=1.
    ...
    # CLI: --exclude-tools <name1,name2>; env: COPIUM_EXCLUDE_TOOLS=<name1,name2>
    ...
    # to enable from the `copium.proxy_extension` group, or `["*"]` for
    # wildcard. Empty/None means no extensions run, even if installed.
    # CLI: --proxy-extension <name1,name2>; env: COPIUM_PROXY_EXTENSIONS.
    ...
    memory_db_path: str = ""  # Empty = auto: {cwd}/.copium/memory.db
    ...
    # x-copium-user-id only. ``global`` keeps the pre-fix single-DB
    ...
    # ``COPIUM_COMPRESSION_MAX_WORKERS``.
    #
    ...
    # ``CopiumProxy._run_compression_in_executor``.
