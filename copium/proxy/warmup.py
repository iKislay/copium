...
``CopiumProxy.startup`` so that request-time use shares one in-process
singleton and concurrent first-use callers do not trigger N parallel loads.

The registry is populated by ``CopiumProxy.startup`` and exposed as
...
class WarmupRegistry:
    """Shared preloaded asset registry populated by ``CopiumProxy.startup``.

    One instance per ``CopiumProxy``. All writes happen on the startup
