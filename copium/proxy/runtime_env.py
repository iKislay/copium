...
Most Copium settings are read once at proxy startup into ``Config`` and are
...
process environment, so a *reused* proxy — one ``copium wrap`` attaches to
...
process-global override store. ``copium wrap`` pushes the values it would
...
(``COPIUM_TARGET_RATIO`` etc.) do not belong here — a fresh proxy already
...
RUNTIME_ENV_KNOBS: tuple[Knob, ...] = (
    Knob("COPIUM_OUTPUT_SHAPER", "bool", "Master switch for output-token shaping."),
    Knob(
        "COPIUM_VERBOSITY_LEVEL", "int", "Verbosity steering level 0-4 (unset = learned/default)."
    ),
    Knob("COPIUM_EFFORT_ROUTER", "bool", "Lower effort on mechanical tool-result continuations."),
    Knob("COPIUM_MECHANICAL_EFFORT", "str", "Effort value used on mechanical continuations."),
    Knob("COPIUM_VERBOSITY_AUTOTUNE", "bool", "Use the AIMD verbosity controller state."),
    Knob(
        "COPIUM_OUTPUT_HOLDOUT",
        "float",
        "Fraction of conversations held out for A/B measurement.",
    ),
    Knob(
        "COPIUM_INTERCEPT_READ_MIN_CHARS",
        "int",
...
def explicit_env(environ: Mapping[str, str] | None = None) -> dict[str, str]:
    ...
    a default on a shared proxy, set it explicitly (e.g. ``COPIUM_OUTPUT_SHAPER=0``).
    """
