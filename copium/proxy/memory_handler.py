...
This module provides memory capabilities for the Copium proxy:
...
if TYPE_CHECKING:
    from copium.memory.backends.local import LocalBackend
...
# Memory tool names for detection (Copium's custom tools)
MEMORY_TOOL_NAMES = {
    "memory_save",
    "memory_search",
    "memory_update",
...
class MemoryConfig:
    ...
    Qdrant connection fields default to values read from ``COPIUM_QDRANT_*``
    environment variables (see :mod:`copium.memory.qdrant_env`). Passing an
    explicit value to the constructor always wins over the environment.
    """

    enabled: bool = False
    backend: Literal["local", "qdrant-neo4j"] = "local"
    db_path: str = "copium_memory.db"
    ...
    native_memory_dir: str = ""  # Directory for native memory files (default: ~/.copium/memories)
    # Qdrant+Neo4j config (Qdrant defaults resolve from COPIUM_QDRANT_* env vars)
    ...
    # Memory Bridge (bidirectional markdown <-> Copium sync)
...
class MemoryHandler:
    ...
    Supports two modes:
    - Custom tools: Copium's memory_save, memory_search, etc. (default)
    ...
    def _init_native_memory_dir(self) -> None:
        ...
        if self.config.native_memory_dir:
            ...
            # Default: workspace memories directory (respects COPIUM_WORKSPACE_DIR)
    ...
    async def _init_backend_locked(self) -> None:
        ...
        if self.config.backend == "local":
            ...
            # Opt-in GPU offload: COPIUM_EMBEDDER_RUNTIME=pytorch_mps routes embedding
            ...
            if os.environ.get("COPIUM_EMBEDDER_RUNTIME", "").strip().lower() == "pytorch_mps":
                try:
                    ...
                    if torch.backends.mps.is_available():
                        ...
                        logger.info(
                            "Memory: COPIUM_EMBEDDER_RUNTIME=pytorch_mps → "
                            "torch embedder on Apple GPU (MPS)"
                        )
                    else:
                        logger.warning(
                            "Memory: COPIUM_EMBEDDER_RUNTIME=pytorch_mps requested but "
            ...
            try:
                from copium.memory.backends.direct_mem0 import (
                    DirectMem0Adapter,
                    Mem0Config,
                )
                ...
                logger.error(
                    ...
                    "Install with: pip install 'copium-ai[memory-stack]'"
                )
                raise
        else:
    ...
    def _get_memory_tools(self) -> list[dict[str, Any]]:
        ...
        if self._memory_tools is None:
            from copium.memory.tools import get_memory_tools_optimized
        ...
        *,
        ...
        Args:
            user_id: User identifier for memory scoping (the base user
                id, derived from ``x-copium-user-id`` upstream).
            ...
            ranker: Optional :class:`~copium.proxy.memory_ranker.MemoryRanker`
        ...
        """
        from copium.proxy.memory_injection import MemoryInjectionBudget
        ...
        # but no x-copium-project-id / x-copium-cwd / system-prompt
        ...
        try:
            ...
            if ranker is not None:
                from copium.proxy.memory_ranker import MemoryCandidate
    ...
    def _append_to_latest_user_tail(
        ...
        *,
        ...
        if provider == "anthropic":
            ...
            # copium.proxy.handlers.anthropic which imports MemoryHandler.
        ...
        if provider == "openai":
            from copium.proxy.helpers import append_text_to_latest_user_chat_message
