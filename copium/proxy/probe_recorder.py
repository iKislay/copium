...
``copium evals probes`` can measure what compression removed from real
proxied sessions. Activated only when ``COPIUM_PROBE_RECORD_DIR`` is set;
...
from copium.pipeline import PipelineEvent, PipelineStage

logger = logging.getLogger(__name__)

RECORD_DIR_ENV = "COPIUM_PROBE_RECORD_DIR"
...
def probe_recorder_from_env() -> CompressionEventRecorder | None:
    """Build a recorder when ``COPIUM_PROBE_RECORD_DIR`` is set, else None.
