"""Top-level helper functions and constants for the Copium proxy.
...
_CODEX_WIRE_DEBUG_ENV = "COPIUM_CODEX_WIRE_DEBUG"
_CODEX_WIRE_DEBUG_DIR_ENV = "COPIUM_CODEX_WIRE_DEBUG_DIR"
...
_PYTHON_FORWARDER_MODE_ENV = "COPIUM_PROXY_PYTHON_FORWARDER_MODE"
...
def extract_tags(headers: Any) -> dict[str, str]:
    """Extract ``x-copium-*`` tags from inbound headers.
...
def _copium_bypass_enabled(headers: Any) -> bool:
    ...
    try:
        bypass = str(headers.get("x-copium-bypass", "")).strip().lower() == "true"
        passthrough = str(headers.get("x-copium-mode", "")).strip().lower() == "passthrough"
...
_RTK_GAIN_SCOPE_ENV = "COPIUM_RTK_GAIN_SCOPE"
...
RTK_STATS_CACHE_TTL_SECONDS = float(os.environ.get("COPIUM_CONTEXT_TOOL_STATS_TTL_SECONDS", "60"))
...
# COPIUM_SSE_BUFFER_MAX_BYTES. Guards against pathological huge events
...
_SSE_EVENT_MAX_BYTES_ENV = "COPIUM_SSE_BUFFER_MAX_BYTES"
...
# Configurable via COPIUM_PROXY_BODY_TOO_LARGE_STATUS for operators who need
# to override (no expected production use; documentation knob).
_BODY_TOO_LARGE_STATUS_ENV = "COPIUM_PROXY_BODY_TOO_LARGE_STATUS"
...
# COPIUM_COMPRESSION_TIMEOUT_SECONDS env var for slow CPUs or long Claude Code
# conversations (GH #946). Falls back to 30 on an unparseable value.
try:
    COMPRESSION_TIMEOUT_SECONDS = float(
        os.environ.get("COPIUM_COMPRESSION_TIMEOUT_SECONDS", "30")
    )
except ValueError:
...
# Copium thought was large enough to compress, the legacy behaviour was to
...
# upstream (which Copium's earlier successful compressions shrunk), then
...
# ``COPIUM_WS_FAIL_OPEN_ON_COMPRESSION_FAILURE=1``. The oversize threshold
# below which transient errors still fall through to passthrough is
# configurable via ``COPIUM_WS_COMPRESSION_FAIL_THRESHOLD_BYTES``
# (default 256 KiB ≈ 64K tokens).
WS_COMPRESSION_FAIL_OPEN_ENV = "COPIUM_WS_FAIL_OPEN_ON_COMPRESSION_FAILURE"
WS_COMPRESSION_OVERSIZE_BYTES_ENV = "COPIUM_WS_COMPRESSION_FAIL_THRESHOLD_BYTES"
...
def decide_compression_failure_action(
    ...
    * Codex client compression timeout → forward. Codex currently treats
      ...
      standalone rather than through ``copium wrap codex``.
    * exception is :class:`asyncio.TimeoutError` → refuse (the compression
      stage hit its own timeout, which only fires on frames Copium
...
def _get_image_compressor():
    ...
    try:
        from copium.image import ImageCompressor
...
# Always-on file logging to the workspace logs directory for `copium perf` analysis.
# Resolved lazily so COPIUM_WORKSPACE_DIR env-var changes are honored.


def _copium_log_dir() -> Path:
    return _paths.log_dir()


def _setup_file_logging() -> None:
    """Add a RotatingFileHandler to the copium root logger.

    Writes to ~/.copium/logs/proxy.log with automatic rotation:
    - Rotates at 10 MB
    ...
    try:
        ...
        # Attach to the copium root logger so all sub-loggers are captured.
        # Disable propagation to root to avoid duplicate writes when
        # wrap.py redirects stderr to the same log file.
        copium_logger = logging.getLogger("copium")
        copium_logger.setLevel(logging.INFO)
        if not any(isinstance(h, RotatingFileHandler) for h in copium_logger.handlers):
            copium_logger.addHandler(handler)
        copium_logger.propagate = False
    except OSError:
...
def _context_tool_summary_payload(
    *,
    ...
    Copium computes session values by subtracting a startup baseline, so
...
def _get_context_tool_stats() -> dict[str, Any] | None:
    """Get context-tool savings for the current Copium proxy session.
...
# ---------------------------------------------------------------------------
#
# `x-copium-*` request headers (e.g. ``x-copium-bypass``,
# ``x-copium-mode``, ``x-copium-user-id``, ``x-copium-stack``,
# ``x-copium-base-url``) are internal control flags consumed by the
...
# ``x-copium-*``, memory ``x-copium-user-id`` lookup) keep using
# the original dict / ``request.headers``. The stripped copy is what
# every upstream-bound forwarder receives.
#
# Note: response-side ``X-Copium-*`` injection (e.g.
# ``x-copium-tokens-saved``) is unrelated — the proxy is allowed to
# tell its client about its own work. This helper only filters
# request-side headers.

_INTERNAL_HEADER_PREFIX = "x-copium-"
...
def _strip_internal_headers(headers: dict[str, str]) -> dict[str, str]:
    """Return a copy of ``headers`` with internal ``x-copium-*`` keys stripped.

    Used at every upstream call site to prevent fingerprinting / leakage of
    internal flags like ``x-copium-bypass``, ``x-copium-mode``,
    ``x-copium-user-id``, ``x-copium-stack``, ``x-copium-base-url``.
    Case-insensitive on the prefix. Returns a NEW dict; never mutates the
    caller's mapping. Pure function. No regex.

    When the operator opt-in ``COPIUM_STRIP_INTERNAL_HEADERS=disabled``
...
#      client value already contained a Copium-required token.
#
...
#     then Copium-required tokens (in the order passed). Dedupe is
...
_BETA_TRACKER_MAX_SESSIONS_ENV = "COPIUM_BETA_TRACKER_MAX_SESSIONS"
...
def _merge_beta_tokens(client_value: str | None, copium_required: list[str]) -> str:
    ...
    * Copium-required tokens append in the order given, skipping any
...
def merge_anthropic_beta(client_value: str | None, copium_required: list[str]) -> str:
    """Merge client `anthropic-beta` value with Copium-required tokens.

    See `_merge_beta_tokens` for full semantics. Order is deterministic:
    client tokens first (in their original order), then copium tokens
    ...
    Returns ``""`` when both inputs are empty.
    """
    return _merge_beta_tokens(client_value, copium_required)


def merge_openai_beta(client_value: str | None, copium_required: list[str]) -> str:
    """Merge client `OpenAI-Beta` value with Copium-required tokens.
...
class SessionBetaTracker:
    ...
    def record_and_get_sticky_betas(
        ...
        When `COPIUM_BETA_HEADER_STICKY=disabled` returns the client
...
def get_session_beta_tracker() -> SessionBetaTracker:
    ...
    Lazily constructed so the env-var bound (`COPIUM_BETA_TRACKER_MAX_SESSIONS`)
...
def log_beta_header_merge(
    *,
    ...
    copium_added: list[str],
    request_id: str | None,
) -> None:
    """Structured log for every cache-affecting beta-header merge.

    `copium_added` is a list of public, documented beta tokens
    ...
    Copium logs. Emitting counts only makes the decision auditable.
    """
    logger.info(
        "event=beta_header_merge provider=%s session_id=%s "
        "client_betas=%d sticky_betas=%d copium_added=%s request_id=%s",
        provider,
        session_id or "",
        client_betas_count,
        sticky_betas_count,
        ",".join(copium_added) if copium_added else "",
        request_id or "",
    )
...
_TOOL_INJECTION_STICKY_ENV = "COPIUM_TOOL_INJECTION_STICKY"
ToolInjectionStickyMode = Literal["enabled", "disabled"]
_TOOL_INJECTION_STICKY_DEFAULT: ToolInjectionStickyMode = "enabled"

_TOOL_TRACKER_MAX_SESSIONS_ENV = "COPIUM_TOOL_TRACKER_MAX_SESSIONS"
...
def serialize_tool_definition_canonical(tool_definition: dict[str, Any]) -> bytes:
    ...
    ``copium/proxy/memory_handler.py`` and
    ``copium/proxy/memory_tool_adapter.py`` already do.
...
def get_session_tool_tracker() -> SessionToolTracker:
    ...
    (`COPIUM_TOOL_TRACKER_MAX_SESSIONS`) is honored at first use.
...
def apply_session_sticky_memory_tools(
    *,
    ...
    Logic (guide §6.3 #2):
      ...
      * If ``COPIUM_TOOL_INJECTION_STICKY=disabled``: bypass tracker,
...
class SessionCcrTracker:
    ...
    Two pieces of state per session:
      ...
      * ``golden_tool_bytes``: canonical serialization of the
        ``copium_retrieve`` tool definition recorded the first time
...
def apply_session_sticky_ccr_tool(
    *,
    ...
    """
    from copium.ccr.tool_injection import CCR_TOOL_NAME, create_ccr_tool_definition
...
# ``copium wrap claude``). For users who run ``claude`` manually we cannot
...
def format_tool_search_disabled_hint(tools: list[Any]) -> str:
    ...
    return (
        ...
        "or launch via `copium wrap claude` (which sets it automatically). "
        "See https://github.com/iKislay/copium/issues/746"
    )
