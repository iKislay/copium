...
Direct port of ``crates/copium-core/src/auth_mode.rs``. The two
...
class AuthMode(str, enum.Enum):
    """Three auth-mode classes Copium routes compression policy through.
    ...
    #: ``accept-encoding``, never strip; never inject ``X-Copium-*``;
...
# short normalized client name. The dashboard / `copium perf` use
...
CLIENT_UA_MAP: tuple[tuple[str, str], ...] = (
    ...
    # explicitly (see `copium/integrations/strands/README.md`).
...
def classify_client(headers: Mapping[str, Any] | Any, *, default: str | None = None) -> str | None:
    ...
    1. **``X-Client`` header** (explicit override) — clients that
       know they're talking to Copium can self-identify with a
