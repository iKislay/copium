...
This is the input-side analog of :class:`copium.proxy.outcome.RequestOutcome`.
...
``x-copium-bypass: true`` requests were silently ignored on every
...
class CompressionDecision:
    ...
    #   * ``"bypass_header"``      — user set x-copium-bypass or
    ...
    def decide(
        ...
        *,
        ...
        headers
            ...
            Both ``x-copium-bypass: true`` and
            ``x-copium-mode: passthrough`` trigger bypass — semantics
            mirrored from :func:`copium.proxy.helpers._copium_bypass_enabled`.
        config
            ``CopiumConfig``-shaped object; only ``optimize: bool``
            is read.
        usage_reporter
