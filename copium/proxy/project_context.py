...
``copium wrap`` launches agents with an ``X-Copium-Project`` header
...
from copium.proxy.savings_tracker import sanitize_project_name

PROJECT_HEADER = "x-copium-project"
PROJECT_PATH_PREFIX = "/p/"

_current_project: ContextVar[str | None] = ContextVar("copium_current_project", default=None)
...
def classify_project(headers: Mapping[str, Any] | Any) -> str | None:
    ...
    if get is None:
        return None
    value = get(PROJECT_HEADER) or get("X-Copium-Project")
...
def with_project_prefix(base_url: str, project: str | None) -> str:
    ...
    ``copium wrap`` for clients that cannot send custom headers.
