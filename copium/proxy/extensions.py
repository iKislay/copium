...
External packages hook into the Copium proxy at startup by declaring an
entry point in the ``copium.proxy_extension`` group in their ``pyproject.toml``:

    [project.entry-points."copium.proxy_extension"]
...
Enabling extensions:
  ...
  * CLI:  ``copium proxy --proxy-extension shield_enterprise,mypkg``
  * Env:  ``COPIUM_PROXY_EXTENSIONS=shield_enterprise,mypkg``
...
ENTRY_POINT_GROUP = "copium.proxy_extension"
ENV_VAR = "COPIUM_PROXY_EXTENSIONS"
...
def _resolve_enabled(enabled: Iterable[str] | None) -> set[str]:
    ...
    Precedence: explicit ``enabled`` argument > ``COPIUM_PROXY_EXTENSIONS``
