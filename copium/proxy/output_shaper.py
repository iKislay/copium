...
Copium's transforms compress what goes INTO the model. This module is the
...
_STEERING_SENTINEL = "<copium_output_shaping>"
_STEERING_SUFFIX = "</copium_output_shaping>"
...
class OutputShaperSettings:
    ...
    Env-driven (like COPIUM_INTERCEPT_ENABLED) so the proxy picks it up
    ...
    def from_env(cls) -> OutputShaperSettings:
        enabled = runtime_env.getenv("COPIUM_OUTPUT_SHAPER", "").lower() in (
            "1",
            "true",
            "yes",
        )
        try:
            level = int(runtime_env.getenv("COPIUM_VERBOSITY_LEVEL", "2"))
        except ValueError:
            level = 2
        level = max(0, min(4, level))
        router = runtime_env.getenv("COPIUM_EFFORT_ROUTER", "1").lower() not in (
            "0",
            "false",
            "no",
        )
        mech = runtime_env.getenv("COPIUM_MECHANICAL_EFFORT", "low")
...
def resolve_verbosity_level(settings: OutputShaperSettings) -> tuple[int, str]:
    ...
    Precedence:
      1. ``COPIUM_VERBOSITY_LEVEL`` set explicitly → manual override.
      2. AIMD controller state (when ``COPIUM_VERBOSITY_AUTOTUNE`` is on).
    ...
    """
    if runtime_env.getenv("COPIUM_VERBOSITY_LEVEL"):
    ...
    autotune = runtime_env.getenv("COPIUM_VERBOSITY_AUTOTUNE", "").lower() in ("1", "true", "yes")
    if autotune:
