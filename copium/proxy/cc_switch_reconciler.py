"""cc-switch reconciler: keep Copium in the request path without fighting cc-switch.
...
2. **Captures** the real provider endpoint and sets it as Copium's upstream
   (runtime, no restart -- ``CopiumProxy.ANTHROPIC_API_URL`` is a class attr
   read per request).
3. **Rewrites only** ``env.ANTHROPIC_BASE_URL`` back to Copium's local URL,
   leaving the token / model / everything else untouched.

Result: ``Claude -> Copium (compress) -> selected provider``. cc-switch never
knows; it is the *trigger*, not a coordination partner. The token rides in the
request (Claude -> Copium -> upstream, passed through verbatim); Copium
never reads or stores it.

Safety
------
- Loop-safe: once ``base_url`` already equals Copium's URL, it is left alone.
- Official / empty env (``{"env": {}}``, what cc-switch writes for "Claude
  ...
  base URL is fragile, so v1 does not route it through Copium. Set
  ``COPIUM_CC_SWITCH_ROUTE_OFFICIAL=1`` to route official through Copium too.
- Gated entirely behind ``COPIUM_CC_SWITCH_RECONCILE=1`` -- off by default, so
...
def reconciler_enabled() -> bool:
    return os.environ.get("COPIUM_CC_SWITCH_RECONCILE", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def _route_official() -> bool:
    return os.environ.get("COPIUM_CC_SWITCH_ROUTE_OFFICIAL", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
class CCSwitchReconciler:
    """Polls Claude settings.json and keeps Copium in the path (see module docstring)."""
    ...
    def tick(self) -> bool:
        ...
        if not url:
            if _route_official():
                ...
                logger.info("cc-switch reconciler: official -> route via Copium")
                return True
    ...
    def _atomic_write(self, data: dict) -> None:
        # Per-process temp name: multiple Copium processes reconciling the
