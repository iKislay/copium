"""Streaming handler mixin for CopiumProxy.
...
from copium.copilot_auth import apply_copilot_api_auth
...
class StreamingMixin:
    """Mixin providing streaming response methods for CopiumProxy."""
    ...
    def _parse_sse_usage(self, chunk: bytes, provider: str) -> dict[str, int] | None:
        ...
        """
        from copium.proxy.helpers import parse_sse_events_from_byte_buffer
    ...
    def _record_ccr_feedback_from_response(
        ...
        """Extract copium_retrieve tool calls from a response and record feedback.
        ...
        for block in content:
            ...
            if block.get("type") != "tool_use":
                continue
            if block.get("name") != "copium_retrieve":
                continue
    ...
    def _record_ccr_feedback_from_openai_sse(self, full_sse_data: str, request_id: str) -> None:
        """Record copium_retrieve feedback from OpenAI Chat Completions SSE.
        ...
        ``copium_retrieve`` invocation as a no-op store call for the
        ...
        *,
        ...
        async def generate():
            ...
            try:
                ...
                # CCR Feedback: Record copium_retrieve tool calls for TOIN learning.
    ...
    async def _stream_openai_via_backend(
        ...
        from copium.proxy.handlers.openai import _infer_openai_cache_write_tokens
        from copium.proxy.outcome import RequestOutcome
        ...
        async def generate():
            ...
            try:
                ...
                # CCR Feedback: record copium_retrieve tool calls so
