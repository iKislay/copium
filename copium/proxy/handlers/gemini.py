"""Gemini handler mixin for CopiumProxy.
...
class GeminiHandlerMixin:
    """Mixin providing Gemini API handler methods for CopiumProxy."""
    ...
    async def handle_gemini_generate_content(
        ...
        _pre_strip_count_gem = sum(1 for k in headers if k.lower().startswith("x-copium-"))
        ...
        # `x-copium-bypass: true`, mutating request bytes under the
        ...
        if len(preserved_indices) == len(contents):
            ...
            if is_streaming:
                ...
                response_headers["x-copium-tokens-before"] = str(total_input_tokens)
                response_headers["x-copium-tokens-after"] = str(total_input_tokens)
                response_headers["x-copium-tokens-saved"] = "0"
    ...
    async def handle_google_cloudcode_stream(
        ...
        # PR-A5 (P5-49): strip internal x-copium-* from upstream-bound headers
        # AFTER `_extract_tags` and `is_cloudcode_antigravity` reads.
        from copium.proxy.helpers import _strip_internal_headers, log_outbound_headers

        _pre_strip_count_cca = sum(1 for k in headers if k.lower().startswith("x-copium-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
    ...
    async def handle_gemini_stream_generate_content(
        ...
        _pre_strip_count_gem_stream = sum(1 for k in headers if k.lower().startswith("x-copium-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
    ...
    async def handle_gemini_count_tokens(
        ...
        _pre_strip_count_gem_count = sum(1 for k in headers if k.lower().startswith("x-copium-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
