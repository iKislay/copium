"""Batch handler mixin for CopiumProxy.
...
from copium.proxy.auth_mode import classify_client
...
class BatchHandlerMixin:
    """Mixin providing batch API handler methods for CopiumProxy."""
    ...
    async def handle_google_batch_create(
        ...
        _pre_strip_count_gb = sum(1 for k in headers if k.lower().startswith("x-copium-"))
    ...
    async def _google_batch_passthrough(
        ...
        _pre_strip_count_gpt = sum(1 for k in headers if k.lower().startswith("x-copium-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
        ...
        # has been synthesized by Copium and is canonically serialized.
        ...
        from copium.proxy.helpers import get_python_forwarder_mode
    ...
    async def handle_google_batch_passthrough(
        ...
        _pre_strip_count_gp = sum(1 for k in headers if k.lower().startswith("x-copium-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
    ...
    async def handle_google_batch_results(
        ...
        _pre_strip_count_gbr = sum(1 for k in headers if k.lower().startswith("x-copium-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
    ...
    async def handle_batch_create(self, request: Request) -> Response:
        ...
        _pre_strip_count_oacc = sum(1 for k in headers if k.lower().startswith("x-copium-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
        ...
        try:
            ...
            compression_metadata = {
                **metadata,
                "copium_compressed": "true",
                "copium_original_file_id": input_file_id,
                "copium_total_requests": str(stats["total_requests"]),
                "copium_tokens_saved": str(stats["total_tokens_saved"]),
                "copium_original_tokens": str(stats["total_original_tokens"]),
                "copium_compressed_tokens": str(stats["total_compressed_tokens"]),
                "copium_savings_percent": f"{stats['savings_percent']:.1f}",
            }
            ...
            # batch_body is synthesized by Copium (compressed file_id +
            ...
            response_headers.pop("content-encoding", None)
            response_headers.pop("content-length", None)
            response_headers["x-copium-tokens-saved"] = str(stats["total_tokens_saved"])
            response_headers["x-copium-savings-percent"] = f"{stats['savings_percent']:.1f}"
    ...
    async def _batch_passthrough(self, request: Request, body: dict) -> Response:
        ...
        headers.pop("content-length", None)
        # PR-A5 (P5-49): strip internal x-copium-* before forwarding upstream.
        _pre_strip_count_obp = sum(1 for k in headers if k.lower().startswith("x-copium-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
