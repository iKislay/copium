...
OpenAI chat, Gemini) never gated on ``x-copium-bypass``, so
...
Precedence (highest first):
  ...
  4. ``mode_disabled`` — operator COPIUM_MEMORY_INJECTION_MODE=disabled
  5. ``mode_tool`` — operator COPIUM_MEMORY_INJECTION_MODE=tool
...
class MemoryDecision:
    ...
    #   * "bypass_header"  — user set x-copium-bypass/x-copium-mode
    #   * "no_handler"     — no memory backend configured on the proxy
    #   * "no_user_id"     — per-request user_id missing
    #   * "mode_disabled"  — operator COPIUM_MEMORY_INJECTION_MODE=disabled
    #   * "mode_tool"      — operator COPIUM_MEMORY_INJECTION_MODE=tool
    ...
    def decide(
        ...
        *,
        ...
        mode_name
            ...
            ``COPIUM_MEMORY_INJECTION_MODE``.
        """
        bypass = _copium_bypass_enabled(headers)
