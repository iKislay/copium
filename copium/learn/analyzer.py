...
# COPIUM_LEARN_CLI_TIMEOUT_SECS for slow networks or large digests.
_CLI_TIMEOUT = 300
...
# analyses run to completion. Override with COPIUM_LEARN_CLI_IDLE_TIMEOUT_SECS.
...
def _detect_default_model() -> str:
    ...
    Priority order:
      ...
      2. COPIUM_LEARN_CLI env var → use specified CLI backend
    ...
    cli_override = os.environ.get("COPIUM_LEARN_CLI")
    if cli_override:
        for cli_name, model, _cmd in _CLI_BACKENDS:
            if cli_name == cli_override:
                logger.info("COPIUM_LEARN_CLI=%s — using %s CLI backend", cli_override, cli_name)
                return model
        valid = ", ".join(name for name, _, _ in _CLI_BACKENDS)
        raise ValueError(
            f"COPIUM_LEARN_CLI={cli_override!r} is not a supported CLI. Valid values: {valid}"
        )
    ...
    raise RuntimeError(
        "No LLM API key found. copium learn needs one of:\n"
        ...
        "Or set COPIUM_LEARN_CLI to a coding agent CLI (claude, gemini, codex).\n"
        "Or install one of those CLIs for auto-detection.\n"
        "Or specify a model directly: copium learn --model <litellm-model-name>"
    )
...
def _call_cli_llm(digest: str, model: str) -> dict:
    ...
    hard_cap = _resolve_timeout_secs("COPIUM_LEARN_CLI_TIMEOUT_SECS", _CLI_TIMEOUT)

    if model == "claude-cli":
        idle_cap = _resolve_timeout_secs("COPIUM_LEARN_CLI_IDLE_TIMEOUT_SECS", _CLI_IDLE_TIMEOUT)
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Check network connectivity, raise COPIUM_LEARN_CLI_TIMEOUT_SECS, "
    ...
    while eofs < 2:
        ...
        if elapsed > hard_cap:
            ...
            raise RuntimeError(
                ...
                "Raise COPIUM_LEARN_CLI_TIMEOUT_SECS for slower networks or "
        ...
        if idle_elapsed > idle_cap:
            ...
            raise RuntimeError(
                ...
                "COPIUM_LEARN_CLI_IDLE_TIMEOUT_SECS, or try a different "
