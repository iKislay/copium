"""One-function compression API for Copium.

The simplest way to use Copium — no proxy, no config, just compress:
...
class CompressConfig:
    ...
    Pass to ``compress()`` or any integration that uses copium.
...
def compress(
    ...
    **kwargs: Any,
    ...
    """Compress messages using Copium's full compression pipeline.

    This is the simplest way to use Copium. No proxy, no config needed.
...
def compress_spreadsheet(
    ...
    **kwargs: Any,
    ...
    (``pip install copium-ai[spreadsheet]``).
    ...
    """
    from copium.transforms.spreadsheet_ingest import load_spreadsheet
...
def _get_pipeline() -> Any:
    ...
    with _pipeline_lock:
        ...
        logger.debug("Copium compression pipeline initialized")
        return _pipeline
