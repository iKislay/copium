...
- ``get_compression_store()`` (the proxy path) defaults to SQLite
  (restart-safe, shared across workers); ``COPIUM_CCR_BACKEND=memory``
...
Usage:
    from copium.cache.backends import SQLiteBackend, CompressionStoreBackend
    from copium.cache.compression_store import CompressionStore, get_compression_store

    # Env-driven default (SQLite at ~/.copium/ccr_store.db)
