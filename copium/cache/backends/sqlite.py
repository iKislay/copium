...
- **Multi-worker sharing.** The database file (WAL mode) is shared
  across worker processes, so a `copium_retrieve` call served by a
...
Set ``COPIUM_CCR_BACKEND=memory`` to opt back into the in-memory
backend, or ``COPIUM_CCR_SQLITE_PATH`` to relocate the database file
(default ``~/.copium/ccr_store.db``).
"""
...
def default_db_path() -> Path:
    """Resolve the database path (env override or ~/.copium/)."""
    env = os.environ.get("COPIUM_CCR_SQLITE_PATH", "").strip()
    if env:
        return Path(env).expanduser()
    return Path.home() / ".copium" / "ccr_store.db"
