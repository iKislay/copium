...
cache_control breakpoints, growing-prefix strategy). If Copium compresses
...
class SessionTrackerStore:
    ...
    Keyed by session ID (from x-copium-session-id header or computed hash).
    ...
    def compute_session_id(
        ...
        Priority:
        1. x-copium-session-id header (explicit)
        2. Hash of (model + system prompt) — stable per conversation
        """
        # Check for explicit session header
        if hasattr(request, "headers"):
            session_header = request.headers.get("x-copium-session-id")
            if session_header:
