"""Central configuration for all ML models used in Copium.
...
Usage:
    ...
    # COPIUM_SENTENCE_TRANSFORMER=intfloat/e5-small-v2
    # COPIUM_SIGLIP=google/siglip-base-patch16-224
"""
...
class MLModelConfig:
    ...
    sentence_transformer: str = field(
        default_factory=lambda: os.environ.get("COPIUM_SENTENCE_TRANSFORMER", "all-MiniLM-L6-v2")
    )
    sentence_transformer_dim: int = 384

    # Image Embeddings (SIGLIP)
    siglip: str = field(
        default_factory=lambda: os.environ.get("COPIUM_SIGLIP", "google/siglip-base-patch16-224")
    )

    # Named Entity Recognition (spaCy)
    spacy: str = field(default_factory=lambda: os.environ.get("COPIUM_SPACY", "en_core_web_sm"))

    # Image Technique Router
    technique_router: str = field(
        default_factory=lambda: os.environ.get(
            "COPIUM_TECHNIQUE_ROUTER", "chopratejas/technique-router"
        )
    )
