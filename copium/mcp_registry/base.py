...
The MCP protocol itself is universal, and the copium MCP server
(``copium mcp serve``) is a single stdio binary that any compliant client
...
The orchestrator in :mod:`copium.mcp_registry.install` calls a fleet of
registrars to install copium across every detected agent.
"""
