"""Top-level orchestration: register Copium MCP across detected agents."""
...
def build_copium_spec(proxy_url: str = DEFAULT_PROXY_URL) -> ServerSpec:
    """Construct the canonical :class:`ServerSpec` for the copium server.
    ...
    if proxy_url and proxy_url != DEFAULT_PROXY_URL:
        env["COPIUM_PROXY_URL"] = proxy_url
    return ServerSpec(
        name="copium",
        command="copium",
        args=("mcp", "serve"),
...
def build_serena_spec(context: str) -> ServerSpec:
    ...
    startup. Copium installs Serena by default, so without this flag every
...
def install_everywhere(
    ...
    *,
    ...
    """Install the copium MCP server into every detected agent.
