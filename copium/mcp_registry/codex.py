...
_MARKER_START = "# --- Copium MCP server ---"
_MARKER_END = "# --- end Copium MCP server ---"


def _marker_start(server_name: str) -> str:
    if server_name == "copium":
        return _MARKER_START
    return f"# --- Copium MCP server: {server_name} ---"


def _marker_end(server_name: str) -> str:
    if server_name == "copium":
        return _MARKER_END
    return f"# --- end Copium MCP server: {server_name} ---"
    ...
    def register_server(self, spec: ServerSpec, *, force: bool = False) -> RegisterResult:
        ...
        if existing is not None and not force:
            ...
            if _marker_start(spec.name) not in content:
                ...
                return RegisterResult(
                    ...
                    f"{spec.name}] entry outside Copium markers; "
        ...
        if existing is not None and force:
            ...
            if _marker_start(spec.name) not in content:
                ...
                # Copium owns. Otherwise appending our table would create a
            ...
            # Drop any prior Copium block before re-writing.
...
def _render_block(spec: ServerSpec) -> str:
    """Render a Copium-marked TOML block for ``spec``."""
    lines: list[str] = [
