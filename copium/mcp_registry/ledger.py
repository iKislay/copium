"""Copium-owned MCP install ledger.

The ledger tracks MCP servers that Copium registered on the user's behalf
when the target agent config cannot carry Copium-specific ownership markers.
It lets unwrap remove only entries still matching the spec Copium installed,
...
def ledger_path() -> Path:
    """Return the Copium MCP install ledger path."""
...
def record_install(agent: str, spec: ServerSpec, *, path: Path | None = None) -> None:
    """Record that Copium installed ``spec`` for ``agent``."""
...
def copium_installed_matching(
    agent: str,
    current_spec: ServerSpec | None,
    *,
    path: Path | None = None,
) -> bool:
    """Return True when the ledger says Copium installed ``current_spec``."""
