...
from copium.copilot_linux_secret import read_copilot_oauth_token as read_linux_secret_token
from copium.copilot_macos_keychain import read_copilot_oauth_token as read_macos_keychain_token
...
def copium_copilot_auth_path() -> Path:
    """Return the path where Copium stores its Copilot OAuth token."""

    override = os.environ.get("COPIUM_COPILOT_AUTH_FILE", "").strip()
    if override:
...
def read_copium_copilot_oauth_token() -> str | None:
    """Return Copium's saved Copilot OAuth token, if one is available."""

    try:
        payload = json.loads(copium_copilot_auth_path().read_text(encoding="utf-8"))
    except FileNotFoundError:
        return None
    except Exception as exc:
        logger.debug("Unable to read Copium Copilot auth file: %s", exc)
        return None
...
def save_copium_copilot_oauth_token(
    token: str,
    *,
...
def iter_oauth_token_candidates() -> list[CopilotTokenCandidate]:
    ...
    copium_copilot_token = read_copium_copilot_oauth_token()
    if copium_copilot_token:
        candidates.append(
            CopilotTokenCandidate(
                token=copium_copilot_token,
                source=f"copium-copilot-auth:{copium_copilot_auth_path()}",
