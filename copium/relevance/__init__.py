"""Relevance scoring module for Copium SDK.
...
1. HybridScorer (DEFAULT - recommended)
   ...
   - Install for full support: pip install copium[relevance]
...
3. EmbeddingScorer (requires sentence-transformers)
   ...
   - Install: pip install copium[relevance]
...
Example usage:
    from copium.relevance import HybridScorer, create_scorer
    ...
    if tier == "bm25":
        ...
        if not EmbeddingScorer.is_available():
            raise RuntimeError(
                ...
                "Install with: pip install copium[relevance]"
            )
