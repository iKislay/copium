"""Configuration models for Copium SDK."""
...
class RelevanceScorerConfig:
    ...
    For full hybrid support, install: pip install copium[relevance]
...
class CacheOptimizerConfig:
    ...
    auto_detect_provider: bool = True  # Auto-detect from CopiumClient provider
...
class CCRConfig:
    ...
    4. RETRIEVE: If LLM needs more, it calls copium_retrieve(hash, query)
    ...
    # with Rust DEFAULT_TTL (crates/copium-core/src/ccr/mod.rs) and
    ...
    inject_tool: bool = True  # Inject copium_retrieve tool into tools array
...
class PrefixFreezeConfig:
    ...
    Copium from invalidating the provider's prefix cache (which would replace
...
class CopiumConfig:
    ...
    # Env var COPIUM_INTERCEPT_ENABLED=1 also enables (for CLI `--intercept-tool-results`).
...
class DiffArtifact:
    ...
    Opt-in via CopiumConfig.generate_diff_artifact = True.
