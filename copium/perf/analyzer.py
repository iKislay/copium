"""Analyze copium proxy logs for performance insights.

Parses PERF log lines from ~/.copium/logs/proxy.log* and produces
...
# Matches: 2026-03-07 13:38:31,009 - copium.proxy - INFO - [hr_...] PERF model=... ...
_PERF_RE = re.compile(
...
def parse_log_files(last_n_hours: float = 168.0) -> PerfReport:
    ...
    log_dir = _paths.log_dir() if os.environ.get("COPIUM_WORKSPACE_DIR") else LOG_DIR
    ...
    def _within_window(ts_str: str | None) -> bool:
        ...
        # alternative (silent drop) makes `copium perf` lie about coverage.
        if cutoff is None:
            return True
...
def format_report(report: PerfReport) -> str:
    ...
    if not report.perf_records and not report.router_records:
        lines.append("No performance data found in ~/.copium/logs/")
        lines.append("")
        lines.append("Start the proxy to begin collecting data:")
        lines.append("  copium proxy")
        return "\n".join(lines)

    # Header
    lines.append("Copium Performance Report")
    lines.append("=" * 60)
...
def _format_toin_highlights() -> list[str]:
    ...
    try:
        from copium.telemetry.toin import get_toin as _get
    ...
    lines.append(
        ...
        f"(eligible for `python -m copium.cli.toin_publish`)"
    )
    lines.append("")
