...
diff compression now goes through `copium._core.DiffCompressor` (built
from `crates/copium-py`). The byte-equality of the two implementations
was verified against 27 recorded fixtures before the Python source was
removed; the Rust crate has its own test coverage in `crates/copium-core/`.
...
class DiffCompressor:
    """Rust-backed `DiffCompressor` (via PyO3 / `copium._core`).
