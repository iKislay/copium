"""Transform pipeline orchestration for Copium SDK."""
...
from ..observability import get_copium_tracer, get_otel_metrics
...
class TransformPipeline:
    ...
    def __init__(
        ...
        self._breaker_threshold = _breaker_env("COPIUM_PIPELINE_BREAKER_THRESHOLD", 3, int)
        self._breaker_cooldown_s = _breaker_env("COPIUM_PIPELINE_BREAKER_COOLDOWN_S", 60.0, float)
    ...
    def _build_default_transforms(self) -> list[Transform]:
        ...
        # OPT-IN: enable via CopiumConfig.intercept_tool_results, or for
        # non-config callers (CLI / SDK / tests) the env var
        # COPIUM_INTERCEPT_ENABLED=1. Off by default while this ships — lets
        ...
        if getattr(self.config, "intercept_tool_results", False) or _os.environ.get(
            "COPIUM_INTERCEPT_ENABLED"
        ):
            from copium.proxy.interceptors import ToolResultInterceptorTransform
        ...
        **kwargs: Any,
        ...
        if model_limit is None:
            raise ValueError(
                ...
                "configure model_context_limits in CopiumClient."
            )
        ...
        tracer = get_copium_tracer()
        span_attributes = {
            "copium.model": model,
            "copium.provider": provider_name or "unknown",
            "copium.message_count": len(messages),
            "copium.tokens.before": tokens_before,
        }
        pipeline_span_context = (
            tracer.start_as_current_span(
                "copium.compression.pipeline",
        ...
        with pipeline_span_context as pipeline_span:
            ...
            for transform in self.transforms:
                ...
                transform_span_context = (
                    tracer.start_as_current_span(
                        "copium.compression.transform",
                        attributes={
                            "copium.model": model,
                            "copium.provider": provider_name or "unknown",
                            "copium.transform": transform.name,
                        },
                    )
                    if record_metrics
                    else nullcontext()
                ...
                with transform_span_context as transform_span:
                    ...
                    if transform_span is not None and transform_span.is_recording():
                        transform_span.set_attribute(
                            "copium.tokens.before", tokens_before_transform
                        )
                        transform_span.set_attribute(
                            "copium.tokens.after", tokens_after_transform
                        )
                        transform_span.set_attribute(
                            "copium.tokens.saved",
                            tokens_before_transform - tokens_after_transform,
                        )
                        transform_span.set_attribute("copium.duration_ms", duration_ms)
                        transform_span.set_attribute(
            ...
            if pipeline_span is not None and pipeline_span.is_recording():
                pipeline_span.set_attribute("copium.tokens.after", tokens_after)
                pipeline_span.set_attribute("copium.tokens.saved", total_saved)
                pipeline_span.set_attribute("copium.duration_ms", pipeline_ms)
                pipeline_span.set_attribute("copium.transforms_applied", len(all_transforms))
                pipeline_span.set_attribute("copium.warnings", len(all_warnings))
