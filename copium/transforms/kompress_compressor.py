...
Requires the [ml] extra: pip install copium-ai[ml]
...
KOMPRESS_BACKEND_ENV = "COPIUM_KOMPRESS_BACKEND"
KOMPRESS_ONNX_FILENAME_ENV = "COPIUM_KOMPRESS_ONNX_FILENAME"
...
#
# An operator can pin an exact file via COPIUM_KOMPRESS_ONNX_FILENAME.
_DEFAULT_ONNX_FILENAMES = (
    "onnx/kompress-int8-wo.onnx",
    "onnx/kompress-fp32.onnx",
    "onnx/kompress-int8.onnx",
)
KOMPRESS_ONNX_INTRA_THREADS_ENV = "COPIUM_KOMPRESS_ONNX_INTRA_THREADS"
KOMPRESS_ONNX_INTER_THREADS_ENV = "COPIUM_KOMPRESS_ONNX_INTER_THREADS"
KOMPRESS_COREML_CACHE_DIR_ENV = "COPIUM_KOMPRESS_COREML_CACHE_DIR"
KOMPRESS_MAX_CONCURRENT_ENV = "COPIUM_KOMPRESS_MAX_CONCURRENT"
KOMPRESS_BATCH_SIZE_ENV = "COPIUM_KOMPRESS_BATCH_SIZE"
...
def _get_model_class() -> type:
    """Return the CopiumCompressorModel class, importing torch on demand."""
    import torch
    import torch.nn as nn
    from transformers import AutoModel

    class CopiumCompressorModel(nn.Module):
...
def _load_kompress_pytorch(
    ...
    with _kompress_lock:
        ...
        CopiumCompressorModel = _get_model_class()
        model = CopiumCompressorModel()
...
def _load_kompress(
    ...
    # allowing opt-in MPS/CoreML experiments via COPIUM_KOMPRESS_BACKEND.
    if _is_onnx_available():
        try:
            return _load_kompress_onnx(model_id, use_coreml=False, allow_download=allow_download)
        except KompressModelNotCached:
    ...
    raise ImportError(
        "Kompress requires onnxruntime or torch. Install with: pip install copium-ai[proxy]"
    )
...
# compression timeout (COPIUM_COMPRESSION_TIMEOUT_SECONDS, default 30s). The
