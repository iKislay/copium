...
def _get_parser(language: str) -> Any:
    ...
    from any thread other than its creator.  Because Copium runs
    ...
    if not _check_tree_sitter_available():
        raise ImportError(
            "tree-sitter is not installed. Install with: pip install copium-ai[code]\n"
...
class CodeAwareCompressor(Transform):
    ...
    def compress(
        ...
        if not _check_tree_sitter_available():
            logger.warning("tree-sitter not available. Install with: pip install copium-ai[code]")
            if self.config.fallback_to_kompress:
                return self._fallback_compress(code, original_tokens)
            return CodeCompressionResult(
                compressed=code,
    ...
    def apply(
        ...
        **kwargs: Any,
        ...
        if not _check_tree_sitter_available():
            warnings.append(
                "tree-sitter not installed. Install with: pip install copium-ai[code]"
            )
