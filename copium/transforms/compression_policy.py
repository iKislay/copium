...
Hand-mirrored port of `copium_core::compression_policy::CompressionPolicy`
...
See `crates/copium-core/src/compression_policy.rs` for the canonical
...
# ``crates/copium-core/src/compression_policy.rs``. Centralised here so
...
def policy_default_payg() -> CompressionPolicy:
    ...
    ``COPIUM_PROXY_AUTH_MODE_POLICY_ENFORCEMENT`` flag is disabled
...
_ENFORCEMENT_ENV = "COPIUM_PROXY_AUTH_MODE_POLICY_ENFORCEMENT"
...
def is_enforcement_enabled() -> bool:
    ...
    # (`copium/telemetry/beacon.py::_OFF_VALUES`) so operators don't
