...
All array compression now goes through `copium._core.SmartCrusher`
(built from `crates/copium-py`). Byte-equality of the two
...
removed; the Rust crate has its own coverage in `crates/copium-core/`
...
The `copium._core` extension is a hard import: there is no Python
...
# `crates/copium-core/.../compaction/mod.rs`.
...
class SmartCrusherConfig:
    ...
    # crates/copium-core/src/transforms/smart_crusher/compaction/compactor.rs).
...
class SmartCrusher(Transform):
    """Rust-backed `SmartCrusher` (via PyO3 / `copium._core`).
    ...
    def __init__(
        ...
        # `observer`: see `copium.transforms.observability`. The
        ...
        # back (`copium.proxy.server` does). Both `enabled=False` and
        ...
        rust_cfg = _RustSmartCrusherConfig(
            ...
            # `copium.config.SmartCrusherConfig` (MCP server, SDK) or a
        ...
        # COPIUM_COMPACTION_FORMAT env var when the kwarg is None.
        # Ignored when with_compaction=False.
        resolved_format = compaction_format or os.environ.get(
            "COPIUM_COMPACTION_FORMAT", "csv-schema"
        )
    ...
    def _mirror_single_hash_to_python_store(
        ...
        # the Rust store's `DEFAULT_TTL` (see crates/copium-core/src/
    ...
    def _notify_observer(self, original_tokens: int, compressed_tokens: int) -> None:
        ...
        `CompressionObserver` (see `copium.transforms.observability`).
