...
def _resolve_detect_backend() -> str:
    ...
    backend = os.environ.get("COPIUM_DETECT_BACKEND", "").strip().lower()
...
def _detect_content(content: str) -> DetectionResult:
    ...
    Stage-3d (PR5) wired this through `copium._core.detect_content_type`,
    ...
    Set `COPIUM_DETECT_BACKEND=rust` or `python` to force a backend.
    ...
    if backend == "python":
        if not _detect_backend_warned:
            ...
            logger.warning(
                ...
                "override with COPIUM_DETECT_BACKEND=rust)."
            )
        return _regex_detect_content_type(content)

    from copium._core import detect_content_type as _rust_detect
...
def _net_cost_cache_ttl_seconds() -> float:
    ...
    ``COPIUM_NET_COST_CACHE_TTL_SECONDS`` for other providers/tiers. A
    ...
    the other ``COPIUM_NET_COST_*`` env guards).
    """
    raw = os.environ.get("COPIUM_NET_COST_CACHE_TTL_SECONDS", "")
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "COPIUM_NET_COST_CACHE_TTL_SECONDS malformed; using default %s",
            _NET_COST_CACHE_TTL_SECONDS,
        )
        return _NET_COST_CACHE_TTL_SECONDS
    if not math.isfinite(ttl) or ttl <= 0.0:
        logger.warning(
            "COPIUM_NET_COST_CACHE_TTL_SECONDS invalid; using default %s",
            _NET_COST_CACHE_TTL_SECONDS,
        )
        return _NET_COST_CACHE_TTL_SECONDS
    return ttl
...
class ContentRouter(Transform):
    ...
    ContentRouter is the recommended entry point for Copium's compression.
    ...
    def __init__(
        ...
        Args:
            ...
            observer: Optional `CompressionObserver` (see
                `copium.transforms.observability`) called once per
    ...
    def _get_diff_compressor(self) -> Any:
        ...
        retired in Stage 3b. The wheel (`copium._core`) is a hard import.
        """
    ...
    def _net_cost_allows(
        ...
        *,
        ...
        (``COPIUM_NET_COST_EXPECTED_READS``, default 10;
        ``COPIUM_NET_COST_P_ALIVE``, default 1.0 — the conservative
        ...
        by the caller, replaces the static ``COPIUM_NET_COST_P_ALIVE``
        ...
        try:
            _reads = float(os.environ.get("COPIUM_NET_COST_EXPECTED_READS", "") or 10.0)
            if not math.isfinite(_reads):
                raise ValueError("non-finite")
            reads = _reads
        except ValueError:
            logger.warning("COPIUM_NET_COST_EXPECTED_READS malformed; using 10")
        ...
        if p_alive_override is not None:
            ...
            try:
                _p_alive = float(os.environ.get("COPIUM_NET_COST_P_ALIVE", "") or 1.0)
                if not math.isfinite(_p_alive):
                    raise ValueError("non-finite")
                p_alive = _p_alive
            except ValueError:
                logger.warning("COPIUM_NET_COST_P_ALIVE malformed; using 1.0")
    ...
    def apply(
        ...
        **kwargs: Any,
        ...
        netcost_enabled = os.environ.get("COPIUM_NET_COST_POLICY") == "1"
        ...
        # times over. With COPIUM_NET_COST_POLICY=1 a *string-content*
        ...
        if pending_tasks:
            max_workers = min(
                len(pending_tasks), int(os.environ.get("COPIUM_COMPRESS_WORKERS", "4"))
            )
