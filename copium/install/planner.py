...
from copium.providers.install_registry import build_install_target_envs
...
def build_manifest(
    *,
    ...
    base_env = {
        "COPIUM_PORT": str(port),
        "COPIUM_HOST": "127.0.0.1",
        "COPIUM_MODE": proxy_mode,
        "COPIUM_BACKEND": backend,
    }
    if anyllm_provider:
        base_env["COPIUM_ANYLLM_PROVIDER"] = anyllm_provider
    if region:
        base_env["COPIUM_REGION"] = region
    if not telemetry_enabled:
        base_env["COPIUM_TELEMETRY"] = "off"
    if memory_enabled:
        base_env["COPIUM_MEMORY_ENABLED"] = "1"
    ...
    container_name = f"copium-{normalized_profile}"
    return DeploymentManifest(
        ...
        image=image,
        service_name=f"copium-{normalized_profile}",
