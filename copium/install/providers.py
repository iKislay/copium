...
from copium.providers.install_registry import (
...
_ENV_MARKER_START = "# >>> copium persistent env >>>"
_ENV_MARKER_END = "# <<< copium persistent env <<<"
    ...
    for name, value in merged.items():
        previous = subprocess.run(
            [
                ...
                "if ($null -eq $value) { '__COPIUM_UNSET__' } else { $value }",
            ],
            capture_output=True,
            text=True,
            check=True,
        ...
        mutations.append(
            ManagedMutation(
                ...
                data={
                    "name": name,
                    "scope": scope_name,
                    "previous": None if previous == "__COPIUM_UNSET__" else previous,
                },
            )
        )
    return mutations
