"""Memory Bridge: bidirectional bridge between markdown files and Copium memory.

Imports markdown memory files (Claude Code MEMORY.md, ChatGPT facts, etc.)
into Copium's semantic memory system, and exports Copium memories back
to organized markdown files. Supports bidirectional sync with hash-based
change detection.

Usage:
    from copium.memory.bridge import MemoryBridge
    from copium.memory.bridge_config import BridgeConfig
    ...
    # Import markdown -> Copium
    stats = await bridge.import_from_markdown()

    # Export Copium -> markdown
...
class MemoryBridge:
    """Bidirectional bridge between markdown memory files and Copium's
    semantic memory system.

    Supports import (md -> Copium), export (Copium -> md), and
    ...
    async def import_from_markdown(
        ...
        """Import markdown memory files into Copium's vector store.
    ...
    async def _import_section(
        ...
        """Import a single parsed section into Copium.
    ...
    async def export_to_markdown(
        ...
        """Export Copium memories to a markdown file.
    ...
    async def sync(
        ...
        """Bidirectional sync between markdown files and Copium.

        1. Import new/changed sections from md files.
        2. Export new organic Copium memories back to md files.
        """
        paths = paths or self._config.md_paths
        user_id = user_id or self._config.user_id
        stats = SyncStats()
