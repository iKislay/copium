"""Copium Memory MCP Server.

A stdio MCP server that exposes copium's memory backend as tools
...
Usage:
    ...
    python -m copium.memory.mcp_server --db /path/to/.copium/memory.db

    # Registered in Codex config.toml (done by `copium wrap codex --memory`):
    [mcp_servers.copium_memory]
    command = "python"
    args = ["-m", "copium.memory.mcp_server", "--db", ".copium/memory.db"]
"""
...
logger = logging.getLogger("copium.memory.mcp")
...
def create_memory_server(db_path: str, user_id: str = "default") -> Server:
    """Create an MCP server backed by copium's local memory."""

    server = Server("copium-memory")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Copium Memory MCP Server")
    parser.add_argument(
        "--db",
        default=str(Path.cwd() / ".copium" / "memory.db"),
