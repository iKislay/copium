"""Mem0 backend adapter for Copium's hierarchical memory system.
...
from copium.memory.ports import MemoryFilter, VectorFilter, VectorSearchResult
...
class Mem0Config:
    ...
    # Qdrant settings (defaults resolve from COPIUM_QDRANT_* env vars)
    ...
    # Collection settings
    collection_name: str = "copium_memories"


class Mem0Backend:
    """
    Mem0 backend implementation for Copium memory system.
    ...
    This adapter maps Mem0's API to Copium's MemoryBackend interface:
    ...
    async def _ensure_client(self) -> Any:
        ...
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "mem0 package not installed. Install with: pip install 'copium-ai[memory-stack]'"
                ) from None
    ...
    def _mem0_result_to_memory(self, result: dict[str, Any]) -> Memory:
        ...
        memory_id = metadata.get("copium_id", result.get("id", str(uuid.uuid4())))
