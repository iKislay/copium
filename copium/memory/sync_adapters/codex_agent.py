...
Syncs memories to/from a copium-managed section in AGENTS.md.
...
from copium.memory.sync import AgentMemory, AgentMemoryAdapter

_MARKER_START = "<!-- copium:memory:start -->"
_MARKER_END = "<!-- copium:memory:end -->"
...
class CodexAdapter(AgentMemoryAdapter):
    ...
    async def read_memories(self) -> list[AgentMemory]:
        """Read memories from the copium section of AGENTS.md."""
    ...
    async def write_memories(self, memories: list[dict[str, Any]]) -> int:
        """Write memories into the copium section of AGENTS.md."""
        if not memories:
            return 0

        # Build section content
        lines = ["## Copium Shared Memory", ""]
        for mem in memories:
