...
This module provides the easiest way to use Copium's memory system.
...
if TYPE_CHECKING:
    from copium.memory.ports import MemorySearchResult
...
class Memory:
    ...
    Examples:
        ...
        # Hosted Qdrant via URL + API key (or set COPIUM_QDRANT_URL /
        # COPIUM_QDRANT_API_KEY in the environment)
        memory = Memory(
    ...
    def __init__(
        ...
        if db_path is None:
            # Default: workspace memory.db (respects COPIUM_WORKSPACE_DIR)
        ...
        # ``None`` sentinels fall back to COPIUM_QDRANT_* env vars so that
