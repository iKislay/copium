...
* ``USER``: one DB per ``x-copium-user-id`` (no project axis).
...
class ProjectResolver:
    ...
    def resolve(self, ctx: RequestContext) -> tuple[str, str] | None:
        ...
        explicit = self._first_nonempty_header(ctx.headers, "x-copium-project-id")
        if explicit:
        ...
        explicit_cwd = self._first_nonempty_header(ctx.headers, "x-copium-cwd")
        if explicit_cwd:
        ...
        if ident is None:
            ...
            if fallback == "empty":
                ...
                logger.warning(
                    ...
                    "hint='set x-copium-project-id or x-copium-cwd header, "
