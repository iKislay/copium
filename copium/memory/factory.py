...
from copium.memory.config import (
    EmbedderBackend,
    MemoryConfig,
    StoreBackend,
    TextBackend,
...
if TYPE_CHECKING:
    from copium.memory.ports import Embedder, MemoryCache, MemoryStore, TextIndex, VectorIndex


# Extension groups for memory backends registered via setuptools entry points.
_MEMORY_STORE_GROUP = "copium.memory_store"
_MEMORY_VECTOR_GROUP = "copium.memory_vector"
_MEMORY_TEXT_GROUP = "copium.memory_text"
...
def _load_external_backend(
    ...
    `copium.cache.compression_store._create_default_ccr_backend`.
    """
    if not name:
        raise ValueError(
...
def _create_store(config: MemoryConfig) -> MemoryStore:
    ...
    if config.store_backend == StoreBackend.SQLITE:
        from copium.memory.adapters.sqlite import SQLiteMemoryStore
...
def _create_embedder(config: MemoryConfig) -> Embedder:
    ...
    with _EMBEDDER_CACHE_LOCK:
        ...
        if config.embedder_backend == EmbedderBackend.LOCAL:
            ...
            from copium.memory.adapters.embedders import OllamaEmbedder
...
def _create_vector_index(config: MemoryConfig) -> VectorIndex:
    ...
    if backend == VectorBackend.AUTO:
        from copium.memory.adapters import HNSW_AVAILABLE, SQLITE_VEC_AVAILABLE
        ...
        if SQLITE_VEC_AVAILABLE:
            ...
            raise ValueError(
                ...
                "Or install the full proxy bundle: pip install copium-ai[proxy]"
            )

    if backend == VectorBackend.SQLITE_VEC:
        from copium.memory.adapters import SQLITE_VEC_AVAILABLE
        ...
        from copium.memory.adapters.sqlite_vector import SQLiteVectorIndex
    ...
    if backend == VectorBackend.HNSW:
        from copium.memory.adapters import HNSW_AVAILABLE
        ...
        from copium.memory.adapters.hnsw import HNSWVectorIndex
...
def _create_text_index(config: MemoryConfig) -> TextIndex:
    ...
    if config.text_backend == TextBackend.FTS5:
        from copium.memory.adapters.fts5 import FTS5TextIndex
...
def _create_cache(config: MemoryConfig) -> MemoryCache:
    ...
    """
    from copium.memory.adapters.cache import LRUMemoryCache
