...
Provides bidirectional sync between copium's memory DB and any
...
Usage:
    from copium.memory.sync import sync, SyncResult
    from copium.memory.sync_adapters.claude_code import ClaudeCodeAdapter
...
logger = logging.getLogger("copium.memory.sync")

# State file for fast no-op detection (workspace bucket, respects
# COPIUM_WORKSPACE_DIR). Resolved at import time, matching prior behavior.
    ...
    async def write_memories(self, memories: list[dict[str, Any]]) -> int:
        ...
        Args:
            memories: List of dicts with keys: content, category, importance,
                      copium_id, source_agent, content_hash.
...
async def sync(
    ...
    """Bidirectional sync between copium DB and an agent's memory.
...
# ---------------------------------------------------------------------------
# CLI entry point: python -m copium.memory.sync --db ... --user ... --agent ...
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Copium memory sync")
