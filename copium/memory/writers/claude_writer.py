...
from copium.memory.sync_adapters.claude_code import encode_claude_project_path
from copium.memory.writers.base import AgentWriter, MemoryEntry
...
class ClaudeCodeMemoryWriter(AgentWriter):
    ...
    def format_memories(self, memories: list[MemoryEntry]) -> str:
        ...
        lines = [
            "## Copium Learned Context",
            "*Auto-maintained by Copium — do not edit manually*",
            "",
        ]
    ...
    def export_topics(
        ...
        for category, entries in grouped.items():
            ...
            filename = f"copium_{category}.md"
            lines = [
                "---",
                f"name: copium-{category}",
                f"description: Copium-learned {category.replace('_', ' ')} patterns",
                "type: reference",
                "---",
                "",
