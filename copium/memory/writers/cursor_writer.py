...
class CursorMemoryWriter(AgentWriter):
    ...
    def default_path(self) -> Path:
        """Default: .cursor/rules/copium-memory.mdc in project root."""
        return self._project_path / ".cursor" / "rules" / "copium-memory.mdc"
    ...
    def export(
        ...
        if target.exists():
            ...
            full_content = (
                "---\n"
                "description: Copium-learned patterns from proxy traffic\n"
                "alwaysApply: true\n"
                "---\n"
                "\n"
