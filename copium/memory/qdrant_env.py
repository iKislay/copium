...
Provides a single source of truth for ``COPIUM_QDRANT_*`` env vars so that
...
- ``COPIUM_QDRANT_URL``        Full URL (e.g. ``https://xyz.cloud.qdrant.io:6333``).
                                  When set, takes precedence over host/port.
- ``COPIUM_QDRANT_HOST``       Hostname. Default: ``localhost``.
- ``COPIUM_QDRANT_PORT``       HTTP port. Default: ``6333``.
- ``COPIUM_QDRANT_API_KEY``    API key for hosted Qdrant (e.g. Qdrant Cloud).
- ``COPIUM_QDRANT_HTTPS``      ``true``/``false``. Forces HTTPS on/off.
- ``COPIUM_QDRANT_PREFER_GRPC````true``/``false``. Use gRPC instead of HTTP.
- ``COPIUM_QDRANT_GRPC_PORT``  gRPC port. Default: ``6334``.
...
def qdrant_env_url() -> str | None:
    """Return ``COPIUM_QDRANT_URL`` or ``None`` if unset."""
    return _strip_env("COPIUM_QDRANT_URL")


def qdrant_env_host() -> str:
    """Return ``COPIUM_QDRANT_HOST`` or the ``localhost`` default."""
    return _strip_env("COPIUM_QDRANT_HOST") or DEFAULT_QDRANT_HOST


def qdrant_env_port() -> int:
    """Return ``COPIUM_QDRANT_PORT`` or the ``6333`` default."""
    return (
        _parse_port(_strip_env("COPIUM_QDRANT_PORT"), "COPIUM_QDRANT_PORT")
        or DEFAULT_QDRANT_PORT
    )


def qdrant_env_api_key() -> str | None:
    """Return ``COPIUM_QDRANT_API_KEY`` or ``None`` if unset."""
    return _strip_env("COPIUM_QDRANT_API_KEY")


def qdrant_env_https() -> bool | None:
    """Return ``COPIUM_QDRANT_HTTPS`` parsed as bool, or ``None`` if unset."""
    return _parse_bool(_strip_env("COPIUM_QDRANT_HTTPS"))


def qdrant_env_prefer_grpc() -> bool:
    """Return ``COPIUM_QDRANT_PREFER_GRPC`` parsed as bool. Default: ``False``."""
    return _parse_bool(_strip_env("COPIUM_QDRANT_PREFER_GRPC")) or False


def qdrant_env_grpc_port() -> int:
    """Return ``COPIUM_QDRANT_GRPC_PORT`` or the ``6334`` default."""
    return (
        _parse_port(_strip_env("COPIUM_QDRANT_GRPC_PORT"), "COPIUM_QDRANT_GRPC_PORT")
