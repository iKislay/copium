"""Grammar-constrained compression.

Ensures compressed output is always valid by using grammar rules to
...
Key features:
- JSON grammar enforcement: compressed JSON is always valid
- Markdown grammar enforcement: compressed markdown preserves structure
- Code grammar enforcement: compressed code preserves syntax
- Schema-aware: function calling schemas are compressed without breaking

The grammar acts as a safety net — if a lossy compression step would
produce invalid output, the grammar catches it and falls back to a
lossless approach.
"""
...
MARKDOWN_GRAMMAR = GrammarRule(
    grammar_type=GrammarType.MARKDOWN,
...
CODE_GRAMMAR = GrammarRule(
    grammar_type=GrammarType.CODE,
...
GRAMMAR_RULES: dict[GrammarType, GrammarRule] = {
    GrammarType.JSON: JSON_GRAMMAR,
    GrammarType.MARKDOWN: MARKDOWN_GRAMMAR,
    GrammarType.CODE: CODE_GRAMMAR,
}


def detect_grammar(content: str) -> GrammarType:
    """Auto-detect the grammar type of content."""
    stripped = content.strip()

    # JSON detection (by structure, not just validity)
    if stripped.startswith(("{", "[")) and stripped.endswith(("}", "]")):
        return GrammarType.JSON

    # Markdown detection
    if re.search(r"^#{1,6}\s", content, re.M) or "```" in content:
        return GrammarType.MARKDOWN
    ...
    if any(sig in content for sig in code_signals):
        return GrammarType.CODE

    # XML detection
    if stripped.startswith("<") and stripped.endswith(">"):
        return GrammarType.XML

    return GrammarType.FREEFORM


def validate_and_repair(content: str, grammar_type: GrammarType | None = None) -> tuple[bool, str, GrammarType]:
    ...
    if grammar_type is None:
        grammar_type = detect_grammar(content)

    if grammar_type == GrammarType.FREEFORM:
        return True, content, grammar_type

    rule = GRAMMAR_RULES.get(grammar_type)
    if rule is None:
        return True, content, grammar_type
...
class GrammarCompressor:
    ...
    def __init__(self, config: GrammarCompressorConfig | None = None):
        self.config = config or GrammarCompressorConfig()

    def compress(
        self,
        content: str,
        compressor: callable | None = None,
        grammar_type: GrammarType | None = None,
    ) -> tuple[str, GrammarType, bool]:
        ...
        if not self.config.enabled:
            return content, GrammarType.FREEFORM, True

        # Detect grammar
        detected = detect_grammar(content)
    ...
    def _is_enforced(self, grammar_type: GrammarType) -> bool:
        """Check if a grammar type is enforced."""
        if grammar_type == GrammarType.JSON:
            return self.config.enforce_json
        if grammar_type == GrammarType.MARKDOWN:
            return self.config.enforce_markdown
        if grammar_type == GrammarType.CODE:
            return self.config.enforce_code
        if grammar_type == GrammarType.XML:
