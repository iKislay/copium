...
happened so an offline aggregator (`copium.cli.toin_publish`) can emit
...
# Usage
    ...
    #   python -m copium.cli.toin_publish --output recommendations.toml
...
TOIN_PATH_ENV_VAR = "COPIUM_TOIN_PATH"

# Default TOIN storage directory and file
DEFAULT_TOIN_DIR = ".copium"
...
def get_default_toin_storage_path() -> str:
    ...
    Checks for the COPIUM_TOIN_PATH environment variable first.
    Falls back to ``${COPIUM_WORKSPACE_DIR}/toin.json`` (which defaults
    to ``~/.copium/toin.json``) when unset.

    Returns:
        The path string for TOIN storage.
    """
    # Preserve legacy behavior: when COPIUM_TOIN_PATH is set we return the
...
class TOINConfig:
    ...
    # Storage
    # Default path is ~/.copium/toin.json (or COPIUM_TOIN_PATH env var)
...
class ToolIntelligenceNetwork:
    """Aggregates tool patterns across all Copium users (observation-only).
    ...
    `copium.cli.toin_publish` and consumed by the Rust proxy at startup.
    ...
    def get_recommendation(
        ...
        offline `copium.cli.toin_publish` CLI into `recommendations.toml`
        ...
        if not cls._DEPRECATION_WARNED:
            ...
            warnings.warn(
                ...
                "startup. See copium/telemetry/toin.py module docstring.",
                DeprecationWarning,
                stacklevel=2,
            )
        return None
    ...
    def iter_patterns(self) -> list[tuple[PatternKey, ToolPattern]]:
        ...
        Used by `copium.cli.toin_publish` to walk every aggregated
    ...
    def import_patterns(self, data: dict[str, Any]) -> None:
        ...
        Copium instances without sharing actual data.
...
TOIN_BACKEND_ENV_VAR = "COPIUM_TOIN_BACKEND"


def _create_default_toin_backend() -> Any:
    """Create a TOIN backend from env (e.g. COPIUM_TOIN_BACKEND=redis).

    Loads adapters via setuptools entry point 'copium.toin_backend'.
    ...
    try:
        ...
        all_eps = entry_points(group="copium.toin_backend")
        ep = next((e for e in all_eps if e.name == backend_type), None)
        if ep is None:
            logger.warning(
                "COPIUM_TOIN_BACKEND=%s but no entry point copium.toin_backend[%s]",
                backend_type,
                backend_type,
            )
            return None
        ...
        kwargs = {
            "url": os.environ.get("COPIUM_TOIN_URL", ""),
            "tenant_prefix": os.environ.get("COPIUM_TOIN_TENANT_PREFIX", ""),
        }
        return fn(**kwargs)
    except Exception as e:
...
def get_toin(config: TOINConfig | None = None) -> ToolIntelligenceNetwork:
    ...
    On first call, checks COPIUM_TOIN_BACKEND env var. If set, loads the
    backend via setuptools entry point 'copium.toin_backend'. Otherwise
