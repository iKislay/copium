...
* ``copium_stack`` — how Copium is being invoked
...
# from unbounded label explosion when clients send arbitrary X-Copium-Stack
# header values.
...
def detect_install_mode(port: int) -> str:
    ...
    1. ``COPIUM_AGENT_TYPE`` env var set → ``wrapped`` (spawned by ``copium wrap``).
    ...
    try:
        if os.environ.get("COPIUM_AGENT_TYPE"):
            return "wrapped"
...
def detect_stack(stats: dict[str, Any] | None = None) -> str:
    """Classify how Copium is being invoked.

    Resolution order:

    1. ``COPIUM_STACK`` env var set → use that slug verbatim.
    2. ``COPIUM_AGENT_TYPE`` env var set → ``wrap_<agent>``.
    ...
    try:
        explicit = normalize_stack(os.environ.get("COPIUM_STACK"))
        if explicit:
            return explicit

        agent_type = os.environ.get("COPIUM_AGENT_TYPE")
        if agent_type:
