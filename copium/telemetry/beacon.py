"""Anonymous usage telemetry beacon for Copium.
...
from copium.telemetry.context import detect_install_mode, detect_stack
...
def is_telemetry_warn_enabled() -> bool:
    ...
    Set COPIUM_TELEMETRY_WARN=off to suppress startup/wrap notices.
    ...
    """
    val = os.environ.get("COPIUM_TELEMETRY_WARN", "on").lower().strip()
    return val not in _OFF_VALUES
...
class TelemetryBeacon:
    ...
    async def start(self) -> None:
        ...
        if not is_telemetry_enabled():
            logger.debug("Telemetry disabled (COPIUM_TELEMETRY=off)")
            return
        self._task = asyncio.create_task(self._loop())
        logger.info(
            "Telemetry: ENABLED (anonymous aggregate stats, opt out: COPIUM_TELEMETRY=off)"
        )
    ...
    async def _report(self) -> None:
        ...
        try:
            from copium._version import __version__ as copium_version
        except Exception:
            copium_version = "unknown"

        # Core identity (always present)
        payload: dict = {
            "session_id": self._session_id,
            "instance_id": self._instance_id,
            "copium_version": copium_version,
            "python_version": (
            ...
            "copium_stack": detect_stack(stats),
        }
        ...
        # --- Performance overhead (how much latency Copium adds) ---
        try:
        ...
        try:
            ...
            # Copium continues working perfectly without telemetry.
