...
def get_telemetry_collector(
    ...
    if _telemetry_collector is None:
        with _collector_lock:
            if _telemetry_collector is None:
                # Honour COPIUM_TELEMETRY (the documented opt-out var,
                # also used by the Supabase beacon at telemetry/beacon.py).
                # Pre-#390 this only checked COPIUM_TELEMETRY_DISABLED,
                # so users who set COPIUM_TELEMETRY=off (the value in
                # the docs) still saw /v1/telemetry report enabled=true.
                # COPIUM_TELEMETRY_DISABLED stays accepted for back-compat.
                from copium.telemetry.beacon import is_telemetry_enabled

                disabled_legacy = os.environ.get("COPIUM_TELEMETRY_DISABLED", "").lower() in (
                    "1",
                    "true",
                )
