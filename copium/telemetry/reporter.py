...
statistics to the Copium cloud for billing. Designed to be non-intrusive:
...
logger = logging.getLogger("copium.telemetry.reporter")

# Grace period: if the cloud API is unreachable, use cached license for up to 7 days
GRACE_PERIOD_SECONDS = 7 * 24 * 3600  # 7 days

# Default cache location (workspace bucket, respects COPIUM_WORKSPACE_DIR).
...
class UsageReporter:
    ...
    usage stats to the Copium cloud. If the cloud is unreachable, the proxy
    ...
    def __init__(
        self,
        license_key: str,
        cloud_url: str = "https://app.copiumlabs.ai",
        ...
        self._proxy: CopiumProxy | None = None
    ...
    async def start(self, proxy: CopiumProxy) -> None:
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        if self._http_client is None:
            self._http_client = httpx.AsyncClient(
                ...
                headers={"User-Agent": "copium-proxy"},
            )
