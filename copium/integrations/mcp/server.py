"""Copium MCP integration helpers for compressing tool outputs.

This module currently provides these MCP integration surfaces:

1. CopiumMCPCompressor - core compression logic for MCP tool results
2. compress_tool_result() - standalone helper for host applications
3. CopiumMCPClientWrapper - client wrapper that compresses tool outputs
4. create_copium_mcp_proxy() - configuration helper for custom proxy setups

The key insight: MCP tool outputs are the PERFECT use case for Copium.
...
Example - Custom proxy configuration:
    ...
    proxy_config = create_copium_mcp_proxy(
        upstream_servers=[
            ("slack", slack_server),
            ("database", db_server),
...
Example - Middleware (for MCP client libraries):
    ...
    middleware = CopiumMCPMiddleware(config)
...
from copium.config import CopiumConfig, SmartCrusherConfig
...
class CopiumMCPCompressor:
    ...
    def compress(
        ...
        # Remove any Copium markers for clean output
        compressed_content = re.sub(r"\n<copium:[^>]+>", "", compressed_content)
...
def compress_tool_result(
    ...
    This is the simplest way to use Copium with MCP in your host application.
    ...
    Args:
        ...
        config: Optional Copium configuration.
    ...
    Example:
        ...
        ```
    """
    compressor = CopiumMCPCompressor(config=config)
...
class CopiumMCPClientWrapper:
    ...
    Example:
        ...
        from copium.integrations.mcp import CopiumMCPClientWrapper

        # Wrap your MCP client
        base_client = Client(transport)
        client = CopiumMCPClientWrapper(base_client)
    ...
    def __init__(
        ...
        """
        self._client = client
        self._compressor = CopiumMCPCompressor(config=config)
...
def create_copium_mcp_proxy(
    upstream_servers: list[tuple[str, MCPServer]],
    config: CopiumConfig | None = None,
) -> dict[str, Any]:
    """Create configuration for a custom Copium MCP proxy/server wrapper.

    This returns the compressor and upstream-server mapping needed by an
    application-defined MCP proxy/server wrapper. Copium does not yet ship
    a ready-to-run ``CopiumMCPProxy`` server implementation.
