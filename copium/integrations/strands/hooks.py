"""Strands SDK hook provider for Copium tool output compression.

This module provides CopiumHookProvider, which implements Strands' HookProvider
interface to intercept tool outputs and compress them using Copium's SmartCrusher.
...
from copium.transforms.smart_crusher import SmartCrusher, SmartCrusherConfig
...
@dataclass
class CopiumHookProvider(HookProvider):  # type: ignore[misc]
    """Strands HookProvider that compresses tool outputs using Copium.

    This hook provider intercepts tool call results via AfterToolCallEvent
    and applies Copium's SmartCrusher to compress large outputs, reducing
    ...
    Example:
        ...
        hook = CopiumHookProvider(min_tokens_to_compress=50)
    ...
    config: CopiumConfig | None = field(default=None)
    ...
    def __post_init__(self) -> None:
        ...
        if self.config is None:
            self.config = CopiumConfig()

        self._initialized = True
        logger.debug(
            "CopiumHookProvider initialized: compress=%s, min_tokens=%d, preserve_errors=%s",
    ...
    def crusher(self) -> SmartCrusher:
        ...
        if self._crusher is None:
            with self._lock:
                ...
                if self._crusher is None:
                    # Use config from CopiumConfig if available
    ...
    def register_hooks(self, registry: HookRegistry, **kwargs: Any) -> None:
        ...
        logger.info(
            "CopiumHookProvider registered: compressing tool outputs >= %d tokens",
            self.min_tokens_to_compress,
        )
    ...
    def reset(self) -> None:
        ...
        logger.debug("CopiumHookProvider metrics reset")
