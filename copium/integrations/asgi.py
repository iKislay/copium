"""ASGI Middleware — add Copium compression to any Python proxy.
...
class CompressionMiddleware:
    ...
    - x-copium-tokens-before: original token count
    - x-copium-tokens-after: compressed token count
    - x-copium-tokens-saved: tokens removed
    - x-copium-compressed: "true" if compression occurred
    """
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            if messages:
                ...
                if result and result.get("tokens_saved", 0) > 0 and "messages" in result:
                    ...
                    logger.info(
                        "Copium%s: %d→%d tokens (saved %d, %.0f%%)",
            ...
            logger.debug("Copium middleware: skipping non-JSON request: %s", e)
        ...
        async def metrics_send(message: MutableMapping[str, Any]) -> None:
            if message["type"] == "http.response.start" and tokens_saved > 0:
                ...
                headers.append((b"x-copium-compressed", b"true"))
                headers.append((b"x-copium-tokens-before", str(tokens_before).encode()))
                headers.append((b"x-copium-tokens-after", str(tokens_after).encode()))
                headers.append((b"x-copium-tokens-saved", str(tokens_saved).encode()))
