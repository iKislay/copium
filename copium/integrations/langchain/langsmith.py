"""LangSmith integration for Copium compression metrics.

This module provides CopiumLangSmithCallbackHandler, a LangChain callback
handler that adds Copium compression metrics to LangSmith traces.

When used with CopiumChatModel, it automatically captures:
...
Example:
    ...
    # Create handler
    handler = CopiumLangSmithCallbackHandler()

    # Use with CopiumChatModel
    llm = CopiumChatModel(
        ChatOpenAI(model="gpt-4o"),
        callbacks=[handler],
    )

    # Traces will include copium.* metadata
...
class CopiumLangSmithCallbackHandler(BaseCallbackHandler):
    """Callback handler that adds Copium metrics to LangSmith traces.
    ...
    2. Used as a callback with CopiumChatModel
    ...
    Example:
        ...
        # - copium.tokens_after
        # - copium.tokens_saved
        # - copium.savings_percent
        # - copium.transforms_applied
    ...
    def __init__(
        ...
        """Initialize CopiumLangSmithCallbackHandler.
        ...
        Args:
            ...
            auto_update_runs: If True, automatically updates LangSmith
                runs with Copium metadata. Default True.
        """
    ...
    def set_copium_metrics(
        ...
        """Set Copium metrics for a run.

        Call this from CopiumChatModel after optimization to attach
        ...
        logger.debug(
            f"Copium metrics set for run {run_id_str}: "
    ...
    def on_llm_end(
        ...
        **kwargs: Any,
        ...
        Attaches pending Copium metrics to the LangSmith run.
        """
        run_id_str = str(run_id)
    ...
    def _attach_metrics_to_run(self, run_id: str, metrics: PendingMetrics) -> None:
        """Attach Copium metrics to a LangSmith run.
        ...
        """
        metadata = {
            "copium.tokens_before": metrics.tokens_before,
            "copium.tokens_after": metrics.tokens_after,
            "copium.tokens_saved": metrics.tokens_saved,
            "copium.savings_percent": round(metrics.savings_percent, 2),
            "copium.transforms_applied": metrics.transforms_applied,
            "copium.optimization_timestamp": metrics.timestamp.isoformat(),
        }
        ...
        if self._client and self._auto_update:
            try:
                ...
                logger.debug(f"Updated LangSmith run {run_id} with Copium metrics")
            except Exception as e:
                logger.debug(f"Could not update LangSmith run: {e}")

    def get_run_metrics(self, run_id: str | UUID) -> dict[str, Any]:
        """Get Copium metrics for a specific run.

        Args:
            run_id: The run ID.

        Returns:
            Dictionary of copium.* metrics for the run.
        """
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        total_saved = sum(m.get("copium.tokens_saved", 0) for m in self._run_metrics.values())
        savings_percents = [
            m.get("copium.savings_percent", 0) for m in self._run_metrics.values()
        ]
