...
This module provides CopiumToolWrapper and wrap_tools_with_copium
...
Example:
    ...
    # Wrap with Copium compression
    wrapped_tools = wrap_tools_with_copium(tools)
...
class CopiumToolWrapper:
    ...
    Example:
        from langchain.tools import Tool
        from copium.integrations import CopiumToolWrapper
        ...
        wrapped = CopiumToolWrapper(search_tool)
    ...
    def _record_metrics(self, original: str, compressed: str, was_compressed: bool) -> None:
        ...
        if was_compressed and chars_saved > 0:
            logger.info(
                f"CopiumToolWrapper[{self.name}]: {chars_before} -> {chars_after} chars "
...
def wrap_tools_with_copium(
    tools: list[BaseTool],
    min_chars_to_compress: int = 1000,
    metrics_collector: ToolMetricsCollector | None = None,
) -> list[StructuredTool]:
    """Wrap multiple LangChain tools with Copium compression.
