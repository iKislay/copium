"""LangChain integration for Copium SDK.
...
Components:
1. CopiumChatModel - Wraps any BaseChatModel to apply Copium transforms
2. CopiumCallbackHandler - Tracks metrics and token usage (observability only)
3. CopiumRunnable - LCEL-compatible Runnable for chain composition
...
Example:
    ...
    optimized_llm = CopiumChatModel(llm)

    # Use normally - Copium automatically optimizes context
...
from .providers import get_copium_provider, get_model_name_from_langchain
...
def _check_langchain_available() -> None:
    ...
    if not LANGCHAIN_AVAILABLE:
        raise ImportError(
            ...
            "Install with: pip install copium[langchain] "
...
class CopiumChatModel(BaseChatModel):
    """LangChain chat model wrapper that applies Copium optimizations.

    Wraps any LangChain BaseChatModel and automatically optimizes the context
    before each API call. This is the recommended way to use Copium with
    LangChain because:
    ...
    Example:
        ...
        # Basic usage
        llm = ChatOpenAI(model="gpt-4o")
        optimized = CopiumChatModel(llm)
        response = optimized.invoke([HumanMessage("Hello!")])

        # With custom config
        from copium import CopiumConfig, CopiumMode
        config = CopiumConfig(default_mode=CopiumMode.OPTIMIZE)
        optimized = CopiumChatModel(llm, config=config)
    ...
    Attributes:
        ...
        copium_client: CopiumClient instance for optimization
    ...
    copium_config: Any = Field(default=None, description="Copium configuration")
    mode: CopiumMode = Field(default=CopiumMode.OPTIMIZE, description="Copium mode")
    ...
    def __init__(
        ...
        mode: CopiumMode = CopiumMode.OPTIMIZE,
        auto_detect_provider: bool = True,
        **kwargs: Any,
    ) -> None:
        """Initialize CopiumChatModel.

        Args:
            wrapped_model: Any LangChain BaseChatModel to wrap
            config: CopiumConfig for optimization settings
            mode: CopiumMode (AUDIT, OPTIMIZE, or SIMULATE)
            auto_detect_provider: Auto-detect provider from wrapped model.
                ...
                Copium provider for accurate token counting.
        ...
        super().__init__(  # type: ignore[call-arg]
            ...
            copium_config=config or CopiumConfig(),
            mode=mode,
    ...
    def _llm_type(self) -> str:
        ...
        return f"copium-{self.wrapped_model._llm_type}"
    ...
    def _convert_messages_to_openai(self, messages: list[BaseMessage]) -> list[dict[str, Any]]:
        """Convert LangChain messages to OpenAI format for Copium."""
        result = []
        for msg in messages:
    ...
    def _optimize_messages(
        ...
        """Apply Copium optimization to messages."""
        ...
        # Apply Copium transforms via pipeline
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        """Generate response with Copium optimization.
        ...
        logger.info(
            f"Copium optimized: {metrics.tokens_before} -> {metrics.tokens_after} tokens "
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        """Stream response with Copium optimization."""
        # Optimize messages
        optimized_messages, metrics = self._optimize_messages(messages)

        logger.info(
            f"Copium optimized (streaming): {metrics.tokens_before} -> "
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        """Async generate response with Copium optimization.
        ...
        logger.info(
            f"Copium optimized (async): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        """Async stream response with Copium optimization.
        ...
        logger.info(
            f"Copium optimized (async streaming): {metrics.tokens_before} -> "
            f"{metrics.tokens_after} tokens"
        )
    ...
    def bind_tools(self, tools: Sequence[Any], **kwargs: Any) -> CopiumChatModel:
        """Bind tools to the wrapped model."""
        new_wrapped = self.wrapped_model.bind_tools(tools, **kwargs)
        return CopiumChatModel(
...
class CopiumCallbackHandler(BaseCallbackHandler):
    """LangChain callback handler for Copium metrics and observability.

    NOTE: Callbacks CANNOT modify messages in LangChain (by design).
    Use CopiumChatModel for actual optimization. This handler is for:
...
class CopiumRunnable:
    """LCEL-compatible Runnable for Copium optimization.

    Use this to add Copium optimization to any LangChain chain using LCEL.

    Example:
        from langchain_openai import ChatOpenAI
        from langchain_core.prompts import ChatPromptTemplate
        from copium.integrations import CopiumRunnable
        ...
        # Add Copium optimization to chain
        chain = prompt | CopiumRunnable() | llm
