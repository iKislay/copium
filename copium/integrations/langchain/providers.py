...
class paths to identify the appropriate Copium provider.

Example:
    from langchain_anthropic import ChatAnthropic
    from copium.integrations.langchain import get_copium_provider

    model = ChatAnthropic(model="claude-3-5-sonnet-20241022")
    provider = get_copium_provider(model)  # Returns AnthropicProvider
"""
...
def get_copium_provider(model: Any) -> Provider:
    """Get appropriate Copium Provider instance for a LangChain model.

    This function automatically detects the provider from the model type
    and returns a configured Copium provider for accurate token counting
    ...
    Returns:
        Configured Copium Provider instance
