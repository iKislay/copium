"""Agno hooks for Copium integration.

This module provides pre_hooks and post_hooks that can be used with
Agno agents to apply Copium optimization at the agent level.
"""
...
@dataclass
class HookMetrics:
    """Metrics collected by Copium pre-hooks.

    Note: These metrics track request counts and timing, not token savings.
    For actual token optimization metrics, use CopiumAgnoModel which
    ...
    # Use CopiumAgnoModel for actual token optimization
    tokens_before: int = 0
    tokens_after: int = 0
    tokens_saved: int = 0
...
class CopiumPreHook:
    ...
    optimization, use CopiumAgnoModel to wrap your model.
    ...
    using CopiumAgnoModel.

    Example:
        from agno.agent import Agent
        from agno.models.openai import OpenAIChat
        from copium.integrations.agno import CopiumPreHook, CopiumAgnoModel
    ...
    def __init__(
        ...
        """Initialize CopiumPreHook.

        Args:
            config: CopiumConfig for optimization settings (stored for future use)
            mode: CopiumMode (stored for future use)
    ...
    def __call__(self, run_input: Any, **kwargs: Any) -> Any:
        ...
        logger.debug(f"CopiumPreHook tracking request {request_id}")
        ...
        # Return input unchanged (use CopiumAgnoModel for actual optimization)
        return run_input
...
def create_copium_hooks(
    config: CopiumConfig | None = None,
    mode: CopiumMode = CopiumMode.OPTIMIZE,
    model: str = "gpt-4o",
    log_level: str = "INFO",
    token_alert_threshold: int | None = None,
) -> tuple[CopiumPreHook, CopiumPostHook]:
    """Create a pair of Copium hooks for Agno agents.
    ...
    """
    pre_hook = CopiumPreHook(config=config, mode=mode, model=model)
    post_hook = CopiumPostHook(
        log_level=log_level,
