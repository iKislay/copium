"""Agno model wrapper for Copium optimization.

This module provides CopiumAgnoModel, which wraps any Agno model
to apply Copium context optimization before API calls.
"""
...
@dataclass
class CopiumAgnoModel(Model):  # type: ignore[misc]
    """Agno model wrapper that applies Copium optimizations.
    ...
    Example:
        ...
        optimized = CopiumAgnoModel(wrapped_model=model)
        ...
        optimized = CopiumAgnoModel(wrapped_model=model, copium_config=config)

        # Agno reasoning with CopiumAgnoModel
        model = OpenAIChat(id="gpt-4o")
        wrapped = CopiumAgnoModel(wrapped_model=model)
        agent = Agent(
            model=wrapped,
            reasoning=True,
    ...
    id: str = field(default="copium-wrapper")
    name: str | None = field(default=None)
    provider: str | None = field(default=None)

    # CopiumAgnoModel specific fields
    wrapped_model: Any = field(default=None)
    copium_config: CopiumConfig | None = field(default=None)
    copium_mode: CopiumMode | None = field(default=None)
    ...
    _copium_provider: Any = field(default=None, repr=False, compare=False)
    _lock: threading.Lock = field(default_factory=threading.Lock, repr=False, compare=False)
    _initialized: bool = field(default=False, repr=False, compare=False)

    def __post_init__(self) -> None:
        """Initialize CopiumAgnoModel after dataclass construction."""
        ...
        if hasattr(self.wrapped_model, "id"):
            self.id = f"copium:{self.wrapped_model.id}"
        ...
        # Initialize config
        if self.copium_config is None:
            self.copium_config = CopiumConfig()

        # Handle deprecated mode parameter
        if self.copium_mode is not None:
            warnings.warn(
                "The 'copium_mode' parameter is deprecated. Use CopiumConfig(default_mode=...) instead.",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def underlying_model(self) -> Any:
        ...
        Example:
            wrapped = CopiumAgnoModel(wrapped_model=Claude(...))
            ...
            with self._lock:
                ...
                if self._pipeline is None:
                    if self.auto_detect_provider:
                        self._copium_provider = get_copium_provider(self.wrapped_model)
                        logger.debug(
                            f"Auto-detected provider: {self._copium_provider.__class__.__name__}"
                        )
                    else:
                        self._copium_provider = OpenAIProvider()
                    self._pipeline = TransformPipeline(
                        config=self.copium_config,
                        provider=self._copium_provider,
                    )
    ...
    def _convert_messages_to_openai(self, messages: list[Any]) -> list[dict[str, Any]]:
        """Convert Agno messages to OpenAI format for Copium.
    ...
    def _optimize_messages(self, messages: list[Any]) -> tuple[list[Any], OptimizationMetrics]:
        ...
        if self._has_thinking_blocks(openai_messages):
            logger.info("Skipping Copium optimization: messages contain extended thinking blocks")
        ...
        model_limit = (
            self._copium_provider.get_context_limit(model) if self._copium_provider else 128000
        )
        ...
        try:
            ...
            logger.warning(
                f"Copium optimization failed, using original messages: {type(e).__name__}: {e}"
            )
    ...
    def response(self, messages: list[Any], **kwargs: Any) -> Any:  # type: ignore[override]
        ...
        applies Copium optimization before delegating to wrapped_model.invoke().
    ...
    # These delegate to the wrapped model after applying Copium optimization
    # =========================================================================
    ...
    def invoke(self, messages: list[Any], **kwargs: Any) -> Any:
        ...
        logger.info(
            f"Copium optimized (invoke): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
    ...
    async def ainvoke(self, messages: list[Any], **kwargs: Any) -> Any:
        ...
        logger.info(
            f"Copium optimized (ainvoke): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
    ...
    def invoke_stream(self, messages: list[Any], **kwargs: Any) -> Iterator[Any]:
        ...
        logger.info(
            f"Copium optimized (invoke_stream): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
    ...
    async def ainvoke_stream(self, messages: list[Any], **kwargs: Any) -> AsyncIterator[Any]:
        ...
        logger.info(
            f"Copium optimized (ainvoke_stream): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
