"""Canonical filesystem contract for Copium.

This module defines the single source of truth for where Copium reads and
writes files. It introduces two canonical roots:

* ``COPIUM_CONFIG_DIR`` -- read-mostly configuration (defaults to
  ``~/.copium/config``). Holds model catalogs, plugin settings, and other
  configuration that users or admins edit.
* ``COPIUM_WORKSPACE_DIR`` -- read-write state (defaults to ``~/.copium``).
...
per-resource override (``COPIUM_SAVINGS_PATH``, ``COPIUM_TOIN_PATH``,
...
COPIUM_CONFIG_DIR_ENV = "COPIUM_CONFIG_DIR"
COPIUM_WORKSPACE_DIR_ENV = "COPIUM_WORKSPACE_DIR"

# ---------------------------------------------------------------------------
# Legacy per-resource env vars (kept for backward compatibility)
# ---------------------------------------------------------------------------

COPIUM_SAVINGS_PATH_ENV = "COPIUM_SAVINGS_PATH"
COPIUM_TOIN_PATH_ENV = "COPIUM_TOIN_PATH"
COPIUM_SUBSCRIPTION_STATE_PATH_ENV = "COPIUM_SUBSCRIPTION_STATE_PATH"

# ---------------------------------------------------------------------------
# Default sub-path fragments
# ---------------------------------------------------------------------------

_WORKSPACE_DIR_DEFAULT = ".copium"
...
def workspace_dir() -> Path:
    ...
    1. ``$COPIUM_WORKSPACE_DIR`` (trimmed, tilde-expanded) if set.
    2. ``~/.copium`` otherwise.
    """

    env_value = _env(COPIUM_WORKSPACE_DIR_ENV)
    if env_value:
        return Path(env_value).expanduser()
...
def config_dir() -> Path:
    ...
    1. ``$COPIUM_CONFIG_DIR`` (trimmed, tilde-expanded) if set.
    2. ``$COPIUM_WORKSPACE_DIR/config`` when the workspace env var is set
       so that a single override relocates both roots coherently.
    3. ``~/.copium/config`` otherwise.
    """

    env_value = _env(COPIUM_CONFIG_DIR_ENV)
    if env_value:
        return Path(env_value).expanduser()
    workspace_env = _env(COPIUM_WORKSPACE_DIR_ENV)
    if workspace_env:
...
def toin_path(explicit: str | os.PathLike[str] | None = None) -> Path:
    ...
    ``~/.copium/toin.json`` to preserve byte-for-byte backward compat.
    """
...
def log_dir() -> Path:
    """Return the directory for Copium log files."""
...
def bin_dir() -> Path:
    """Return the directory where Copium ships vendored binaries."""
...
def models_config_path() -> Path:
    ...
    Note: the ``COPIUM_MODEL_LIMITS`` env var is a *content* override
    ...
    location and deliberately ignores ``COPIUM_MODEL_LIMITS``.
    """
