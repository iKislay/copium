...
class AgentSavingsProfile:
    ...
    def proxy_env(self) -> dict[str, str]:
        """Return env vars for Copium proxy/wrapper entry points."""

        return {
            "COPIUM_MODE": self.proxy_mode,
            "COPIUM_SAVINGS_PROFILE": self.name,
            "COPIUM_SAVINGS_TARGET": f"{self.target_savings:.2f}",
            "COPIUM_TARGET_RATIO": f"{self.target_ratio:.2f}",
            "COPIUM_COMPRESS_USER_MESSAGES": ("1" if self.compress_user_messages else "0"),
            "COPIUM_COMPRESS_SYSTEM_MESSAGES": ("1" if self.compress_system_messages else "0"),
            "COPIUM_PROTECT_RECENT": str(self.protect_recent),
            "COPIUM_PROTECT_ANALYSIS_CONTEXT": ("1" if self.protect_analysis_context else "0"),
            "COPIUM_MIN_TOKENS": str(self.min_tokens_to_compress),
            "COPIUM_MAX_ITEMS": str(self.max_items_after_crush),
            "COPIUM_SMART_CRUSHER_COMPACTION": (
                "1" if self.smart_crusher_with_compaction else "0"
            ),
            "COPIUM_FORCE_KOMPRESS": "1" if self.force_kompress else "0",
            "COPIUM_ACCURACY_GUARD": self.accuracy_guard,
        }
