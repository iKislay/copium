...
(``x-codex-primary-used-percent`` etc.) and copium captured those headers
...
CODEX_USAGE_URL = (
    os.environ.get("COPIUM_CODEX_USAGE_URL", "https://chatgpt.com/backend-api/wham/usage").strip()
...
class CodexRateLimitState(QuotaTracker):
    ...
    be registered with the :class:`~copium.subscription.base.QuotaTrackerRegistry`.
    ...
    copium, so :meth:`start` and :meth:`stop` are no-ops.
    """
