...
- Thread-safe state updates via threading.Lock (consistent with copium patterns)
...
token breakdowns per window that enable:
  - Copium efficiency metrics (tokens saved = raw - what proxy sent)
...
from copium.subscription.base import QuotaTracker
from copium.subscription.client import SubscriptionClient
...
_PERSIST_FILE_ENV = _paths.COPIUM_SUBSCRIPTION_STATE_PATH_ENV
_DEFAULT_PERSIST_DIR = ".copium"
...
# without uninstalling the binary or unsetting ``COPIUM_CONTEXT_TOOL``.
# Used for diagnostics and for environments where RTK is intentionally
# excluded from copium accounting (e.g. shadow tests).
#
...
_RTK_WIRING_ENV = "COPIUM_RTK_WIRING"
...
# ``copium/proxy/server.py``: a non-blocking ``fcntl.flock`` on
# ``COPIUM_RTK_POLL_LOCK`` (default under the workspace dir). Only the
...
_RTK_POLL_LOCK_ENV = "COPIUM_RTK_POLL_LOCK"
...
def _validate_rtk_env_at_startup() -> None:
    ...
    Raises ``ValueError`` loudly if ``COPIUM_RTK_WIRING`` is set to an
...
class SubscriptionTracker(QuotaTracker):
    ...
    Implements :class:`~copium.subscription.base.QuotaTracker` so it can
    be registered with :func:`~copium.subscription.base.get_quota_registry`
    ...
    def __init__(
        ...
        # :func:`~copium.proxy.helpers._get_context_tool_stats`) so it
        ...
        # entire pre-Copium RTK history as one fake delta. Switching to
    ...
    def update_contribution(
        ...
        *,
        ...
        """Update copium contribution counters for the current session window.
        ...
        :func:`copium.proxy.helpers._get_rtk_stats`) when the caller does
        ...
        reads 0 instead of the entire pre-Copium RTK history.
    ...
    def _poll_rtk_delta(self) -> int:
        ...
        :func:`copium.proxy.helpers._get_rtk_stats` and reads the
        ...
        - ``COPIUM_RTK_WIRING=disabled`` — operator opt-out.
        ...
        PR-G2 remediation (H1): ``COPIUM_RTK_WIRING`` is now validated at
        ...
        try:
            ...
            from copium.proxy.helpers import _get_rtk_stats
        ...
        # the pre-Copium RTK history. Falls back to the top-level
    ...
    def _try_acquire_rtk_poll_lock(self) -> bool:
        ...
        ``copium/proxy/server.py`` (fcntl.flock, LOCK_EX | LOCK_NB).
        """
    ...
    def _compute_used_since_reset(self, window: RateLimitWindow | None) -> int | None:
        ...
        if now < window.resets_at:
            return None
        try:
            from copium.subscription import session_tracking
    ...
    async def _maybe_poll(self) -> None:
        ...
        if not is_active:
            ...
            from copium.subscription.client import read_cached_oauth_token
        ...
        try:
            from copium.observability.metrics import get_otel_metrics
    ...
    def _maybe_reset_contribution(self, snapshot: SubscriptionSnapshot) -> None:
        ...
        if (
            ...
            logger.info("5h window rolled over; resetting copium contribution counters")
            self._state.contribution = CopiumContribution()
...
def configure_subscription_tracker(
    ...
    a typo (``COPIUM_RTK_WIRING=enabld``) crashes the proxy at startup
