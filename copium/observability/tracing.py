"""OTEL tracing helpers for Copium and Langfuse."""
...
from .metrics import _copium_version, _parse_bool, _parse_key_value_pairs
...
_tracing_lock = Lock()
_global_tracer: CopiumTracer | None = None
...
class LangfuseTracingConfig:
    """Configuration for Copium-managed Langfuse OTLP trace export."""
    ...
    @classmethod
    def from_env(cls, *, default_service_name: str = "copium") -> LangfuseTracingConfig:
        public_key = os.environ.get("LANGFUSE_PUBLIC_KEY", "").strip()
        secret_key = os.environ.get("LANGFUSE_SECRET_KEY", "").strip()

        return cls(
            enabled=_parse_bool(
                os.environ.get("COPIUM_LANGFUSE_ENABLED"),
                default=False,
            ),
            ...
            service_name=os.environ.get(
                "COPIUM_LANGFUSE_SERVICE_NAME", default_service_name
            ).strip()
            or default_service_name,
            resource_attributes=_parse_key_value_pairs(
                os.environ.get("COPIUM_LANGFUSE_RESOURCE_ATTRIBUTES")
            ),
        )
...
class CopiumTracer:
    """Tracer facade used by shared Copium compression paths."""

    def __init__(self, tracer_provider: Any | None = None):
        if tracer_provider is None:
            self._tracer = trace.get_tracer(_SCOPE_NAME, _copium_version())
        else:
            self._tracer = tracer_provider.get_tracer(_SCOPE_NAME, _copium_version())
...
def get_copium_tracer() -> CopiumTracer:
    global _global_tracer

    if _global_tracer is None:
        with _tracing_lock:
            if _global_tracer is None:
                _global_tracer = CopiumTracer()

    return _global_tracer


def set_copium_tracer(copium_tracer: CopiumTracer) -> CopiumTracer:
    global _global_tracer
    with _tracing_lock:
        _global_tracer = copium_tracer
    return copium_tracer
...
def configure_langfuse_tracing(
    ...
    try:
        ...
        logger.warning(
            ...
            "Install copium-ai[otel] to enable Langfuse OTLP tracing."
        )
        return get_copium_tracer()
    ...
    copium_tracer = CopiumTracer(tracer_provider=tracer_provider)
...
def shutdown_copium_tracing() -> None:
    global _global_tracer
    global _owned_tracer_provider
    global _owned_langfuse_config
