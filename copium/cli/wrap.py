"""Wrap CLI commands to run through Copium proxy.

Usage:
    copium wrap claude                    # Start proxy + context tool + claude
    copium wrap copilot -- --model ...    # Start proxy + launch GitHub Copilot CLI
    copium wrap codex                     # Start proxy + OpenAI Codex CLI
    copium wrap aider                     # Start proxy + aider
...
from copium._version import __version__ as _COPIUM_VERSION
from copium.agent_savings import (
    apply_agent_savings_env_defaults,
)
from copium.copilot_auth import (
    has_oauth_auth,
    resolve_client_bearer_token,
    resolve_copilot_api_url,
    resolve_subscription_bearer_token_details,
)
from copium.providers.aider import build_launch_env as _build_aider_launch_env
from copium.providers.claude import (
    TOOL_SEARCH_DEFAULT,
    TOOL_SEARCH_ENV,
)
from copium.providers.claude import (
...
from copium.providers.codex import build_launch_env as _build_codex_launch_env
from copium.providers.codex.install import codex_uses_chatgpt_auth
from copium.providers.codex.threads import retag_to_copium, retag_to_native
from copium.providers.copilot import (
...
from copium.providers.cursor import render_setup_lines as _render_cursor_setup_lines
from copium.providers.mistral_vibe import build_launch_env as _build_mistral_vibe_launch_env
from copium.providers.openclaw import (
...
from copium.proxy.project_context import with_project_prefix as _with_project_prefix

from .main import main

_CONTEXT_TOOL_ENV = "COPIUM_CONTEXT_TOOL"
...
_WRAP_PROXY_TIMEOUT_ENV = "COPIUM_WRAP_PROXY_TIMEOUT"
...
def _selected_context_tool() -> str:
    ...
    ``COPIUM_CONTEXT_TOOL=lean-ctx`` to let lean-ctx configure the supported
...
def _wrap_agent_savings_profile(agent_type: str) -> str | None:
    ...
    if agent_type not in _AGENT_SAVINGS_WRAP_AGENTS:
        return None
    return os.environ.get("COPIUM_SAVINGS_PROFILE") or _DEFAULT_AGENT_SAVINGS_PROFILE
...
def _print_telemetry_notice() -> None:
    ...
    Respects the COPIUM_TELEMETRY and COPIUM_TELEMETRY_WARN feature flags.
    Does nothing when telemetry or warnings are disabled.
    """
    from copium.telemetry.beacon import format_telemetry_notice
...
def _check_proxy(port: int) -> bool:
    """Check if Copium proxy is running on given port."""
    try:
...
def _format_unbindable_port_error(port: int, error: OSError, agent_type: str) -> str:
    ...
    command = "copium proxy"
    if agent_type != "unknown":
        command = f"copium wrap {agent_type}"
...
def _start_proxy(
    ...
    *,
    ...
    Logs are written to ~/.copium/logs/proxy.log to avoid pipe buffer
    deadlocks (macOS pipe buffer is ~64KB — a busy proxy fills it quickly,
    blocking the process).
    """
    cmd = [sys.executable, "-m", "copium.cli", "proxy", "--port", str(port)]

    # Forward COPIUM_MODE env var so the proxy respects the user's mode choice
    copium_mode = os.environ.get("COPIUM_MODE")
    if copium_mode:
        cmd.extend(["--mode", copium_mode])
    ...
    _backend = backend or os.environ.get("COPIUM_BACKEND")
    if _backend:
        cmd.extend(["--backend", _backend])

    _anyllm = anyllm_provider or os.environ.get("COPIUM_ANYLLM_PROVIDER")
    if _anyllm:
        cmd.extend(["--anyllm-provider", _anyllm])

    _region = region or os.environ.get("COPIUM_REGION")
    if _region:
    ...
    if agent_type != "unknown":
        proxy_env["COPIUM_AGENT_TYPE"] = agent_type
        proxy_env.setdefault("COPIUM_STACK", f"wrap_{agent_type}")
...
def _setup_lean_ctx_agent(agent: str, verbose: bool = False) -> Path | None:
    ...
    from copium.lean_ctx import get_lean_ctx_path
    from copium.lean_ctx.installer import ensure_lean_ctx
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="copium-lean-ctx-") as setup_cwd:
                ...
                retained_hooks = [
                    ...
                    if not (
                        ...
                        and any(
                            ...
                            for marker in _COPIUM_HOOK_MARKERS
                        )
                    )
                ]
...
def _write_claude_wrap_base_url(
    ...
    *,
    ...
    routes through the Copium proxy without touching the global user settings
...
def _setup_copium_mcp(
    registrar: Any, port: int, *, verbose: bool = False, force: bool = False
) -> None:
    """Register the copium MCP server with the given agent (idempotent).
    ...
    nothing — the agent has no ``copium_retrieve`` tool to call.
    ...
    """
    from copium.mcp_registry import build_copium_spec, format_result
...
def _setup_serena_mcp(
    ...
    A prior ``copium wrap`` may have persisted a Serena entry built from an
    ...
    config is one Copium installed, force-update it to the current spec. A
    user-managed Serena (absent from our ledger) is left untouched and the
    mismatch is reported as before.
    """
    from copium.mcp_registry import build_serena_spec, format_result
    from copium.mcp_registry.base import RegisterStatus
    from copium.mcp_registry.ledger import copium_installed_matching, record_install
    ...
    # Migrate a stale Copium-installed entry. register_server won't overwrite
    # a differing spec without force, so an older Copium Serena entry would
    # otherwise persist across re-wraps. Force-update it only when the ledger
    # proves Copium installed the entry that's currently on disk — never a
    # user-managed Serena.
    if (
        result.status == RegisterStatus.MISMATCH
        and not force
        and copium_installed_matching(registrar.name, registrar.get_server("serena"))
    ):
    ...
    line = format_result(
        ...
        label="Serena MCP",
        verbose=verbose,
        overwrite_hint="update or remove the existing serena MCP entry, then rerun copium wrap",
        restart_hint=f"restart {registrar.display_name} if it was already running",
    )
    if line is not None:
        click.echo(line)
...
def _remove_copium_installed_serena_mcp(registrar: Any) -> str:
    """Remove Serena MCP only if the ledger proves Copium installed it."""
    from copium.mcp_registry.ledger import clear_install, copium_installed_matching

    current = registrar.get_server("serena")
    if not copium_installed_matching(registrar.name, current):
        return "not_copium_owned"
...
def _disable_serena_mcp(registrar: Any, *, verbose: bool = False) -> None:
    ...
    Serena is registered by default, so a prior ``copium wrap`` persists a
    ...
    remove the entry Copium installed. A user-managed Serena (absent from
    ...
    status = _remove_copium_installed_serena_mcp(registrar)
    if status == "removed":
    ...
    elif status == "not_copium_owned":
        click.echo(
            "  Serena MCP is present but user-managed — leaving it in place "
            "(--no-serena only removes entries Copium installed)"
        )
    else:  # "failed"
        click.echo(
...
def _setup_code_graph(verbose: bool = False) -> bool:
    ...
    """
    from copium.graph.installer import ensure_cbm, get_cbm_path
...
_RTK_MARKER = "<!-- copium:rtk-instructions -->"

# Memory MCP markers
_MEMORY_MCP_MARKER = "# --- Copium memory MCP (auto-injected) ---"
_MEMORY_MCP_END = "# --- end Copium memory ---"
_MEMORY_AGENTS_MARKER = "<!-- copium:memory-instructions -->"

# Codex config injection markers
_CODEX_TOP_LEVEL_MARKER = "# --- Copium proxy (auto-injected by copium wrap codex) ---"
_CODEX_END_MARKER = "# --- end Copium ---"
_CODEX_MCP_MARKER = "# --- Copium MCP server ---"
_CODEX_MCP_END = "# --- end Copium MCP server ---"
# File name used for the pre-wrap snapshot of the Codex config file.  The
# snapshot lets `copium unwrap codex` restore the exact prior state, even
# if the user had their own `model_provider` / `[model_providers.*]` config
# before running wrap.
_CODEX_CONFIG_BACKUP_SUFFIX = ".copium-backup"
...
def _strip_codex_copium_blocks(content: str, *, remove_mcp: bool = False) -> str:
    """Remove all Copium-managed blocks from a Codex ``config.toml`` string.
    ...
    if remove_mcp:
        # Remove Copium-managed MCP blocks written by `wrap codex`.
        content = _remove_marker_span(content, _CODEX_MCP_MARKER, _CODEX_MCP_END)
        content = re.sub(
            r"(?ms)^# --- Copium MCP server: [^\n]+ ---\n.*?"
            r"^# --- end Copium MCP server: [^\n]+ ---\n?",
            "",
            content,
        )
    ...
    # Strip any orphaned `[model_providers.copium]` table with the fields we
    # write.  We only remove it if the table is recognisably ours (base_url
    # mentions localhost and a Copium proxy port).  This protects users who
    # happen to have a differently configured `copium` provider.
...
# Top-level bare keys we redirect to copium values when the user already
...
def _redirect_existing_top_level_keys(content: str, port: int) -> str:
    ...
    which would break ``codex`` startup after ``copium wrap codex`` runs
    ...
    the value with the copium one and append ``# was: <original-value>``
    ...
    pre-wrap file can be restored byte-for-byte on ``copium unwrap
    codex``.
...
def _snapshot_codex_config_if_unwrapped(config_file: Path, backup_file: Path) -> None:
    ...
    Called as the first step of every Copium injection into Codex's
    ``config.toml``.  Guarantees that ``copium unwrap codex`` can restore the
    ...
    * If the config already contains a Copium marker, a wrap run is already
...
def _ensure_rtk_binary(verbose: bool = False) -> Path | None:
    ...
    from copium.rtk.installer import ensure_rtk
...
# case-insensitively by copium.proxy.project_context.PROJECT_HEADER).
_PROJECT_HEADER_NAME = "X-Copium-Project"


def _project_name_from_cwd() -> str | None:
    """Project label for X-Copium-Project: basename of the launch directory.
...
def _apply_project_header_env(env: dict[str, str]) -> None:
    """Inject X-Copium-Project into ``ANTHROPIC_CUSTOM_HEADERS``.
    ...
    Copium proxy uses the X-Copium-Project header for per-project
    savings attribution.  An existing user-supplied x-copium-project
...
def _inject_codex_provider_config(port: int) -> None:
    """Inject a Copium model provider into Codex's config.toml.

    Two keys need to be in effect for the proxy to route all traffic:

    * ``model_provider = "copium"`` — selects the custom provider for
    ...
    * ``openai_base_url = "http://127.0.0.1:{port}/v1"`` — overrides the
    ...
    in place to the copium values (with the previous value kept in a
    ...
    ``config.toml.copium-backup`` so ``copium unwrap codex``
    ...
    provider_section = (
        ...
        # env var (COPIUM_PROJECT, set by `copium wrap codex`) exists at
        # Codex runtime.  Inline table keeps the key inside this section so
        # _strip_codex_copium_blocks removes it with the rest of the block.
        f'env_http_headers = {{ "{_PROJECT_HEADER_NAME}" = "COPIUM_PROJECT" }}\n'
        f"{_CODEX_END_MARKER}\n"
    )

    # The two redirectable keys and their copium target values.
    _REDIRECT_TARGETS = {
    ...
    try:
        ...
        if config_file.exists():
            content = config_file.read_text()
            # Remove any prior Copium-managed blocks before re-injecting so
            # the operation is idempotent and supports port changes.
            content = _strip_codex_copium_blocks(content)
            ...
            # to the copium values; for keys the user has not declared,
        ...
        click.echo(f"  Codex config: injected Copium provider (WS + HTTP) into {config_file}")
        ...
        retag_to_copium(_codex_home_dir())
    except Exception as e:
...
def _restore_codex_provider_config() -> tuple[str, Path]:
    ...
    * ``"cleaned"``  — no backup existed, but the Copium-managed block was
      found and stripped out (preserving surrounding user content).
    * ``"removed"``  — the config file only contained Copium-managed
      content (created by wrap) and has been deleted.
    * ``"noop"``     — nothing to undo; no Copium marker and no backup.
    """
    config_file, backup_file = _codex_config_paths()
    ...
    if config_file.exists():
        ...
        if _CODEX_TOP_LEVEL_MARKER in original or _CODEX_END_MARKER in original:
            cleaned = _strip_codex_copium_blocks(original, remove_mcp=True)
            if not cleaned.strip():
                # Nothing left but Copium content — remove the file entirely
...
def _emit_wrap_interrupted(agent: str, marker_path: Path | None) -> None:
    ...
    re-running the same ``copium wrap <agent>`` command is idempotent and
    safe.
    """
    if marker_path is not None:
        click.echo(
            f"\n  Wrap was interrupted; marker file at {marker_path} is on "
            f"disk. Rerun `copium wrap {agent}` to retry — it's idempotent."
        )
    else:
        click.echo(
            f"\n  Wrap was interrupted before any on-disk changes. Rerun "
            f"`copium wrap {agent}` to retry — it's idempotent."
        )
...
def _print_wrap_banner(agent: str) -> None:
    """Print a centered ``COPIUM WRAP: <AGENT>`` banner.
    ...
    """
    title = f"COPIUM WRAP: {agent.upper()}"
...
def _inject_memory_mcp_config(db_path: str, user_id: str) -> None:
    """Register copium memory as an MCP server in Codex's config.toml.
    ...
    mcp_section = (
        f"\n{_MEMORY_MCP_MARKER}\n"
        f"[mcp_servers.copium_memory]\n"
        f'command = "{python_bin}"\n'
        f'args = ["-m", "copium.memory.mcp_server", "--db", "{db_path_toml}", "--user", "{user_id}"]\n'
...
def _inject_memory_agents_md(file_path: Path) -> bool:
    ...
    memory_block = (
        ...
        "## Memory\n\n"
        "Use the `copium_memory` MCP server for persistent cross-session knowledge.\n\n"
...
def _inject_continue_rtk_systemmessage(config_file: Path, verbose: bool = False) -> bool:
    ...
    * each ``models[i].systemMessage`` where ``models[i]`` is a dict
    ...
    The RTK marker (``<!-- copium:rtk-instructions -->``) is the idempotency
...
def _proxy_health_config(payload: dict[str, Any] | None) -> dict[str, Any] | None:
    """Extract the config block from a Copium /health payload."""
    if payload is None:
        return None
...
def _agent_savings_config_mismatches(
    ...
    checks: tuple[tuple[str, str, str, str], ...] = (
        ("COPIUM_SAVINGS_PROFILE", "savings_profile", "savings-profile", "str"),
        ("COPIUM_TARGET_RATIO", "target_ratio", "target-ratio", "float"),
        (
            "COPIUM_COMPRESS_USER_MESSAGES",
            "compress_user_messages",
            "compress-user-messages",
            "bool",
        ),
        (
            "COPIUM_COMPRESS_SYSTEM_MESSAGES",
            "compress_system_messages",
            "compress-system-messages",
            "bool",
        ),
        ("COPIUM_PROTECT_RECENT", "protect_recent", "protect-recent", "int"),
        (
            "COPIUM_PROTECT_ANALYSIS_CONTEXT",
            "protect_analysis_context",
            "protect-analysis-context",
            "bool",
        ),
        ("COPIUM_MIN_TOKENS", "min_tokens_to_crush", "min-tokens", "int"),
        ("COPIUM_MAX_ITEMS", "max_items_after_crush", "max-items", "int"),
        (
            "COPIUM_SMART_CRUSHER_COMPACTION",
            "smart_crusher_with_compaction",
            "smart-crusher-compaction",
            "bool",
        ),
        ("COPIUM_ACCURACY_GUARD", "accuracy_guard", "accuracy-guard", "str"),
    )
...
def _proxy_needs_version_restart(payload: dict[str, Any] | None) -> bool:
    """Return True when a running Copium proxy uses a different package version."""
    running_version = _proxy_version(payload)
    return (
        running_version is not None
        and running_version != "unknown"
        and _COPIUM_VERSION != "unknown"
        and running_version != _COPIUM_VERSION
    )
...
def _stop_local_proxy_for_unwrap(port: int) -> str:
    """Stop a local Copium proxy for durable unwrap commands.

    Returns a status string:
      * ``"stopped"``: a Copium proxy was identified and stopped.
      * ``"not_running"``: nothing is listening on the requested port.
      * ``"unidentified"``: something is listening, but it did not expose
        Copium's health/config payload, so we did not kill it.
      * ``"no_pid"``: the service looked like Copium but did not expose a PID.
...
def _echo_unwrap_proxy_stop_status(status: str, port: int) -> None:
    ...
    if status == "stopped":
        click.echo(f"  Stopped local Copium proxy on port {port}.")
    elif status == "not_running":
        click.echo(f"  No local Copium proxy detected on port {port}.")
...
def _recover_persistent_proxy(port: int) -> bool:
    ...
    from copium.install.models import InstallPreset, SupervisorKind
    from copium.install.runtime import start_detached_agent, start_persistent_docker, wait_ready
...
def _should_use_copilot_oauth(
    *,
    ...
    effective_backend = backend or os.environ.get("COPIUM_BACKEND")
...
def _push_runtime_env(port: int, no_proxy: bool) -> None:
    ...
    if no_proxy:
        return
    from copium.proxy import runtime_env as _rt
    ...
    if not no_proxy:
            ...
            if probe_ready(manifest.health_url):
                ...
                if helpers._proxy_needs_version_restart(health_payload):
                    ...
                    if active_sessions > 0 or other_wrappers:
                        ...
                        click.echo(
                            f"  Proxy on port {port} is running Copium {running_version}; "
                            f"current CLI is {_COPIUM_VERSION}."
                        )
                        click.echo(
        ...
        if helpers._check_proxy(port):
            ...
            if helpers._proxy_needs_version_restart(health_payload):
                ...
                click.echo(
                    f"  Proxy on port {port} is running Copium {running_version}; "
                    f"restarting with {_COPIUM_VERSION}..."
                )
        ...
        click.echo(f"  Starting Copium proxy on port {port}...")
        try:
            proc = cast(
...
def _make_cleanup(proxy_proc_holder: list, port: int = 8787) -> Any:
    ...
    Only kills the proxy when no other live copium-wrapped clients remain,
...
def _launch_tool(
    ...
    *,
    ...
    try:
        click.echo()
        padded = f"COPIUM WRAP: {tool_label}".center(47)
        click.echo("  ╔═══════════════════════════════════════════════╗")
        click.echo(f"  ║{padded}║")
        click.echo("  ╚═══════════════════════════════════════════════╝")
        click.echo()
        ...
        click.echo()
        click.echo(f"  Launching {tool_label} (API routed through Copium)...")
...
def _build_openclaw_plugin_entry(
    *,
    ...
    """Merge managed Copium plugin settings with any existing entry payload."""
...
def _write_openclaw_plugin_entry(openclaw_bin: str, entry: dict[str, object]) -> None:
    """Persist the Copium plugin config entry."""
    _run_checked(
        [
            openclaw_bin,
            "config",
            "set",
            "plugins.entries.copium",
            json.dumps(entry, separators=(",", ":")),
            "--strict-json",
        ],
        action="openclaw config set plugins.entries.copium",
    )
...
def _copy_openclaw_plugin_into_extensions(
    *,
    ...
    target_dir = extensions_dir / "copium"
...
def wrap(ctx: click.Context) -> None:
    ...
    Supported tools (one Click subcommand per tool):
        ...
        copium wrap vibe                # Mistral Vibe
        copium wrap cursor              # Cursor (prints config instructions)
        copium wrap cline               # Cline (VS Code; prints config instructions)
        copium wrap continue            # Continue (VS Code/JetBrains; injects systemMessage)
    ...
    \b
    `wrap` vs `proxy`:
        - `copium wrap <tool>` — convenience: starts the proxy for you,
...
@click.option(
    ...
    "(env: COPIUM_BACKEND). For Vertex, prefer CLAUDE_CODE_USE_VERTEX=1 (native, "
    "keeps your GCP auth) over a litellm backend.",
)
@click.option(
    "--region",
    default=None,
    help="Cloud region for Vertex/Bedrock backends (env: COPIUM_REGION).",
)
...
def claude(
    ...
    """Launch Claude Code through Copium proxy.

    \b
    Sets ANTHROPIC_BASE_URL to route all Anthropic API calls through Copium.
    ...
    Examples:
        ...
        copium wrap claude --code-graph        # With code graph intelligence
        copium wrap claude --no-context-tool  # Skip CLI context-tool setup
        copium wrap claude --no-mcp           # Skip MCP retrieve tool registration
        copium wrap claude --no-serena        # Skip Serena MCP registration
    """
    if prepare_only:
        if not no_rtk:
            if _selected_context_tool() == _CONTEXT_TOOL_LEAN_CTX:
    ...
    # Memory sync BEFORE proxy startup — sync copium DB ↔ Claude's files
    if memory:
        try:
            import subprocess as _sp

            mem_dir = Path.cwd() / ".copium"
    ...
    try:
        ...
        # Copium and the proxy compresses the request, then forwards to the
        ...
        # by Copium. This is the turnkey Vertex compression path.
        ...
        if not no_mcp:
            ...
            _setup_copium_mcp(ClaudeRegistrar(), port, verbose=verbose)
        elif verbose:
        ...
        click.echo()
        click.echo("  Launching Claude Code (API routed through Copium)...")
        if use_vertex:
            click.echo(
        ...
        if use_vertex:
            ...
            # we only redirect its Vertex endpoint to Copium.
        ...
        # daemon's environment) also route through Copium.
        ...
        # directory's name via X-Copium-Project (user override wins).
...
@click.option("--keep-mcp", is_flag=True, help="Keep Copium MCP registrations")
...
def unwrap_claude(
    ...
    """Undo durable setup from ``copium wrap claude``."""
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║          COPIUM UNWRAP: CLAUDE              ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    if not keep_mcp:
        ...
        if registrar.detect():
            ...
            if removed_copium:
                click.echo("  Removed Copium MCP retrieve tool from Claude.")
            else:
                click.echo("  Copium MCP retrieve tool was not registered in Claude.")
            if removed_code_graph:
                click.echo("  Removed code graph MCP server from Claude.")
            if serena_status == "removed":
                click.echo("  Removed Copium-installed Serena MCP server from Claude.")
            elif serena_status == "failed":
                click.echo("  Serena MCP server matched Copium ledger but could not be removed.")
        else:
    ...
    click.echo()
    click.echo("✓ Claude is no longer durably wrapped by Copium.")
...
@click.option(
    "--backend",
    default=None,
    help="API backend for the proxy: 'anthropic', 'anyllm', 'litellm-vertex', etc. (env: COPIUM_BACKEND)",
)
@click.option(
    "--anyllm-provider",
    default=None,
...
@click.option(
    ...
    is_flag=True,
    help=(
        "Experimental: route GitHub-authenticated Copilot CLI traffic through Copium "
...
def copilot(
    ...
    """Launch GitHub Copilot CLI through Copium proxy.

    \b
    Configures Copilot CLI BYOK provider variables so Copilot routes through
    the local Copium proxy. In auto mode, the wrapper uses Anthropic-style
    ...
    Examples:
        ...
        copium wrap copilot --provider-type openai --wire-api responses -- --model gpt-5.4
        copium wrap copilot --subscription -- --model gpt-4.1
        copium wrap copilot --no-context-tool -- --prompt "explain this file"
    ...
    if _should_use_copilot_oauth(
        ...
        if not client_bearer:
            raise click.ClickException(
                ...
                "token, but none could be resolved. Run `copium copilot-auth login` first, or set "
        ...
        if not env.get("COPILOT_PROVIDER_API_KEY"):
            ...
            click.echo(
                ...
                f"  `copium wrap copilot` uses Copilot's BYOK mode, which bypasses GitHub's\n"
                f"  Copilot API and routes requests directly to the model provider through the\n"
                f"  Copium proxy. A GitHub Copilot subscription alone is not sufficient.\n\n"
                f"  Set one of:\n"
    ...
    if not subscription and not _copilot_model_configured(copilot_args, env):
        ...
        if _is_auto_model(raw_model):
            click.echo(
                ...
                "      copium wrap copilot --subscription -- --model auto"
            )
            raise SystemExit(1)
        else:
            click.echo(
...
def codex(
    ...
    """Launch OpenAI Codex CLI through Copium proxy.

    \b
    Sets OPENAI_BASE_URL to route all OpenAI API calls through Copium.
    Sets up the selected CLI context tool so Codex uses token-optimized
    commands (60-90% savings on shell output). Also
    registers the copium MCP server in the active Codex config file
    so Codex can call ``copium_retrieve`` on compression markers.

    \b
    Examples:
        copium wrap codex                         # Start proxy + context tool + mcp + codex
        copium wrap codex -- "fix the bug"        # Pass prompt to codex
        copium wrap codex --no-context-tool       # Skip CLI context-tool setup
        copium wrap codex --no-mcp                # Skip MCP retrieve tool registration
        copium wrap codex --no-serena             # Skip Serena MCP registration
        copium wrap codex --port 9999             # Custom proxy port
        copium wrap codex --backend anyllm --anyllm-provider groq
    """
    # Snapshot Codex config.toml BEFORE any wrap-time mutation so
    # `copium unwrap codex` can restore the user's pre-wrap state
    # byte-for-byte. The snapshot is a no-op if the backup already exists
    # or if the file already has Copium markers, so this is safe to
    ...
    # Register copium MCP server in Codex config.toml so Codex can
    # call copium_retrieve on compression markers from the proxy.
    if not no_mcp:
        from copium.mcp_registry import CodexRegistrar
        ...
        _setup_copium_mcp(CodexRegistrar(), port, verbose=verbose, force=True)
    elif verbose:
        click.echo("  Skipping MCP retrieve tool (--no-mcp)")
    ...
    if memory:
        ...
        try:
            ...
            from copium.memory.sync import sync_import
    ...
    # X-Copium-Project header to COPIUM_PROJECT via env_http_headers, so
    # Codex sends it only when this var is set.  A user-set value wins.
    _codex_project = _project_name_from_cwd()
    if _codex_project and "COPIUM_PROJECT" not in env:
        env["COPIUM_PROJECT"] = _codex_project

    # Inject Copium provider into Codex config so WebSocket traffic also
...
def aider(
    ...
    """Launch aider through Copium proxy.

    \b
    Sets OPENAI_API_BASE to route all API calls through Copium.
    Sets up the selected CLI context tool so aider uses token-optimized commands.

    \b
    Examples:
        copium wrap aider                              # Start proxy + context tool + aider
        copium wrap aider -- --model gpt-4o            # Use GPT-4o
        copium wrap aider -- --model claude-sonnet-4   # Use Claude
        copium wrap aider --no-context-tool            # Skip CLI context-tool setup
...
def vibe(
    ...
    """Launch Mistral Vibe through Copium proxy.

    \b
    Sets VIBE_PROVIDERS to route all Mistral API calls through Copium.

    \b
    Examples:
        copium wrap vibe                         # Start proxy + vibe
        copium wrap vibe -- "fix the bug"        # Pass prompt to vibe
        copium wrap vibe --port 9999             # Custom proxy port
        copium wrap vibe --no-context-tool       # Skip CLI context-tool setup
    """
    if prepare_only:
        return
...
def cursor(
    ...
    """Start Copium proxy for use with Cursor.
    ...
    \b
    Example:
        copium wrap cursor                # Start proxy + context-tool instructions
        copium wrap cursor --no-context-tool  # Proxy only, no CLI context tool
        copium wrap cursor --port 9999    # Custom proxy port
    """
...
def cline(
    ...
    """Start Copium proxy for use with Cline (VS Code extension).
    ...
    the API Base URL to point at the local Copium proxy.

    \b
    Uninstall: there is no ``copium unwrap cline`` subcommand. To remove the
    injected guidance, hand-edit ``.clinerules`` at the project root and
    delete everything between ``<!-- copium:rtk-instructions -->`` and
    ``<!-- /copium:rtk-instructions -->`` (inclusive). If ``lean-ctx`` mode
    ...
    \b
    Examples:
        copium wrap cline                  # Start proxy + .clinerules instructions
        copium wrap cline --no-context-tool # Proxy only, no CLI context tool
        copium wrap cline --port 9999      # Custom proxy port
    """
...
def continue_dev(
    ...
    """Start Copium proxy for use with Continue (VS Code / JetBrains).
    ...
    \b
    Uninstall: there is no ``copium unwrap continue`` subcommand. To remove
    the injected guidance, hand-edit ``.continue/config.json`` and delete
    everything between ``<!-- copium:rtk-instructions -->`` and
    ``<!-- /copium:rtk-instructions -->`` (inclusive) from every
    ...
    \b
    Examples:
        copium wrap continue                # Start proxy + inject systemMessage
        copium wrap continue --no-context-tool   # Proxy only
        copium wrap continue --port 9999    # Custom proxy port
        copium wrap continue --config path/to/config.json
    """
...
def goose(
    ...
    """Launch Goose (Block) CLI through Copium proxy.

    \b
    Sets OPENAI_BASE_URL and ANTHROPIC_BASE_URL to route Goose's API calls
    through Copium. Sets up the selected CLI context tool by injecting RTK
    guidance into .goosehints at the project root (Goose reads this file as
    extra system context).

    \b
    Uninstall: there is no ``copium unwrap goose`` subcommand. To remove the
    ...
    \b
    Examples:
        copium wrap goose                          # Start proxy + context tool + goose
        copium wrap goose -- session               # Start a Goose session
        copium wrap goose -- --provider anthropic  # Pass args to goose
        copium wrap goose --no-context-tool        # Skip CLI context-tool setup
    """
...
def openhands(
    ...
    """Launch OpenHands CLI through Copium proxy.

    \b
    Sets OPENAI_BASE_URL / ANTHROPIC_BASE_URL to route OpenHands' API calls
    through Copium. Instructions are injected via the
    ...
    ``<!-- copium:rtk-instructions -->`` marker. To uninstall, simply do not
    ...
    \b
    Examples:
        copium wrap openhands                # Start proxy + context tool + openhands
        copium wrap openhands -- --task ...  # Pass args to openhands
        copium wrap openhands --no-context-tool
    """
...
@click.option("--proxy-port", default=8787, type=int, help="Copium proxy port")
@click.option("--startup-timeout-ms", default=20000, type=int, help="Proxy startup timeout")
@click.option(
    "--gateway-provider-id",
    "gateway_provider_ids",
    multiple=True,
    help="OpenClaw provider id to route through Copium (repeatable; default: openai-codex)",
)
@click.option(
    "--python-path",
    default=None,
...
@click.option(
    "--no-auto-start",
    is_flag=True,
    help="Disable plugin auto-start of local copium proxy",
)
@click.option(
    "--no-restart",
    is_flag=True,
...
def openclaw(
    ...
    """Install and configure Copium OpenClaw plugin in one command.
    ...
    Example:
      ...
      copium wrap openclaw --plugin-path C:\\git\\copium\\plugins\\openclaw
    """
    if prepare_only:
    ...
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║           COPIUM WRAP: OPENCLAW             ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    existing_entry = _read_openclaw_config_value(openclaw_bin, "plugins.entries.copium")
    entry = _build_openclaw_plugin_entry(
        existing_entry=existing_entry,
        proxy_port=proxy_port,
        startup_timeout_ms=startup_timeout_ms,
    ...
    _set_openclaw_context_engine_slot(openclaw_bin, "copium")
    _run_checked(
    ...
    inspect_result = _run_checked(
        [openclaw_bin, "plugins", "inspect", "copium"],
        action="openclaw plugins inspect copium",
    )
    if verbose and inspect_result.stdout.strip():
        click.echo(inspect_result.stdout.strip())

    click.echo()
    click.echo("✓ OpenClaw is configured to use Copium context compression.")
    click.echo("  Plugin: copium")
    click.echo("  Slot:   plugins.slots.contextEngine = copium")
    click.echo()
...
def unwrap_openclaw(
    ...
    """Disable the Copium OpenClaw plugin and restore the legacy engine slot."""
    if prepare_only:
        click.echo(
            json.dumps(
    ...
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║          COPIUM UNWRAP: OPENCLAW            ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    click.echo("  Disabling Copium plugin and removing engine mapping...")
    ...
    click.echo()
    click.echo("✓ OpenClaw Copium wrap removed.")
    click.echo("  Plugin: copium (installed, disabled)")
...
def unwrap_codex(port: int, no_stop_proxy: bool) -> None:
    """Undo ``copium wrap codex`` edits to the active Codex config file.

    Behaviour:

    * If a pre-wrap backup (``config.toml.copium-backup``) exists, the
      original file is restored byte-for-byte and the backup is removed.
    * Otherwise, if the config file still contains the Copium-managed
      block, that block is stripped out and the rest of the file is
      preserved.
    * If the config only ever contained Copium-written content, the file
      is removed entirely so Codex falls back to its defaults.
    * If neither a backup nor a Copium block is present, this is a safe
      no-op (the user either never wrapped that config, or already unwrapped
      it). When ``CODEX_HOME`` is unset, print a warning hint because Copium
      may be looking at the default config while Codex was wrapped with a
      custom home.
    """
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║           COPIUM UNWRAP: CODEX              ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    if status == "restored":
        ...
        click.echo(f"  Removed Copium block from {config_file}; other content preserved.")
    elif status == "removed":
        click.echo(f"  Removed {config_file} (contained only Copium-written config).")
    else:
        if not os.environ.get("CODEX_HOME"):
            click.echo(
                "  Warning: found no Copium wrap markers in the default Codex config. "
                "If you wrapped Codex with CODEX_HOME, rerun unwrap with the same "
                "environment variable, e.g. CODEX_HOME=/path/to/codex-home "
                "copium unwrap codex."
            )
        click.echo(f"  Nothing to undo: {config_file} has no Copium wrap markers.")

    # Serena is written as its own [mcp_servers.serena] table with Copium
    ...
    if codex_registrar.detect():
        serena_status = _remove_copium_installed_serena_mcp(codex_registrar)
        if serena_status == "removed":
            click.echo("  Removed Copium-installed Serena MCP server from Codex.")
        elif serena_status == "failed":
            click.echo("  Serena MCP server matched Copium ledger but could not be removed.")

    if status in {"restored", "cleaned", "removed"}:
        # Hand the threads back to the native-provider menu so the full history
        # stays visible once Codex no longer routes through Copium. Best-effort.
