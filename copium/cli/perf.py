...
def perf(hours: float, raw: bool, output_format: str) -> None:
    ...
    \b
    Reads logs from ~/.copium/logs/proxy.log and shows:
    ...
    \b
    Examples:
        copium perf                      Analyze last 7 days
        copium perf --hours 24           Analyze last 24 hours
        copium perf --raw                Show raw parsed records
        copium perf --format json        Aggregated report as JSON
        copium perf --format csv --hours 24 > last-24h.csv
        copium perf --format json --raw  Raw records as a JSON array
    """
    ...
    if raw:
        ...
        if not report.perf_records:
            click.echo("No PERF records found. Run the proxy first: copium proxy")
    else:
