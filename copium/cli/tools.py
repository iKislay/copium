...
Exposes:
    ...
    copium sg …           ->  ast-grep (from the ast-grep-cli PyPI wheel)
    copium diff A B …     ->  difftastic
    copium loc [PATH] …   ->  scc
    copium tools install  ->  pre-fetch all bundled binaries
    copium tools doctor   ->  print a status table
    copium tools list     ->  show the registry
...
tool without any Copium-specific protocol.
"""
...
def _exec_tool(tool: str, argv: Sequence[str]) -> None:
    try:
        ...
        click.secho(
            f"error: {e}\nHint: run `copium tools install` on a networked machine, "
    ...
    # the parent `copium` process, not these thin passthroughs).
    cmd = [str(path), *argv]
    if not _is_windows():
    ...
    for name in selected:
        ...
        if binaries._is_pypi_tool(name):  # noqa: SLF001
            ...
            if on_path:
                ...
                click.secho(
                    f"{name}: not on PATH — `pip install copium-ai` should provide it",
                    fg="yellow",
                )
                exit_code = 1
            continue
