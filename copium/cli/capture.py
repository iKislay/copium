...
def capture_group() -> None:
    """Capture and compare network traffic for Copium investigations."""
...
@click.option(
    ...
    required=True,
    type=click.Path(exists=True, dir_okay=False, path_type=Path),
    help="JSONL capture from the Copium-proxied Claude Code lane.",
)
@click.option(
    "--output",
    "markdown_output",
...
def network_diff(
    ...
    """Compare direct and Copium MITM capture JSONL files."""
    ...
    diff = compare_captures(direct, copium, pair_by=pair_by)
