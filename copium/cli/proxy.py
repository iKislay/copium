...
from copium.providers.registry import resolve_api_overrides, resolve_api_targets
from copium.proxy.modes import PROXY_MODE_TOKEN, normalize_proxy_mode
...
@click.option(
    ...
    default="127.0.0.1",
    envvar="COPIUM_HOST",
    help="Host to bind to (default: 127.0.0.1, env: COPIUM_HOST)",
)
@click.option(
    "--port",
    "-p",
    ...
    type=int,
    envvar="COPIUM_PORT",
    help="Port to bind to (default: 8787, env: COPIUM_PORT)",
)
@click.option(
    "--workers",
    default=1,
    type=click.IntRange(min=1),
    envvar="COPIUM_WORKERS",
    help="Number of Uvicorn worker processes (default: 1, env: COPIUM_WORKERS)",
)
@click.option(
    "--limit-concurrency",
    default=1000,
    type=click.IntRange(min=1),
    envvar="COPIUM_LIMIT_CONCURRENCY",
    help=(
        "Maximum concurrent connections before Uvicorn returns 503 "
        "(default: 1000, env: COPIUM_LIMIT_CONCURRENCY)"
    ),
)
@click.option(
    "--max-connections",
    default=500,
    type=click.IntRange(min=1),
    envvar="COPIUM_MAX_CONNECTIONS",
    help="Maximum upstream HTTP connections (default: 500, env: COPIUM_MAX_CONNECTIONS)",
)
@click.option(
    "--max-keepalive",
    ...
    envvar="COPIUM_MAX_KEEPALIVE",
    help="Maximum upstream keep-alive connections (default: 100, env: COPIUM_MAX_KEEPALIVE)",
)
@click.option(
    "--mode",
    default=None,
    ...
    help=(
        ...
        "Legacy aliases (token_mode, token_savings, token_copium, cache_mode, "
        "cost_savings) are still accepted. Env: COPIUM_MODE."
    ),
)
@click.option(
    "--target-ratio",
...
@click.option(
    "--no-ccr-inject-tool",
    is_flag=True,
    envvar="COPIUM_NO_CCR_INJECT_TOOL",
    help=(
        "Don't inject the CCR copium_retrieve tool. Run compression-only — "
        "for streaming / non-MCP clients that can't resolve the retrieve tool "
        "and would otherwise error on it. Env: COPIUM_NO_CCR_INJECT_TOOL."
    ),
)
@click.option(
    "--no-ccr-marker",
    is_flag=True,
    envvar="COPIUM_NO_CCR_MARKER",
    help=("Don't add CCR retrieval markers to compressed content. Env: COPIUM_NO_CCR_MARKER."),
)
@click.option(
    "--no-ccr-proactive-expansion",
    is_flag=True,
    envvar="COPIUM_NO_CCR_PROACTIVE_EXPANSION",
    help=(
        "Disable proactive expansion of previously compressed content. "
        "Env: COPIUM_NO_CCR_PROACTIVE_EXPANSION."
    ),
)
@click.option(
    "--proxy-extension",
    "proxy_extension",
    multiple=True,
    envvar="COPIUM_PROXY_EXTENSIONS",
    help=(
        ...
        "every discovered extension. Env: COPIUM_PROXY_EXTENSIONS."
    ),
)
@click.option(
    "--no-subscription-tracking",
    is_flag=True,
    envvar="COPIUM_NO_SUBSCRIPTION_TRACKING",
    help=(
        "Disable the Anthropic Claude Code subscription usage poller "
        "(GET /api/oauth/usage). Env: COPIUM_NO_SUBSCRIPTION_TRACKING."
    ),
)
@click.option(
    ...
    default=None,
    envvar="COPIUM_SUBSCRIPTION_POLL_INTERVAL",
    help=(
        ...
        "Env: COPIUM_SUBSCRIPTION_POLL_INTERVAL."
    ),
)
@click.option(
    "--retry-max-attempts",
...
@click.option(
    ...
    default=None,
    envvar="COPIUM_CONNECT_TIMEOUT_SECONDS",
    help=(
        "Upstream connection timeout in seconds (1–300, default: 10). "
        "Env: COPIUM_CONNECT_TIMEOUT_SECONDS."
    ),
)
@click.option(
    ...
    type=int,
    default=None,
    envvar="COPIUM_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY",
    help=(
        ...
        "Env: COPIUM_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY."
    ),
)
@click.option(
    ...
    type=float,
    default=None,
    envvar="COPIUM_ANTHROPIC_PRE_UPSTREAM_ACQUIRE_TIMEOUT_SECONDS",
    help=(
        ...
        "Env: COPIUM_ANTHROPIC_PRE_UPSTREAM_ACQUIRE_TIMEOUT_SECONDS."
    ),
)
@click.option(
    ...
    type=float,
    default=None,
    envvar="COPIUM_ANTHROPIC_PRE_UPSTREAM_MEMORY_CONTEXT_TIMEOUT_SECONDS",
    help=(
        "Fail-open timeout for Anthropic memory-context lookup while the request "
        "still holds a pre-upstream slot. "
        "Default: 2.0 seconds. "
        "Env: COPIUM_ANTHROPIC_PRE_UPSTREAM_MEMORY_CONTEXT_TIMEOUT_SECONDS."
    ),
)
@click.option(
    "--log-file",
    default=None,
    envvar="COPIUM_LOG_FILE",
    help=(
        ...
        "Disabled in --stateless mode. Env: COPIUM_LOG_FILE."
    ),
)
@click.option(
    "--log-messages",
    is_flag=True,
    envvar="COPIUM_LOG_MESSAGES",
    help=(
...
@click.option(
    ...
    help=(
        ...
        "~/.copium/logs/codex_wire or workspace .copium/logs/codex_wire)."
    ),
)
@click.option(
    "--budget",
...
@click.option(
    ...
    help=(
        ...
        "Env: COPIUM_BUDGET_PERIOD."
    ),
)
# Code-aware compression (AST-based, requires `pip install copium-ai[code]`).
# Pair of flags so users can override the env-var default in either direction.
# We resolve COPIUM_CODE_AWARE_ENABLED in the body (not via Click's envvar=),
...
@click.option(
    ...
    help=(
        ...
        "tree-sitter dependency: pip install copium-ai[code]. "
        "Default: disabled. Env: COPIUM_CODE_AWARE_ENABLED=1 to enable."
    ),
)
@click.option(
    "--disable-kompress",
    is_flag=True,
    envvar="COPIUM_DISABLE_KOMPRESS",
    help=(
...
@click.option(
    ...
    help=(
        ...
        "x-copium-user-id and/or x-copium-project-id / x-copium-cwd request headers."
    ),
)
@click.option(
    "--memory-db-path",
    default="",
    envvar="COPIUM_MEMORY_DB_PATH",
    help=(
        "Path to the legacy single-file memory DB (used in --memory-storage=global, "
        "and as the seed for the project-mode storage root). "
        "Default: {cwd}/.copium/memory.db. Env: COPIUM_MEMORY_DB_PATH."
    ),
)
@click.option(
    "--memory-storage",
    ...
    help=(
        ...
        "no cross-project bleed. user: one DB per x-copium-user-id. global: a single "
...
@click.option(
    "--memory-project-root",
    default="",
    envvar="COPIUM_MEMORY_PROJECT_ROOT",
    help=(
        ...
        "workspace. Takes effect after the x-copium-project-id and x-copium-cwd "
        "headers. Env: COPIUM_MEMORY_PROJECT_ROOT."
    ),
)
@click.option(
    "--no-memory-tools",
    is_flag=True,
    envvar="COPIUM_NO_MEMORY_TOOLS",
    help=(
        "Disable automatic injection of memory_save/memory_search tools into requests. "
        "Env: COPIUM_NO_MEMORY_TOOLS."
    ),
)
@click.option(
    "--no-memory-context",
    is_flag=True,
    envvar="COPIUM_NO_MEMORY_CONTEXT",
    help=(
        "Disable automatic injection of relevant past memories into the system prompt. "
        "Env: COPIUM_NO_MEMORY_CONTEXT."
    ),
)
@click.option(
    "--memory-top-k",
...
@click.option(
    ...
    help=(
        ...
        "(default: localhost, also reads COPIUM_QDRANT_HOST)"
    ),
)
@click.option(
    "--memory-qdrant-port",
    ...
    default=None,
    help=(
        "Qdrant port for the qdrant-neo4j backend (default: 6333, also reads COPIUM_QDRANT_PORT)"
    ),
)
@click.option(
    "--memory-qdrant-api-key",
    default=None,
    help=("API key for hosted Qdrant (e.g. Qdrant Cloud). Also reads COPIUM_QDRANT_API_KEY."),
)
...
@click.option(
    ...
    help=(
        ...
        "cost of slower learning. Default: 5. (env: COPIUM_MIN_EVIDENCE)"
    ),
)
...
@click.option(
    ...
    help="Cloud region for Bedrock/Vertex/etc (default: us-west-2). Env: COPIUM_REGION.",
)
@click.option(
    "--bedrock-region",
    default=None,
...
@click.option(
    ...
    "(env: COPIUM_EMBEDDING_SERVER=true)",
)
@click.option(
    "--embedding-server-socket",
    default=None,
    help="Unix socket path for the embedding server sidecar. "
    "Default: /tmp/copium-embed-{port}.sock. "
    "(env: COPIUM_EMBEDDING_SERVER_SOCKET)",
)
@click.pass_context
def proxy(
    ctx: click.Context,
    ...
    if intercept_tool_results:
        from copium.binaries import ensure_tools
        ...
        if missing:
            ...
            click.secho(
                ...
                f"be installed: {missing}. Run `copium tools doctor` to diagnose, "
                "or omit the flag to start the proxy without interceptors.",
                fg="red",
                err=True,
            )
            sys.exit(1)
        os.environ["COPIUM_INTERCEPT_ENABLED"] = "1"
    ...
    effective_anyllm_provider = os.environ.get("COPIUM_ANYLLM_PROVIDER") or anyllm_provider

    # Resolve mode: CLI flag > env var > default
    effective_mode: str = normalize_proxy_mode(
        mode or os.environ.get("COPIUM_MODE") or PROXY_MODE_TOKEN
    )

    # Stateless mode: CLI flag or env var
    is_stateless = stateless or os.environ.get("COPIUM_STATELESS", "").lower() in (
        "true",
        "1",
        "yes",
        "on",
    ...
    if codex_wire_debug or codex_wire_debug_dir:
        ...
        os.environ["COPIUM_CODEX_WIRE_DEBUG_DIR"] = codex_wire_debug_dir or str(
            _paths.codex_wire_debug_dir()
        )

    # Stateless mode: suppress TOIN filesystem persistence
    if is_stateless:
        os.environ["COPIUM_TOIN_BACKEND"] = "none"

    # License key for managed/enterprise deployments (optional)
    license_key = os.environ.get("COPIUM_LICENSE_KEY")
    ...
    # COPIUM_QDRANT_* env vars. Explicit CLI values win over env.
    ...
    config = ProxyConfig(
        ...
        compress_user_messages=_get_env_bool("COPIUM_COMPRESS_USER_MESSAGES", False),
        min_tokens_to_crush=_get_env_int_optional("COPIUM_MIN_TOKENS") or 500,
        max_items_after_crush=_get_env_int_optional("COPIUM_MAX_ITEMS") or 50,
        exclude_tools=_parse_exclude_tools(None) or None,
        tool_profiles=_parse_tool_profiles([]) or None,
        smart_crusher_with_compaction=_get_env_bool_optional("COPIUM_SMART_CRUSHER_COMPACTION"),
        savings_profile=os.environ.get("COPIUM_SAVINGS_PROFILE") or None,
        target_ratio=target_ratio,
        compress_system_messages=_get_env_bool_optional("COPIUM_COMPRESS_SYSTEM_MESSAGES"),
        protect_recent=_get_env_int_optional("COPIUM_PROTECT_RECENT"),
        protect_analysis_context=_get_env_bool_optional("COPIUM_PROTECT_ANALYSIS_CONTEXT"),
        ...
        # `--proxy-extension a,b --proxy-extension c` and `COPIUM_PROXY_EXTENSIONS=a,b,c`
        ...
        or os.environ.get("COPIUM_LOG_MESSAGES", "").lower() in ("true", "1", "yes", "on"),
        ...
        # 2. Otherwise read COPIUM_CODE_AWARE_ENABLED (truthy = on).
        ...
        code_aware_enabled=(
            ...
            else os.environ.get("COPIUM_CODE_AWARE_ENABLED", "").strip().lower()
            in ("true", "1", "yes", "on")
        ),
        ...
        **qdrant_overrides,
        ...
        traffic_learning_agent_type=os.environ.get("COPIUM_AGENT_TYPE", "unknown"),
        ...
        # path. ``None`` -> CopiumProxy computes ``max(2, min(8,
...
"""
        ...
        # LiteLLM backend
        from copium.backends.litellm import get_provider_config
...
IMPORTANT for {provider_config.display_name} users:
  ...
  2. Set a dummy Anthropic key: ANTHROPIC_API_KEY="sk-ant-dummy"
     (Copium ignores this - it uses your {provider_config.display_name} credentials)
...
Memory (Multi-Provider):
  ...
  - Storage mode: {config.memory_storage_mode} (per-project DB by default — set x-copium-project-id / x-copium-cwd to override)
...
"""
    ...
    if is_telemetry_enabled():
        telemetry_line = (
            ...
            "                Disable: COPIUM_TELEMETRY=off or copium proxy --no-telemetry"
        )
    else:
        telemetry_line = "  Telemetry:    DISABLED"

    # Discover proxy extensions (third-party packages registered via the
    # `copium.proxy_extension` entry-point group). Surfaced in the banner
    ...
    try:
        from copium.proxy.extensions import discover as _discover_extensions
    ...
    if not _ext_available:
        ...
        extensions_line = (
            ...
            f"(opt-in: --proxy-extension <name> or COPIUM_PROXY_EXTENSIONS=<n>)"
        )
    ...
    from copium.proxy.server import _get_code_aware_banner_status
    ...
    _stable_turn = int(os.environ.get("COPIUM_COMPRESSION_STABLE_AFTER_TURN", "0"))
    _stale_turns = int(os.environ.get("COPIUM_STALE_READ_COMPRESS_AFTER_TURNS", "0"))
    _embed_socket = os.environ.get("COPIUM_EMBEDDING_SERVER_SOCKET") or (
        embedding_server and (embedding_server_socket or f"/tmp/copium-embed-{port}.sock")
    )
    _tuning_lines: list[str] = []
    if _stable_turn:
        _tuning_lines.append(
            f"  Prefix stability:        conservative for first {_stable_turn} turns"
            f"  (COPIUM_COMPRESSION_STABLE_AFTER_TURN={_stable_turn})"
        )
    if _stale_turns:
        _tuning_lines.append(
            f"  Stale read compression:  reads older than {_stale_turns} turns eligible"
            f"  (COPIUM_STALE_READ_COMPRESS_AFTER_TURNS={_stale_turns})"
        )
    if _embed_socket:
    ...
    if _tuning_lines:
        ...
        tuning_section = (
            "\nPerformance Tuning:  (all defaults — set COPIUM_COMPRESSION_STABLE_AFTER_TURN"
            " / COPIUM_STALE_READ_COMPRESS_AFTER_TURNS to tune)"
        )
...
""")
    ...
    try:
        from copium.update_check import format_update_notice
    ...
    if embedding_server:
        _embed_socket = embedding_server_socket or f"/tmp/copium-embed-{config.port}.sock"
        # Pass socket path to all worker processes via environment variable
        os.environ["COPIUM_EMBEDDING_SERVER_SOCKET"] = _embed_socket
        click.echo(f"  Embedding server: starting sidecar on {_embed_socket}...")

        import asyncio as _asyncio

        from copium.memory.adapters.watchdog import EmbeddingServerWatchdog
        ...
        try:
            ...
            os.environ.pop("COPIUM_EMBEDDING_SERVER_SOCKET", None)
    ...
    try:
        ...
        # a richer one above. Direct `python -m copium.proxy.server` keeps
