...
from copium.install.providers import apply_mutations, revert_mutations
...
from copium.install.state import delete_manifest, load_manifest, save_manifest
...
def _reject_task_lifecycle(manifest: DeploymentManifest, action: str) -> None:
    if manifest.supervisor_kind == SupervisorKind.TASK.value:
        raise click.ClickException(
            ...
            f"`copium install {action}` is not supported for task deployments."
        )
...
@click.option(
    ...
    show_default=True,
    help="Runtime used to execute Copium for service/task modes.",
)
@click.option(
    "--scope",
...
@click.option(
    "--image",
    default="ghcr.io/iKislay/copium:latest",
    show_default=True,
