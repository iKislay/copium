"""`copium update` — self-update across the supported install methods.

Detects how Copium was installed and runs the matching upgrade command, or
...
from copium.update_check import (
    PACKAGE_NAME,
...
class InstallMethod:
    """How Copium is installed and how (or whether) to upgrade it."""
...
def _is_source_checkout() -> bool:
    try:
        from copium._version import _source_root
...
def _in_docker() -> bool:
    try:
        ...
        return Path("/.dockerenv").exists() or bool(
            os.environ.get("COPIUM_IN_DOCKER", "").strip()
        )
    except Exception:
        return False
...
def _package_location() -> str | None:
    """Normalized base directory the copium-ai distribution is installed in."""
    try:
        from importlib.metadata import distribution
...
def _managed_env_guidance() -> str:
    ...
    return (
        "Copium is installed in an externally-managed system Python (PEP 668). "
...
def detect_install_method(extras: str | None = None) -> InstallMethod:
    ...
    if _in_docker():
        return InstallMethod(
            ...
            can_self_update=False,
            guidance=(
                "Running inside a container — pull a newer Copium image instead of self-updating."
            ),
        )
...
def update(check_only: bool, assume_yes: bool, allow_pre: bool, extras: str | None) -> None:
    """Update Copium to the latest release.
    ...
    click.echo("Checking PyPI for the latest Copium release...")
    ...
    if current:
        ...
        try:
            if Version(latest) <= Version(current):
                click.echo(f"Copium is up to date ({current}).")
                return
        except InvalidVersion:
            pass
        click.echo(f"Update available: {current} → {latest}")
    else:
        click.echo(f"Latest Copium release: {latest}")
    ...
    click.echo(f"Copium upgraded to {latest}.")
    click.echo("Restart any running `copium proxy` to pick up the new version.")
