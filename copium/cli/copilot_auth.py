...
from copium.copilot_auth import (
    DEFAULT_GITHUB_HOST,
    copium_copilot_auth_path,
    poll_copilot_device_authorization,
    read_copium_copilot_oauth_token,
    save_copium_copilot_oauth_token,
...
def copilot_auth() -> None:
    """Manage Copium's GitHub Copilot OAuth token."""
...
def login(domain: str) -> None:
    ...
    path = save_copium_copilot_oauth_token(token, domain=domain)
...
def status() -> None:
    """Show whether Copium has a saved Copilot OAuth token."""

    token = read_copium_copilot_oauth_token()
