...
def evals() -> None:
    ...
    \b
    Examples:
        copium evals memory       Run LoCoMo memory evaluation
        copium evals memory-v2    Run V2 evaluation with LLM-controlled tools
    """
    pass
...
def memory_eval_v2(
    ...
    \b
    Examples:
        copium evals memory-v2 -n 3
        copium evals memory-v2 --answer-model gpt-4o --save-model gpt-4o-mini
    """
    _run_memory_eval_v2(
        n_conversations=n_conversations,
        categories=categories,
...
def memory_eval_compat(
    ...
    """Deprecated: Use 'copium evals memory' instead."""
    click.echo("Note: 'memory-eval' is deprecated. Use 'copium evals memory'", err=True)
    _run_memory_eval(
        n_conversations=n_conversations,
        categories=categories,
        include_adversarial=include_adversarial,
...
def memory_eval_v2_compat(
    ...
    """Deprecated: Use 'copium evals memory-v2' instead."""
    click.echo("Note: 'memory-eval-v2' is deprecated. Use 'copium evals memory-v2'", err=True)
    _run_memory_eval_v2(
        n_conversations=n_conversations,
        categories=categories,
        include_adversarial=include_adversarial,
...
def _run_memory_eval(
    *,
    ...
    try:
        from copium.evals.memory import (
            LoCoMoEvaluator,
            MemoryEvalConfig,
        ...
        from copium.memory import MemoryConfig
    except ImportError as e:
        click.echo("Error: Memory eval dependencies not installed.")
        click.echo("Run: pip install copium[memory,evals]")
...
@click.option(
    ...
    help="Directory of JSONL recordings written via COPIUM_PROBE_RECORD_DIR.",
)
@click.option(
    "--json-output",
...
def probes(recordings_dir: Path, json_output: Path | None) -> None:
    ...
    COPIUM_PROBE_RECORD_DIR set. Recordings contain full conversation
    content in plaintext and stay on this machine.
    """
    import json as json_module

    from copium.evals.session_probes import render_report, run_probes
...
def adversarial(json_output: Path | None) -> None:
    ...
    from copium.evals.adversarial_grid import render_report, run_adversarial_grid
