...
def memory(ctx: click.Context) -> None:
    ...
    \b
    Examples:
        copium memory list                     List all stored memories
        copium memory list --limit 10          List the 10 most recent memories
        copium memory list --scope USER        List only USER-level memories
        copium memory list --since 7d          List memories from the last 7 days
        copium memory show <id>                Show full details of a memory
        copium memory stats                    Show memory statistics
        copium memory edit <id> --content ...  Edit a memory's content
        copium memory delete <id>              Delete a memory
        copium memory prune --older-than 30d   Delete memories older than 30 days
        copium memory purge --confirm          Delete ALL memories
        copium memory export --output file.json  Export all memories to JSON
        copium memory import file.json         Import memories from JSON
    """
    pass
...
def list_memories(
    ...
    \b
    Examples:
        copium memory list                  List most recent memories
        copium memory list --limit 10       Show only 10 memories
        copium memory list --scope USER     Show only USER-level memories
        copium memory list --since 7d       Show memories from the last 7 days
        copium memory list --search "api"   Search for memories containing "api"
    """
...
def show_memory(
    ...
    \b
    Examples:
        copium memory show abc123            Show memory details
        copium memory show abc123 --json     Output as raw JSON
    """
    store = get_store(db_path)
    ...
    try:
        ...
        if old_count > 0:
            console.print(
                ...
                f"Consider running: copium memory prune --older-than 30d[/dim]"
            )
...
def edit_memory(
    ...
    \b
    Examples:
        copium memory edit abc123 --content "Updated content"
        copium memory edit abc123 --importance 0.8
        copium memory edit abc123 -c "New content" -i 0.9
    """
...
def delete_memories(
    ...
    \b
    Examples:
        copium memory delete abc123           Delete one memory
        copium memory delete abc123 def456    Delete multiple memories
        copium memory delete abc123 --force   Skip confirmation
    """
    store = get_store(db_path)
...
def prune_memories(
    ...
    Examples:
        copium memory prune --older-than 30d              Delete memories older than 30 days
        copium memory prune --scope TURN                  Delete all TURN-level memories
        copium memory prune --low-importance 0.3          Delete low-importance memories
        copium memory prune --older-than 7d --scope TURN  Combine filters
        copium memory prune --older-than 30d --dry-run    Preview what would be deleted
    """
...
def export_memories(ctx: click.Context, db_path: str, output: str | None) -> None:
    ...
    \b
    Examples:
        copium memory export                       Output to stdout
        copium memory export --output backup.json  Save to file
        copium memory export -o backup.json        Save to file (short form)
    """
    store = get_store(db_path)
...
def import_memories(ctx: click.Context, db_path: str, file: str, force: bool) -> None:
    ...
    \b
    Examples:
        copium memory import backup.json          Import from file
        copium memory import backup.json --force  Skip confirmation
    """
    file_path = Path(file)
