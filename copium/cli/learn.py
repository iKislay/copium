"""CLI commands for Copium Learn — offline failure learning."""
...
External plugins register via 'copium.learn_plugin' entry point.
...
def learn(
    ...
    be installed via pip (entry point: copium.learn_plugin).

    \b
    Examples:
        copium learn                        # Auto-detect agent & model
        copium learn --apply                # Write recommendations
        copium learn --model gpt-4o         # Use GPT-4o for analysis
        copium learn --all                  # Analyze all projects
        copium learn --agent codex --all    # Analyze all Codex sessions
    """
    import os
    ...
    for agent_name, plugin in agent_configs:
        ...
        if analyze_all:
            ...
            if not targets and len(agent_configs) == 1:
                ...
                click.echo("Try: copium learn --all  or  copium learn --project <path>")
    ...
    for proj in targets:
        ...
        if apply:
            ...
            click.echo(
                ...
                "COPIUM_OUTPUT_SHAPER=1 and COPIUM_VERBOSITY_LEVEL is unset."
            )
        else:
