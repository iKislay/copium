"""Reconcile Codex thread provider tags across the Copium proxy boundary.

Codex stamps every thread with the ``model_provider`` it ran under and filters
its history/projects menu by the active provider set.  When Copium rewrites
Codex's config to route through the custom ``copium`` provider (see
:mod:`copium.providers.codex.install`), threads created through Copium are
tagged ``copium`` while native threads keep ``openai`` -- so the two sets never
appear in the same menu, and connecting Copium appears to "lose" history.

To keep the menu whole we retag threads to match whichever provider is active:
``openai -> copium`` when Copium is enabled, ``copium -> openai`` when it is
...
def retag_to_copium(codex_home: Path) -> None:
    """Pull existing native threads into the copium-provider menu (on enable)."""
    retag_thread_providers(codex_home, frm=NATIVE_PROVIDER, to=COPIUM_PROVIDER)


def retag_to_native(codex_home: Path) -> None:
    """Hand threads back to the native-provider menu (on revert)."""
    retag_thread_providers(codex_home, frm=COPIUM_PROVIDER, to=NATIVE_PROVIDER)
