"""Anthropic provider implementation for Copium SDK.
...
def _load_custom_model_config() -> dict[str, Any]:
    ...
    Checks (in order):
    1. COPIUM_MODEL_LIMITS environment variable (JSON string or file path)
    2. ~/.copium/models.json config file
    ...
    env_config = os.environ.get("COPIUM_MODEL_LIMITS", "")
    if env_config:
        try:
            ...
            logger.debug(f"Loaded custom model config from COPIUM_MODEL_LIMITS: {loaded}")
        except (json.JSONDecodeError, OSError) as e:
            logger.warning(f"Failed to load COPIUM_MODEL_LIMITS: {e}")
...
class AnthropicTokenCounter(TokenCounter):
    ...
    def __init__(self, model: str, client: Any = None, warn: bool = True):
        ...
        try:
            from copium.tokenizers.tiktoken_counter import (
                TiktokenLoadError,
                load_encoding,
            )
...
class AnthropicProvider(Provider):
    ...
    Custom Model Configuration:
        ...
        1. Environment variable (JSON string):
           export COPIUM_MODEL_LIMITS='{"context_limits": {"my-model": 200000}}'

        2. Environment variable (file path):
           export COPIUM_MODEL_LIMITS=/path/to/models.json

        3. Config file (~/.copium/models.json):
           {
             "anthropic": {
    ...
    def get_context_limit(self, model: str) -> int:
        ...
        2. COPIUM_MODEL_LIMITS environment variable
        3. ~/.copium/models.json config file
    ...
    def _warn_unknown_model(self, model: str, limit: int, reason: str) -> None:
        ...
        if model not in _UNKNOWN_MODEL_WARNINGS:
            ...
            logger.warning(
                ...
                f"To configure explicitly, set COPIUM_MODEL_LIMITS env var or "
                f"add to ~/.copium/models.json"
            )
