...
from copium.proxy.handlers.openai import _resolve_codex_routing_headers

logger = logging.getLogger("copium.proxy.routes")
...
def register_provider_routes(app: FastAPI, proxy: Any) -> None:
    ...
    # copium/proxy/handlers/bedrock.py for the SigV4 caveat.
    ...
    async def passthrough(request: Request, path: str):
        custom_base = request.headers.get("x-copium-base-url")
        if custom_base:
