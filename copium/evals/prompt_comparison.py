...
This module provides tools to verify that Copium preserves the semantic
meaning of prompts by comparing prompts before and after processing.

This is CRITICAL for proving that Copium doesn't change the meaning of prompts
when they go through the proxy.

Usage:
    from copium.evals.prompt_comparison import compare_prompts, PromptComparer

    # Simple comparison
    result = compare_prompts(
        original_prompt="What is the capital of France?",
        copium_modified_prompt="What is the capital of France?",
    )
...
class PromptComparisonResult:
    ...
    Attributes:
        original: The original prompt before Copium processing.
        copium_modified: The prompt after Copium processing.
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "copium_modified": self.copium_modified,
    ...
    def from_dict(cls, data: dict[str, Any]) -> PromptComparisonResult:
        ...
        return cls(
            ...
            copium_modified=data["copium_modified"],
...
def compare_prompts(
    ...
    Args:
        original_prompt: The original prompt before Copium processing.
        copium_modified_prompt: The prompt after Copium processing.
    ...
    Example:
        result = compare_prompts(
            ...
            copium_modified_prompt="Explain quantum computing in simple terms.",
        )
    ...
    judge_prompt = SEMANTIC_EQUIVALENCE_JUDGE_PROMPT.format(
        ...
        modified_prompt=copium_modified_prompt,
    )
    ...
    return PromptComparisonResult(
        ...
        copium_modified=copium_modified_prompt,
...
class PromptComparer:
    ...
    def compare(
        ...
        result = compare_prompts(
            original_prompt=original_prompt,
            copium_modified_prompt=copium_modified_prompt,
            judge_model=self.judge_model,
    ...
    def compare_and_log(
        ...
        Returns:
            PromptComparisonResult with the comparison outcome.
        """
        result = self.compare(original_prompt, copium_modified_prompt, metadata)
...
def compare_messages(
    ...
    Args:
        original_messages: Original message list before Copium.
        modified_messages: Modified message list after Copium.
        judge_model: The OpenAI model to use as judge.
    ...
    result = compare_prompts(
        ...
        copium_modified_prompt=modified_str,
        judge_model=judge_model,
        api_key=api_key,
        metadata={
...
def batch_compare_prompts(
    ...
    def compare_pair(
        ...
        result = compare_prompts(
            ...
            copium_modified_prompt=modified,
            judge_model=judge_model,
            api_key=api_key,
...
def verify_copium_preservation(
    original_messages: list[dict[str, Any]],
    copium_messages: list[dict[str, Any]],
    judge_model: str = "gpt-4o",
    api_key: str | None = None,
    fail_on_difference: bool = False,
) -> PromptComparisonResult:
    """Verify that Copium preserves the semantic meaning of a request.

    This is the main entry point for verifying Copium's accuracy preservation.
    It compares the original messages with those modified by Copium and
    raises an error if differences are found (when fail_on_difference=True).

    Args:
        original_messages: Messages before Copium processing.
        copium_messages: Messages after Copium processing.
        judge_model: The model to use for judging.
        api_key: OpenAI API key.
    ...
    Example:
        # Capture messages before and after Copium
        original = [{"role": "user", "content": "Hello, how are you?"}]
        after_copium = [{"role": "user", "content": "Hello, how are you?"}]

        result = verify_copium_preservation(
            original, after_copium, fail_on_difference=True
        )
    """
    result = compare_messages(
        original_messages=original_messages,
        modified_messages=copium_messages,
        judge_model=judge_model,
        api_key=api_key,
    )
    ...
    if fail_on_difference and not result.are_equivalent:
        raise ValueError(
            f"Copium modified prompt semantics! "
