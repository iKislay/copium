...
batch API requests through the Copium proxy.
...
CLI:
    python -m copium.evals.batch_compression_eval --provider anthropic --samples 10
"""
...
Context:
The Copium SDK is a context optimization layer for LLM applications. It was created
...
Question: Which repository has the most stars? Answer with just the repository name.""",
            ...
            context_facts=["copium", "1250", "llm-cache", "890", "prompt-optimizer", "2100"],
        ),
        BatchTestCase(
            id="json_003",
            category=TestCategory.JSON_EXTRACTION,
...
def evaluate_token_counting_accuracy(
    ...
    This verifies that Copium's token counting matches the provider's
