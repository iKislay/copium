...
baseline vs Copium accuracy + token savings.
"""
...
class BenchmarkRunResult:
    ...
    # For lm-eval (standard benchmarks): baseline vs copium scores
    baseline_score: float | None = None
    copium_score: float | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.copium_score is not None:
            d["copium_score"] = round(self.copium_score, 4)
...
class SuiteResult:
    ...
    def standard_benchmarks(self) -> list[BenchmarkRunResult]:
        """Benchmarks with baseline vs copium comparison."""
    ...
    if standard:
        lines.extend(
            [
                ...
                "",
                "| Benchmark | Category | N | Baseline | Copium | Delta | Tokens Saved | Status |",
                "|-----------|----------|---|----------|----------|-------|--------------|--------|",
            ]
        )
        for b in standard:
            ...
            copium_str = f"{b.copium_score:.3f}" if b.copium_score is not None else "N/A"
            ...
            lines.append(
                ...
                f"{copium_str} | {delta_str} | {comp_str} | {status} |"
            )
        lines.append("")
...
def generate_html(result: SuiteResult) -> str:
    ...
    for b in standard:
        ...
        standard_rows += f"""        <tr>
            ...
            <td>{baseline_str}</td><td>{copium_str}</td><td>{delta_str}</td>
            <td>{comp_str}</td>
...
<html>
<head>
    <title>Copium Accuracy Report Card</title>
    <style>
        body {{
...
<body>
    <h1>Copium Accuracy Report Card</h1>
    <div class="meta">
    ...
    {"<table><tr><th>Benchmark</th><th>Category</th><th>N</th><th>Baseline</th><th>Copium</th><th>Delta</th><th>Saved</th><th>Status</th></tr>" if standard else ""}
    {standard_rows}
    ...
    <footer>Generated by <strong>Copium Evaluation Framework</strong> | <code>pip install copium-ai[evals]</code></footer>
</body>
</html>"""
