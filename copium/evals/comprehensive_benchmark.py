...
This module runs industry-standard benchmarks to prove Copium preserves accuracy.

Approach:
1. Run benchmarks directly against LLM provider (baseline)
2. Run same benchmarks through Copium proxy
...
Usage:
    ...
    python -m copium.evals.comprehensive_benchmark --quick

    # Full benchmark
    python -m copium.evals.comprehensive_benchmark --tasks mmlu,hellaswag

    # Compare with baseline
    python -m copium.evals.comprehensive_benchmark --compare
"""
...
class ComparisonResult:
    """Comparison between baseline and Copium results."""
    ...
    def delta(self) -> float:
        return self.copium_score - self.baseline_score

    @property
    def accuracy_preserved(self) -> bool:
        """True if Copium score is within 2% of baseline."""
        return self.copium_score >= self.baseline_score - 0.02

    @property
    def accuracy_improved(self) -> bool:
        """True if Copium score exceeds baseline."""
        return self.copium_score > self.baseline_score
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "copium": round(self.copium_score, 4),
...
class BenchmarkSuiteResult:
    ...
    copium_results: list[BenchmarkResult] = field(default_factory=list)
    ...
    def avg_delta(self) -> float:
        """Average score delta (positive = Copium better)."""
        if not self.comparisons:
            return 0.0
    ...
    def summary(self) -> dict[str, Any]:
        return {
            ...
            "copium": [r.to_dict() for r in self.copium_results],
    ...
    def print_summary(self) -> None:
        ...
        print("\n" + "=" * 70)
        print("COPIUM COMPREHENSIVE BENCHMARK RESULTS")
        print("=" * 70)
        ...
        if self.comparisons:
            print(f"{'Task':<20} {'Baseline':>10} {'Copium':>10} {'Delta':>10} {'Status':>10}")
            print("-" * 70)
            for c in self.comparisons:
                status = "PASS" if c.accuracy_preserved else "FAIL"
                delta_str = f"{c.delta:+.4f}"
                print(
                    f"{c.task:<20} {c.baseline_score:>10.4f} {c.copium_score:>10.4f} {delta_str:>10} {status:>10}"
                )
            print("-" * 70)
            ...
            print("Copium results only (no baseline comparison):")
            print(f"{'Task':<20} {'Score':>10} {'Metric':<15}")
            print("-" * 50)
            for r in self.copium_results:
...
def run_lm_eval(
    ...
    Args:
        ...
        base_url: Base URL for API (for Copium proxy)
...
def run_copium_benchmark(
    model: str = "gpt-4o-mini",
    tasks: list[str] | None = None,
    limit: int | None = None,
    copium_port: int = 8787,
) -> list[BenchmarkResult]:
    """Run benchmark through Copium proxy."""
    logger.info(f"Running Copium benchmark with {model} through proxy...")

    # lm_eval expects base_url to be the full path to chat/completions
    base_url = f"http://localhost:{copium_port}/v1/chat/completions"
...
def compare_results(
    ...
    copium: list[BenchmarkResult],
) -> list[ComparisonResult]:
    """Compare baseline and Copium results."""
    comparisons = []

    # Match results by task
    baseline_by_task = {r.task: r for r in baseline}
    copium_by_task = {r.task: r for r in copium}

    for task in baseline_by_task:
        if task in copium_by_task:
            b = baseline_by_task[task]
            h = copium_by_task[task]
            comparisons.append(
                ComparisonResult(
                    task=task,
                    metric=b.metric,
                    baseline_score=b.score,
                    copium_score=h.score,
                )
            )
...
def run_comprehensive_benchmark(
    ...
    Args:
        ...
        copium_port: Port where Copium proxy is running
    ...
    # This ensures a fair comparison - any speedup from Copium is real, not from warm cache
    if compare_baseline:
    ...
    # Run Copium benchmark (after baseline warms things up)
    suite.copium_results = run_copium_benchmark(
        model=model,
        tasks=tasks,
        limit=limit,
        copium_port=copium_port,
    )

    # Compare results
    if compare_baseline:
        suite.comparisons = compare_results(suite.baseline_results, suite.copium_results)

    suite.total_duration_seconds = time.time() - start_time
    return suite


def check_copium_proxy(port: int = 8787) -> bool:
    """Check if Copium proxy is running."""
    import socket
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run comprehensive LLM benchmarks to verify Copium accuracy"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--compare", "-c", action="store_true", help="Compare with baseline (run without Copium)"
    )
    parser.add_argument(
        "--port", "-p", type=int, default=8787, help="Copium proxy port (default: 8787)"
    )
    parser.add_argument("--output", "-o", default=None, help="Output file for results JSON")
    parser.add_argument(
        "--copium-only", action="store_true", help="Only run through Copium (skip baseline)"
    )
    ...
    if not check_copium_proxy(args.port):
        print(f"ERROR: Copium proxy not running on port {args.port}")
        print(f"Start it with: copium proxy --port {args.port}")
        sys.exit(1)

    # Run benchmarks
    compare = args.compare and not args.copium_only
