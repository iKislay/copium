...
if TYPE_CHECKING:
    from copium.evals.reports.report_card import SuiteResult
...
class BenchmarkSpec:
    ...
    pass_threshold: float = 0.98  # Copium score >= baseline - (1-threshold)
...
BENCHMARK_SUITE: list[BenchmarkSpec] = [
    ...
    # -----------------------------------------------------------------------
    # Standard benchmarks via lm-eval harness (through Copium proxy)
    BenchmarkSpec(
        name="GSM8K",
        category="reasoning",
        tier=1,
...
def _start_proxy(port: int) -> subprocess.Popen | None:
    """Start Copium proxy as a subprocess. Returns process handle."""
    logger.info(f"Starting Copium proxy on port {port}...")
    try:
        proc = subprocess.Popen(
            [sys.executable, "-m", "copium.proxy.server", "--port", str(port)],
        ...
        for _ in range(30):
            ...
            if _check_proxy(port):
                logger.info(f"Copium proxy ready on port {port}")
                return proc
...
class SuiteRunner:
    ...
    def _ensure_proxy(self) -> bool:
        """Ensure the Copium proxy is running (needed for lm-eval benchmarks)."""
        if _check_proxy(self.copium_port):
            return True
        if self.auto_start_proxy:
            self._proxy_proc = _start_proxy(self.copium_port)
    ...
    def _cleanup_proxy(self) -> None:
        ...
        if self._proxy_proc:
            logger.info("Stopping Copium proxy...")
    ...
    def _run_lm_eval_benchmark(
        ...
        from copium.evals.comprehensive_benchmark import (
            compare_results,
            run_baseline_benchmark,
            run_copium_benchmark,
        )
        ...
        # Run through Copium proxy
        print("    Running through Copium proxy...")
        copium_results = run_copium_benchmark(
            model=spec.model or self.model,
            tasks=tasks,
            limit=limit,
            copium_port=self.copium_port,
        )

        # Compare
        comparisons = compare_results(baseline_results, copium_results)
        ...
        if comparisons:
            ...
            copium_score = c.copium_score
            delta = c.delta
            passed = c.accuracy_preserved

        return {
            "baseline_score": baseline_score,
            "copium_score": copium_score,
            "delta": delta,
            "passed": passed,
            "n_samples": limit,
        }
    ...
    def _run_before_after_benchmark(
        ...
        from copium.evals.core import EvalMode
        from copium.evals.datasets import load_dataset_by_name, load_tool_output_samples
        ...
        proxy_url = (
            f"http://localhost:{self.copium_port}" if _check_proxy(self.copium_port) else None
        )
    ...
    def run(self) -> SuiteResult:
        ...
        from copium.evals.cost_tracker import CostTracker
        from copium.evals.reports.report_card import BenchmarkRunResult, SuiteResult
        ...
        if has_lm_eval:
            ...
            if not proxy_available:
                print("WARNING: Copium proxy not available. Skipping lm-eval benchmarks.")
                print("  Start with: copium proxy --port 8787")
                    ...
                    if spec.runner_type == "lm_eval":
                        ...
                        results.append(
                            BenchmarkRunResult(
                                ...
                                copium_score=raw.get("copium_score"),
                                delta=raw.get("delta"),
    ...
    def _print_summary(self, result: SuiteResult, tracker: Any) -> None:
        ...
        from copium.evals.reports.report_card import generate_markdown
