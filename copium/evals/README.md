...
| Benchmark | Category | N | Baseline | Copium | Delta |
|-----------|----------|---|----------|----------|-------|
...
```bash
pip install "copium-ai[all]"    # Everything including evals (recommended)
pip install "copium-ai[evals]"  # Evaluation framework only
```
...
python -m copium.evals suite --tier 1 -o eval_results/

# Extended suite (Tiers 1+2, ~$8, ~1 hr)
python -m copium.evals suite --tier 2 -o eval_results/

# CI mode — exit 1 on any regression
python -m copium.evals suite --tier 1 --ci

# List all available datasets
python -m copium.evals list
```

### Running with the Proxy (Recommended)

For the most accurate evaluation, run through the Copium proxy which provides
...
COPIUM_PROBE_RECORD_DIR=~/.copium/probe-recordings copium proxy start

# 2. Use your agent through the proxy as normal, then score retention:
copium evals probes --recordings ~/.copium/probe-recordings
```
...
When the Copium proxy is running, the "compressed" path goes through the full
...
on:
  pull_request:
    paths: ['copium/transforms/**', 'copium/evals/**']
  schedule:
    - cron: '0 6 * * 1'  # Weekly
    ...
    steps:
      ...
      - name: CCR Round-trip (zero cost)
        run: |
          ...
          "
      - name: Quick eval
        run: python -m copium.evals quick -n 8 --provider openai --model gpt-4o-mini
        env:
