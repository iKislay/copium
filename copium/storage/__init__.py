"""Storage modules for Copium SDK."""
...
def create_storage(store_url: str) -> Storage:
    ...
    the setuptools entry point copium.storage_backend with name=<scheme>.
    ...
    if store_url.startswith("sqlite://"):
        ...
        # Unknown scheme: try entry point copium.storage_backend[name=scheme]
        scheme = store_url.split("://", 1)[0].lower() if "://" in store_url else ""
        if scheme:
            try:
                from importlib.metadata import entry_points

                all_eps = entry_points(group="copium.storage_backend")
