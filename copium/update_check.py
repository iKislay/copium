"""Best-effort "is a newer Copium released?" check.
...
Opt out with ``COPIUM_UPDATE_CHECK=off``. Also skipped in ``--stateless``
...
def is_update_check_enabled() -> bool:
    ...
    Disabled by ``COPIUM_UPDATE_CHECK=off``, stateless mode
    (``COPIUM_STATELESS=true``/``1``/``yes``/``on``, matching the proxy's own
    parsing), or any CI environment (``CI`` set).
    """
    if _env_off("COPIUM_UPDATE_CHECK"):
        return False
    if _env_on("COPIUM_STATELESS"):
        return False
...
def installed_version() -> str | None:
    ...
    ``copium._version.get_version()`` — the latter computes a synthetic
...
def write_cache(latest_version: str, *, now: float | None = None) -> None:
    ...
    try:
        from copium.paths import ensure_workspace_dir
...
def fetch_latest_version(*, allow_pre: bool = False, timeout: float = 4.0) -> str | None:
    ...
    try:
        req = urllib.request.Request(
            _PYPI_JSON_URL,
            headers={"Accept": "application/json", "User-Agent": "copium-update-check"},
        )
...
def run_check(*, allow_pre: bool = False, now: float | None = None) -> str | None:
    ...
    Synchronous — used directly by ``copium update`` and indirectly by
...
def maybe_check_async() -> threading.Thread | None:
    ...
    try:
        ...
        thread = threading.Thread(target=_worker, name="copium-update-check", daemon=True)
        thread.start()
        return thread
    except Exception:
...
def format_update_notice() -> str | None:
    ...
    try:
        ...
        return f"Update available: Copium {latest} (you have {current}) - run: copium update"
    except Exception:
