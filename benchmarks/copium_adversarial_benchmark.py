"""
Copium ADVERSARIAL Benchmark: True Worst Cases
...
Focus on methodology differences and key findings.""",
        ...
        expected_behavior="Copium should have minimal compression - prose has no structural redundancy",
    )
...
Don't skip any file or change.""",
        ...
        expected_behavior="Copium should struggle - code changes are unique and can't be summarized",
    )
...
Describe what you observe about the data format and structure.""",
        ...
        expected_behavior="Copium CANNOT compress this - random data has no patterns",
    )
...
def create_small_data_scenario() -> AdversarialScenario:
    return AdversarialScenario(
        ...
        expected_behavior="Copium has no opportunity - data is already minimal",
    )
...
Pay attention to who said what and how opinions evolved.""",
        ...
        expected_behavior="Copium should preserve conversation flow - context matters",
    )
...
def run_adversarial_benchmark(api_key: str = None) -> dict:
    ...
    print("=" * 70)
    print("COPIUM ADVERSARIAL BENCHMARK")
    ...
    if COPIUM_AVAILABLE:
        db_path = os.path.join(tempfile.gettempdir(), "copium_adversarial.db")
    ...
    for scenario in scenarios:
        ...
        if copium_client:
            print("\n[2/2] COPIUM...")
            copium = run_scenario(copium_client, scenario, "copium")
            print(f"   Input tokens: {copium.input_tokens:,}")
            print(f"   Cost: ${copium.cost_usd:.4f}")
            results.append(copium)

            if baseline.input_tokens > 0:
                change = (copium.input_tokens - baseline.input_tokens) / baseline.input_tokens
                print(f"\n   📊 Token change: {change:+.1%}")
                if change > 0:
                    print("   ⚠️  COPIUM INCREASED TOKENS (overhead > savings)")
                elif change > -0.1:
    ...
    print(f"\n{'Scenario':<30} {'Baseline':>12} {'Copium':>12} {'Change':>12}")
    print("-" * 66)
