...
Strategies:
  1. Baseline — no Copium, no markers
  2. Copium compression — full pipeline, CompressionCache keeps bytes stable
  3. Copium + prefix freeze — pipeline skips frozen (already-cached) messages
  4. Copium + explicit markers — pipeline + 4 cache_control breakpoints
...
CACHE_MODE_EXPLICIT = "explicit_4"  # Copium's 4 strategic breakpoints
...
def main():
    ...
    # Strategy 3: Copium explicit markers (4 strategic breakpoints)
    if "markers" in strategies_to_run:
        step += 1
        print(f"[{step}/{num_strategies}] Copium explicit (4 strategic breakpoints)...")
        r = run_strategy(
            "Copium 4x",
            api_key,
            model,
