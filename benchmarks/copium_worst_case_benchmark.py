"""
Copium Worst-Case Benchmark: Where Compression Hurts

This benchmark tests scenarios where Copium's statistical compression
may NOT be beneficial - to understand the limits of the approach.

Worst cases for Copium:
...
def generate_unique_support_tickets(num_tickets: int = 50) -> dict:
    ...
    This is hard for Copium because:
...
class WorstCaseScenario:
    """A scenario where Copium might struggle."""
...
def run_worst_case_benchmark(api_key: str = None) -> dict:
    ...
    print("=" * 70)
    print("COPIUM WORST-CASE BENCHMARK")
    ...
    if COPIUM_AVAILABLE:
        db_path = os.path.join(tempfile.gettempdir(), "copium_worst_case.db")
        ...
        print("WARNING: Copium not available, running baseline only")
        copium_client = None
    ...
    for scenario in scenarios:
        ...
        if copium_client:
            print("\n[2/2] Running COPIUM...")
            copium_result = run_scenario(copium_client, scenario, "copium")
            print(f"   Input tokens: {copium_result.input_tokens:,}")
            print(f"   Output tokens: {copium_result.output_tokens:,}")
            print(f"   Cost: ${copium_result.cost_usd:.4f}")

            avg_copium_score = (
                sum(v["score"] for v in copium_result.validation_scores.values())
                / len(copium_result.validation_scores)
                if copium_result.validation_scores
                else 0
            )
            print(f"   Validation score: {avg_copium_score:.1%}")
            ...
            if baseline_result.input_tokens > 0:
                token_change = (
                    copium_result.input_tokens - baseline_result.input_tokens
                ) / baseline_result.input_tokens
                quality_change = avg_copium_score - avg_baseline_score
    ...
    print(f"\n{'Scenario':<30} {'Baseline Tokens':>15} {'Copium Tokens':>15} {'Quality Δ':>12}")
    print("-" * 72)

    for br in baseline_results:
        hr = next((r for r in copium_results if r.scenario_name == br.scenario_name), None)
        if hr:
            b_score = (
