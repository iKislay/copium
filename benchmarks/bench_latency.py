#!/usr/bin/env python3
"""Latency benchmark for Copium compression pipeline.
...
class ProfilingPipeline:
    ...
    def apply(
        ...
        from copium.utils import deep_copy_messages
...
def format_terminal_report(results: list[LatencyResult]) -> str:
    ...
    lines.append("")
    lines.append("=" * 100)
    lines.append("  COPIUM LATENCY BENCHMARK")
    lines.append("=" * 100)
    lines.append("")
...
def format_markdown_report(results: list[LatencyResult]) -> str:
    ...
    lines.append("# Copium Latency Benchmarks")
    lines.append("")
    lines.append(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Copium latency benchmark",
    ...
    print("Copium Latency Benchmark")
    print("=" * 40)
    print()
