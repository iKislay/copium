...
/// Policy for stripping internal `x-copium-*` headers from upstream-bound
/// requests (PR-A5, fixes P5-49).
///
/// When `enabled` (default), every header whose name starts with
/// `x-copium-` is dropped before the upstream call. Stops fingerprinting
/// of the proxy via subscription-revocation flags (`x-copium-bypass`,
/// `x-copium-mode`, etc.) and prevents leakage of internal user-id /
...
pub enum StripInternalHeaders {
    /// Strip every `x-copium-*` header from upstream-bound requests.
    /// Default. Operationally safe.
    Enabled,
    /// Forward `x-copium-*` to upstream verbatim. Diagnostic-only;
...
///
/// Source priority: CLI flag → `COPIUM_PROXY_CACHE_CONTROL_AUTO_FROZEN`
/// env var → default (`enabled`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
#[clap(rename_all = "snake_case")]
...
/// `COPIUM_PROXY_AUTH_MODE_POLICY_ENFORCEMENT` env var →
/// default (`disabled`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
#[clap(rename_all = "snake_case")]
...
#[command(
    name = "copium-proxy",
    version,
    about = "Copium transparent reverse proxy"
)]
pub struct CliArgs {
    /// Address the proxy listens on (e.g. 0.0.0.0:8787).
    #[arg(long, env = "COPIUM_PROXY_LISTEN", default_value = "0.0.0.0:8787")]
    ...
    #[arg(long, env = "COPIUM_PROXY_UPSTREAM")]
    pub upstream: Url,
    ...
    /// Enable Copium compression on LLM-shaped requests
    ...
    /// `--compression` (or set `COPIUM_PROXY_COMPRESSION=1`).
    #[arg(
        long = "compression",
        env = "COPIUM_PROXY_COMPRESSION",
    ...
    ///
    /// Source priority: CLI flag → `COPIUM_PROXY_COMPRESSION_MODE`
    /// env var → default (`off`).
    #[arg(
        long = "compression-mode",
        env = "COPIUM_PROXY_COMPRESSION_MODE",
        value_enum,
    ...
    #[arg(
        ...
        env = "COPIUM_PROXY_CACHE_CONTROL_AUTO_FROZEN",
        value_enum,
    ...
    #[arg(
        ...
        env = "COPIUM_PROXY_AUTH_MODE_POLICY_ENFORCEMENT",
        value_enum,
    ...
    #[arg(
        ...
        env = "COPIUM_PROXY_STRIP_INTERNAL_HEADERS",
        value_enum,
    ...
    ///
    /// Source priority: CLI flag → `COPIUM_PROXY_ENABLE_RESPONSES_STREAMING`
    /// env var → default (`true`).
    #[arg(
        long = "enable-responses-streaming",
        env = "COPIUM_PROXY_ENABLE_RESPONSES_STREAMING",
    ...
    ///
    /// Source priority: CLI flag → `COPIUM_PROXY_ENABLE_CONVERSATIONS_PASSTHROUGH`
    /// env var → default (`true`).
    #[arg(
        long = "enable-conversations-passthrough",
        env = "COPIUM_PROXY_ENABLE_CONVERSATIONS_PASSTHROUGH",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
    ...
    ///
    /// Source priority: CLI flag → `COPIUM_PROXY_ENABLE_BEDROCK_NATIVE`
    /// env var → default (`true`).
    #[arg(
        long = "enable-bedrock-native",
        env = "COPIUM_PROXY_ENABLE_BEDROCK_NATIVE",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
    ...
    ///
    /// Source priority: CLI flag → `COPIUM_PROXY_BEDROCK_REGION`
    /// env var → `AWS_REGION` env var → default (`us-east-1`).
    #[arg(
        long = "bedrock-region",
        env = "COPIUM_PROXY_BEDROCK_REGION",
    ...
    ///
    /// Source priority: CLI flag → `COPIUM_PROXY_BEDROCK_ENDPOINT`
    /// env var → derived-from-region.
    #[arg(long = "bedrock-endpoint", env = "COPIUM_PROXY_BEDROCK_ENDPOINT")]
    ...
    ///
    /// Source priority: CLI flag → `COPIUM_PROXY_AWS_PROFILE`
    /// env var → `AWS_PROFILE` env var → default chain.
    #[arg(long = "aws-profile", env = "COPIUM_PROXY_AWS_PROFILE")]
    ...
    ///
    /// Source priority: CLI flag → `COPIUM_PROXY_BEDROCK_VALIDATE_EVENTSTREAM_CRC`
    /// env var → default (`true`).
    #[arg(
        long = "bedrock-validate-eventstream-crc",
        env = "COPIUM_PROXY_BEDROCK_VALIDATE_EVENTSTREAM_CRC",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
    ...
    ///
    /// Source priority: CLI flag → `COPIUM_PROXY_VERTEX_REGION`
    /// env var → default (`us-central1`).
    #[arg(
        long = "vertex-region",
        env = "COPIUM_PROXY_VERTEX_REGION",
    ...
    ///
    /// Source priority: CLI flag → `COPIUM_PROXY_VERTEX_ADC_SCOPE`
    /// env var → default (`cloud-platform`).
    #[arg(
        long = "vertex-adc-scope",
        env = "COPIUM_PROXY_VERTEX_ADC_SCOPE",
...
pub struct Config {
    ...
    pub auth_mode_policy_enforcement: AuthModePolicyEnforcement,
    /// Whether to strip internal `x-copium-*` headers from
...
impl Config {
    ...
    pub fn for_test(upstream: Url) -> Self {
        Self {
            ...
            // Production default: strip internal `x-copium-*` headers
