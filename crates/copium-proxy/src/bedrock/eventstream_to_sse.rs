...
//! CRC-checksummed framing format. Most clients of Copium (Claude
//! Code, the Anthropic SDK in non-Bedrock mode, the Copium Python
    ...
    match v {
        HeaderValue::String(s) => {
            if s.len() <= 64 {
                ...
                // was only stabilised in Rust 1.91; copium's MSRV is 1.80.)
                let mut end = 64;
