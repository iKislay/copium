...
///
/// Response-side `X-Copium-*` injection (e.g. `x-copium-tokens-saved`)
...
pub const INTERNAL_HEADER_PREFIX: &str = "x-copium-";
...
/// Returns true when `name` matches the internal `x-copium-*` prefix
...
///   - When `strip_internal == true`, `x-copium-*` headers stripped
///     (PR-A5, fixes P5-49). Operators can disable via
///     `COPIUM_PROXY_STRIP_INTERNAL_HEADERS=disabled` for diagnostic
///     shadow tracing.
...
mod tests {
    ...
    fn internal_header_detected_case_insensitive() {
        ...
        assert!(is_internal_header(
            &HeaderName::from_bytes(b"X-Copium-Mode").unwrap()
        ));
        assert!(is_internal_header(
            &HeaderName::from_bytes(b"X-COPIUM-FOO").unwrap()
        ));
        assert!(!is_internal_header(&HeaderName::from_static(
            "x-request-id"
        )));
    ...
    fn strip_internal_headers_removes_only_internal_prefix() {
        ...
        h.insert("x-copium-bypass", HeaderValue::from_static("true"));
        h.insert("x-copium-mode", HeaderValue::from_static("passthrough"));
        h.insert("x-request-id", HeaderValue::from_static("req-1"));
        let removed = strip_internal_headers(&mut h);
        assert_eq!(removed, 2);
        assert!(h.get("x-copium-bypass").is_none());
        assert!(h.get("x-copium-mode").is_none());
    ...
    fn build_forward_strips_internal_when_enabled() {
        ...
        incoming.insert("x-copium-bypass", HeaderValue::from_static("true"));
        ...
        assert!(out.get("x-copium-bypass").is_none());
    }
    ...
    fn build_forward_keeps_internal_when_disabled() {
        ...
        assert!(out.get("authorization").is_some());
        assert!(out.get("x-copium-bypass").is_some());
    }
}
