...
use copium_proxy::vertex::TokenSource;
use copium_proxy::{build_app, AppState, Config};
...
pub fn install_static_token_source(mut state: AppState, bearer: &str) -> AppState {
    state.vertex_token_source = Arc::new(copium_proxy::vertex::StaticTokenSource::new(
        bearer.to_string(),
