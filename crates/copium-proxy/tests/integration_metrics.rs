...
use copium_proxy::observability;
use copium_proxy::sse::openai_responses::ResponseState;
use copium_proxy::sse::SseFramer;
use serde_json::json;
...
async fn cache_hit_rate_emitted_per_session() {
    ...
    let proxy = start_proxy_with(&format!("http://{addr}"), |c| {
        c.compression_mode = copium_proxy::config::CompressionMode::Off;
    })
    .await;
...
async fn passthrough_bytes_modified_zero_when_no_compression() {
    ...
    use copium_proxy::observability::record_passthrough_bytes_modified;
...
fn service_tier_validate_known_returns_canonical_constant() {
    use copium_proxy::observability::metric_names::service_tier;
...
async fn compression_ratio_per_strategy_does_not_replicate_aggregate() {
    ...
    use copium_proxy::observability::observe_compression_ratio;
