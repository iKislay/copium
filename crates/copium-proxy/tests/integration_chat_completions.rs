...
use copium_proxy::sse::openai_chat::ChunkState;
...
/// (matches the copium-core dispatch test fixture's compressibility
/// profile).
...
async fn passthrough_no_compression_byte_equal() {
    ...
    let proxy = start_proxy_with(&upstream.uri(), |c| {
        c.compression = true;
        c.compression_mode = copium_proxy::config::CompressionMode::LiveZone;
    })
    .await;
