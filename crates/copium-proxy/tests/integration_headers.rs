...
//! internal `x-copium-*` strip (PR-A5, fixes P5-49).

mod common;

use common::{start_proxy, start_proxy_with};
use copium_proxy::config::StripInternalHeaders;
...
#[tokio::test]
async fn x_copium_request_headers_stripped() {
    let upstream = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/v1/messages"))
        .respond_with(move |req: &wiremock::Request| {
            // PR-A5: internal x-copium-* must NOT reach upstream.
            assert!(
                req.headers.get("x-copium-bypass").is_none(),
                "x-copium-bypass leaked upstream"
            );
            assert!(
                req.headers.get("x-copium-mode").is_none(),
                "x-copium-mode leaked upstream"
            );
            assert!(
                req.headers.get("x-copium-user-id").is_none(),
                "x-copium-user-id leaked upstream"
            );
    ...
    let resp = reqwest::Client::new()
        ...
        .header("x-copium-bypass", "true")
        .header("x-copium-mode", "passthrough")
        .header("x-copium-user-id", "alice")
        .body("{}")
        .send()
        .await
        .unwrap();
...
#[tokio::test]
async fn x_copium_case_insensitive_stripped() {
    let upstream = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/v1/messages"))
        .respond_with(move |req: &wiremock::Request| {
    ...
    let resp = reqwest::Client::new()
        .post(format!("{}/v1/messages", proxy.url()))
        .header("X-Copium-Foo", "1")
        .header("x-Copium-Bar", "2")
        .header("X-COPIUM-BAZ", "3")
        .body("{}")
        .send()
        .await
        .unwrap();
...
async fn legitimate_headers_passthrough_with_strip_enabled() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(move |req: &wiremock::Request| {
            ...
            assert!(req.headers.get("x-copium-bypass").is_none());
            ResponseTemplate::new(200)
        })
        .mount(&upstream)
        .await;
...
async fn disabled_mode_passes_internal_headers_through() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(move |req: &wiremock::Request| {
            ...
            assert_eq!(req.headers.get("x-copium-bypass").unwrap(), "true");
            assert_eq!(req.headers.get("x-copium-mode").unwrap(), "passthrough");
            ResponseTemplate::new(200).set_body_string("{}")
        })
        .mount(&upstream)
        .await;
