...
async fn bedrock_proxy(
    upstream: &MockServer,
    customize: impl FnOnce(&mut copium_proxy::Config),
) -> common::ProxyHandle {
...
async fn usage_extracted_from_translated_stream() {
    ...
    use copium_proxy::sse::anthropic::AnthropicStreamState;
    use copium_proxy::sse::framing::SseFramer;
