...
fn from_config_redis_unsupported_when_feature_off() {
    use copium_core::ccr::backends::CcrBackendInitError;
...
mod redis_tests {
    use super::*;
    use copium_core::ccr::backends::RedisCcrStore;

    /// Reads `COPIUM_TEST_REDIS_URL` from the environment — when the
    /// feature is on but no URL is configured we silently no-op. CI
    /// runs the redis test in a docker-compose'd matrix.
    fn redis_url() -> Option<String> {
        std::env::var("COPIUM_TEST_REDIS_URL").ok()
    }

    #[test]
    fn redis_round_trip() {
        let Some(url) = redis_url() else {
            eprintln!("skipping redis_round_trip: COPIUM_TEST_REDIS_URL not set");
            return;
        };
    ...
    fn redis_round_trip_via_from_config() {
        let Some(url) = redis_url() else {
            eprintln!("skipping redis_round_trip_via_from_config: COPIUM_TEST_REDIS_URL not set");
            return;
        };
        let cfg = CcrBackendConfig::Redis {
            url,
