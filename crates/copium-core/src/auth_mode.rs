...
//!   `User-Agent`, never inject `X-Copium-*`, never strip
//!   `accept-encoding`).
//!
...
pub enum AuthMode {
    ...
    /// `X-Copium-*`; never mutate `User-Agent`.
    Subscription,
}
...
/// `crates/copium-core/benches/auth_mode.rs` asserts <10us / call.
...
mod inline_tests {
    ...
    //! copium-core --lib` exercises the helper without pulling in
    //! the integration-test binary. The exhaustive test matrix lives
    //! in `crates/copium-core/tests/auth_mode.rs`.
