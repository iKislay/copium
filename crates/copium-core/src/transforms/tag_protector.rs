...
//! `<tool_call>`, `<thinking>`, `<copium:tool_digest>`, etc.) that
...
//!   `{{COPIUM_TAG_…}}` substring, Python silently let the collision
...
const DEFAULT_PREFIX: &str = "{{COPIUM_TAG_";
...
pub struct ProtectStats {
    ...
    /// input contained a literal `{{COPIUM_TAG_` substring.
...
/// `text`. We try `{{COPIUM_TAG_` first; if the input contains it
...
fn pick_placeholder_prefix(text: &str) -> (String, bool) {
    ...
    for salt in 0u32..16 {
        let candidate = format!("{{{{COPIUM_TAG_{salt}_");
    ...
    let prefix = FALLBACK
        .get_or_init(|| "{{COPIUM_TAG_FALLBACK_a4f1c7e2_".to_string())
        .clone();
    (prefix, true)
}
...
mod tests {
    ...
    fn restore_lost_placeholder_idempotent_when_all_missing() {
        ...
        let blocks = vec![
            ...
            ("{{COPIUM_TAG_1}}".to_string(), "<b>2</b>".to_string()),
            ("{{COPIUM_TAG_2}}".to_string(), "<c>3</c>".to_string()),
        ];
    ...
    fn fixed_in_3e4_placeholder_collision_is_avoided() {
        // Bug #5: input contains literal `{{COPIUM_TAG_…}}`. The
        // walker should pick a salted prefix and report the collision
        // in stats.
        let text = "User wrote {{COPIUM_TAG_0}} on purpose. \
        ...
        assert_ne!(blocks[0].0, "{{COPIUM_TAG_0}}");
    }
