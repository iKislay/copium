...
//! and maps its 200+ labels onto Copium's existing
...
/// `COPIUM_MAGIKA_INIT_TIMEOUT_SECS`.
const MAGIKA_INIT_TIMEOUT_SECS_DEFAULT: u64 = 5;

fn magika_init_timeout() -> Duration {
    let secs = std::env::var("COPIUM_MAGIKA_INIT_TIMEOUT_SECS")
        .ok()
...
/// Classify `content` and return the mapped Copium [`ContentType`].
///
...
/// Map a magika label string to Copium's [`ContentType`] enum.
///
