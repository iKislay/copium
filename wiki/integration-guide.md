...
You don't need to run the Copium proxy. Copium is a compression library that works with **any** LLM client, proxy, or framework.
...
| Claude Code / Cursor / Copilot CLI | [Copium proxy](#proxy) | 1 command or env var |
| Agno agents | [Agno integration](#agno) | Wrap model |
| LangChain | [LangChain integration](#langchain) | Wrap model |
| Non-Python app | [Copium proxy](#proxy) | HTTP |
| TypeScript SDK | [`compress()`](#typescript-sdk) | `npm install copium-ai` |
| Vercel AI SDK | [`copiumMiddleware()`](#typescript-sdk) | Middleware adapter |
| OpenAI Node SDK | [`withCopium()`](#typescript-sdk) | Client wrapper |
| Anthropic TS SDK | [`withCopium()`](#typescript-sdk) | Client wrapper |
...
If you're already using LiteLLM as your LLM gateway, add Copium as a callback:
...
- `x-copium-compressed: true` — compression was applied
- `x-copium-tokens-saved: 1234` — tokens removed

---

## Proxy

The Copium proxy is a standalone HTTP server. Best for non-Python apps or tools that only support base URL configuration (Claude Code, Cursor, GitHub Copilot CLI).
...
For translated backends, the Copilot wrapper can switch to Copium's OpenAI-compatible route:

```bash
copium wrap copilot --backend anyllm --anyllm-provider groq -- --model gpt-4o
```

By default, `copium wrap copilot` installs `rtk` and appends token-optimized shell guidance to `.github/copilot-instructions.md` so Copilot sessions reuse the same command-saving conventions as other wrapped agent CLIs. Use `--no-rtk` to skip that step.

For Copilot's **hosted** API (`--subscription` and the implicit OAuth path), Copium routes to the generic host `https://api.githubcopilot.com`, which serves the full model set. **Enterprise / data-residency** tenants on a dedicated Copilot host pin it with `GITHUB_COPILOT_API_URL` (e.g. `export GITHUB_COPILOT_API_URL=https://api.<your-host>.githubcopilot.com`); the override flows through to the upstream request. See [`TESTING-copilot-subscription.md`](https://github.com/iKislay/copium/blob/main/TESTING-copilot-subscription.md).
...
```json
{ "plugins": { "slots": { "contextEngine": "copium" } } }
```
...
The plugin auto-detects a running Copium proxy or starts one. Compression happens in `assemble()` — zero changes to the agent's behavior.

See the [OpenClaw plugin documentation](https://github.com/iKislay/copium/tree/main/plugins/openclaw) for full setup.

---

## Compression Hooks (Advanced)

Customize compression behavior without modifying Copium's code:

```python
from copium import compress, CompressionHooks, CompressContext
...
**Q: Does Copium change the response format?**
No. Your LLM returns the same response format. Copium only modifies the input messages.

**Q: What if compression removes something the LLM needs?**
Copium stores originals in CCR (Compress-Cache-Retrieve). The LLM can call `copium_retrieve` to get full uncompressed content. Compression summaries tell the LLM what's available.
