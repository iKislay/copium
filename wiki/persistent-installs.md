...
Copium can now be installed as a durable local runtime instead of only being started ad hoc with `copium proxy` or `copium wrap ...`.

Use the Python-native `copium install` CLI when you want supported tools to keep talking to an always-on proxy at `http://127.0.0.1:8787` and have `wrap` reuse or recover that deployment instead of starting a second ephemeral proxy.

## Runtime matrix

| Mode | What stays running | Primary entrypoint |
|---|---|---|
| Persistent Service | Native background service | `copium install apply --preset persistent-service` |
| Persistent Task | Scheduled watchdog + on-demand runner | `copium install apply --preset persistent-task` |
| Persistent Docker | Restartable Docker container | `copium install apply --preset persistent-docker` |
| On-Demand CLI (Python) | Nothing after command exits | `copium proxy` |
| On-Demand CLI (Docker) | Nothing after container exits | Docker-native wrapper / compose CLI |
| Wrapped (Python) | Proxy lasts for wrapped session | `copium wrap ...` |
...
```bash
copium install apply --preset persistent-docker --scope user --providers auto
```

This uses Docker's restart policy instead of an OS supervisor.

If you are using the Docker-native host wrapper instead of a Python install, you can now use `copium install apply|status|start|stop|restart|remove` for the `persistent-docker` preset directly from the installed wrapper. Service/task installs and provider/user/system mutation flows still belong to the Python-native CLI.
...
`apply` creates or updates a named deployment profile, stores its manifest under `~/.copium/deploy/<profile>/manifest.json`, applies reversible configuration changes, and starts the selected runtime.
...
- `--runtime python` runs `copium proxy` directly
- `--runtime docker` runs Copium inside Docker while keeping the deployment managed locally
...
| Scope | What changes |
|---|---|
| `provider` | Tool-specific config surfaces where Copium can make a precise reversible edit |
...
```bash
copium install apply --providers auto
copium install apply --providers all --scope user
copium install apply --providers manual --target claude --target copilot
```
...
The Python-native `copium wrap ...` flow checks for a matching persistent deployment on the requested port before it starts a new ephemeral proxy. If an installed deployment exists but is stopped or unhealthy, it attempts to recover it first.
...
- `copium install ...` -> full persistent service, task, and Docker lifecycle management, including provider/user/system mutation
...
> **Note:** `COPIUM_WORKSPACE` (the host-side bind-mount source used
> by the compose file) is **not** the same variable as
> `COPIUM_WORKSPACE_DIR` (the canonical Copium state root inside
