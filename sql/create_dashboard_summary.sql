...
BEGIN
  ...
  -- Version breakdown
  SELECT COALESCE(jsonb_object_agg(COALESCE(copium_version, '?'), cnt), '{}'::jsonb)
  INTO _versions
  FROM (
    SELECT copium_version, COUNT(*) AS cnt
    FROM proxy_telemetry_v2
    GROUP BY copium_version
  ) sub;
