 ...
 * from the Copium proxy's compression store.
 */
...
export function createCopiumRetrieveTool(config: RetrieveToolConfig) {
  const proxyOrigin = normalizeAndValidateProxyUrl(config.proxyUrl);

  return {
    name: "copium_retrieve",
    description:
      "Retrieve original uncompressed content from Copium's compression store. " +
