# Spec — `copium-oauth2` (generic OAuth2 client-credentials upstream auth)

Feature request: [iKislay/copium#778](https://github.com/iKislay/copium/issues/778).
Status: implementation ready; awaiting maintainer 👍 before merge.

## Summary

A vendor-neutral proxy extension (registers on Copium's `copium.proxy_extension` seam) that
mints an OAuth2 **client-credentials** (RFC 6749 §4.4) bearer from a configured token endpoint and
injects it as the upstream `Authorization` on every proxied request. Lets Copium front any
...
Opt-in only, via Copium's existing flags — **no new CLI flags**:

    copium proxy --backend litellm-openai --proxy-extension oauth2
    # or COPIUM_PROXY_EXTENSIONS=oauth2

All configuration is env (12-factor; nothing baked in):

| Env var | Required | Meaning |
|---|---|---|
| `COPIUM_OAUTH2_TOKEN_URL` | yes (else no-op) | OAuth2 token endpoint; must be `https` (loopback `http` allowed for tests) |
| `COPIUM_OAUTH2_CLIENT_ID` / `_CLIENT_SECRET` | yes | client credentials |
| `COPIUM_OAUTH2_SCOPES` | no | space/comma-separated scopes |
| `COPIUM_OAUTH2_AUDIENCE` | no | `audience` form param |
| `COPIUM_OAUTH2_RESOURCE` | no | RFC 8707 target `resource` form param |
| `COPIUM_OAUTH2_GRANT_TYPE` | no | default `client_credentials` |
| `COPIUM_OAUTH2_AUTH_STYLE` | no | `post` (form creds) or `basic` (HTTP Basic) |
| `COPIUM_OAUTH2_HEADERS` | no | static upstream headers, `K=V,K2=V2` (control chars rejected) |
| `COPIUM_OAUTH2_TIMEOUT` / `_SKEW` | no | token request timeout / pre-expiry refresh skew (s) |
| `COPIUM_OAUTH2_ALLOW_INSECURE` | no | `1` to allow a non-loopback `http` token_url (discouraged) |
...
- **None unless explicitly enabled.** The entry point is dormant until `--proxy-extension oauth2`
  is passed, and even then a **no-op** unless `COPIUM_OAUTH2_TOKEN_URL` is set.
- When active, it **overwrites the request `Authorization` header** with the minted bearer before
  ...
  request then carries a bearer, Copium classifies it as OAuth-mode auth — same as supplying a
...
| Malformed `COPIUM_OAUTH2_HEADERS` (CR/LF) | offending pair dropped with a warning (no header injection) |
...
- **Wrappers (`copium wrap`, agent hooks):** the extension lives at the proxy layer, so anything
...
- No metrics/telemetry emitted; piggybacks on Copium's existing request logging. (A future
...
- **Instant rollback:** drop `--proxy-extension oauth2` (or unset `COPIUM_PROXY_EXTENSIONS`), or
...
- **Runtime:** standard library only (no new core dependency). `litellm` is touched **only** if
  `COPIUM_OAUTH2_HEADERS` is set, and it is already a Copium backend dependency — declared here
