"""Copium CCR retrieve plugin.

The copium proxy (127.0.0.1:8787) compresses large tool outputs in LLM
...
COPIUM_RETRIEVE_SCHEMA = {
    "name": "copium_retrieve",
    "description": (
        "Retrieve the original uncompressed content behind a copium "
...
def _handle_copium_retrieve(args: dict, **kw) -> str:
    ...
    try:
        ...
        return tool_error(
            f"copium proxy unreachable at {_PROXY_URL} ({type(exc).__name__}). "
    ...
    if resp.status_code != 200:
        return tool_error(f"copium proxy returned HTTP {resp.status_code}: {resp.text[:200]}")
...
def register(ctx) -> None:
    """Register the copium_retrieve tool. Called by the plugin loader."""
    ctx.register_tool(
        name="copium_retrieve",
        toolset="copium",
        schema=COPIUM_RETRIEVE_SCHEMA,
        handler=_handle_copium_retrieve,
        emoji="🗜️",
    )
