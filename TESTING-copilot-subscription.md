# Testing: GitHub Copilot subscription mode (`copium wrap copilot --subscription`)
...
[file a report](https://github.com/iKislay/copium/issues/new?template=copilot-subscription-test-report.md).

> ⚠️ This is experimental, and it reads your Copilot login token + routes your
> Copilot CLI traffic through a local Copium proxy. Only run it if you're
> comfortable with that. The branch is open for inspection.

## What it does (and what "subscription" means here)

Normally `copium wrap copilot` is **BYOK** — you bring an Anthropic/OpenAI API
...
still routing through Copium so your context gets compressed.

Mechanically: the Copilot CLI's only interposition hook is its provider-override
(the "BYOK transport"), so Copium uses that knob but supplies **your
...
Copium routes wrapped Copilot traffic to GitHub's **generic public host**,
...
Copium deliberately does **not** auto-select a per-account host from
...
it regressed `copium wrap copilot` after 0.22.4
([#610](https://github.com/iKislay/copium/issues/610)).
...
copium wrap copilot --subscription -- --model gpt-5.4
```

If you operate such an environment and would like Copium to **auto-detect** the
correct host instead of pinning it, please [open an issue](https://github.com/iKislay/copium/issues/new) —
...
```bash
pipx install --pip-args='--pre' copium-ai     # or: pip install --pre copium-ai
# (no separate API key needed — that's the point)
copium wrap copilot --subscription -- --model gpt-4o -p "Reply with exactly: COPIUM_OK"
```

- **If it prints `COPIUM_OK`** → auto-discovery works on your Linux. 🎉 Report success.
- **If it errors with "no reusable bearer token"** → discovery missed your token. Please grab the **schema** so we can fix it (redact the secret), then confirm the mechanism works via the env var:
  ...
  GITHUB_COPILOT_TOKEN='<your-token>' copium wrap copilot --subscription -- --model gpt-4o -p "Reply with: COPIUM_OK"
  ```
...
```powershell
$env:COPIUM_DOCKER_IMAGE = "ghcr.io/iKislay/copium:<branch-tag>"   # ask the maintainer for the tag
# run the Docker-native installer (scripts/install.ps1), then:
$env:GITHUB_COPILOT_TOKEN = "<your-token>"
copium wrap copilot --subscription -- --model gpt-4o -p "Reply with: COPIUM_OK"
```
Report whether it prints `COPIUM_OK`.
...
```bash
pipx install --pip-args='--pre' copium-ai
copium wrap copilot --subscription -- --model gpt-4o -p "Reply with exactly: COPIUM_OK"
```
...
Please open a
[Copilot subscription test report](https://github.com/iKislay/copium/issues/new?template=copilot-subscription-test-report.md)
with:

- **OS + version** and **how you installed** (pipx/pip wheel, Docker, source).
- Was plain `copilot` logged in?
- Did `wrap copilot --subscription` print **`COPIUM_OK`**? Paste any error.
