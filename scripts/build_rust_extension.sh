#!/usr/bin/env bash
# Build + install the Rust extension (copium._core) into the active venv.
#
...
# in-tree `copium/` directory because the dual-package layout left the
# .so in `crates/copium-py/python/copium/`. That dance is no longer
...
# wheel contains both the Python source and the compiled `copium/_core.so`,
...
log "verifying \`from copium._core import DiffCompressor, SmartCrusher\`"
python -c '
import sys
try:
    from copium._core import DiffCompressor, SmartCrusher
...
log "copium._core build + install + verify: OK"
