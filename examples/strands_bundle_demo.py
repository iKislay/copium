#!/usr/bin/env python3
"""Strands agent + Copium — drop-in compression demo.

This is what a real Strands user writes. The ONLY Copium-specific
...
The demo:
  ...
  2. Builds a normal Strands `Agent` with that tool + the bundle's MCP
     tools (copium_compress / copium_retrieve / copium_stats).
  3. Sends ONE user query.
  4. Lets the agent loop autonomously.
  5. Prints the answer + the proxy /stats summary so you can SEE that
     Copium compressed the verbose tool output on the way to Bedrock.

The Copium proxy is started as a background process here so the
...
# ============================================================================
# Boilerplate: start the Copium proxy as a child process for the demo.
...
def _start_proxy() -> subprocess.Popen[bytes]:
    ...
    log_path = Path("/tmp/copium_bundle_demo_proxy.log")
...
def _print_stats_panel() -> None:
    ...
    # MCP-side work (copium_compress / copium_retrieve called by the LLM
...
# Copium-specific bits are the `CopiumBundle` import and one
...
from copium.integrations.strands import CopiumBundle  # noqa: E402
...
def run_agent_demo() -> None:
    ...
    # === The only Copium-specific code in your app ===
    bundle = CopiumBundle(
        proxy_url=PROXY_URL,
    ...
    agent = Agent(
        model=model,
        # bundle.tools = [Copium MCP client]; we also add our local tool
...
def main() -> int:
    print("=" * 72)
    print(" Strands agent + Copium (drop-in compression)")
    print("=" * 72)
    print(f" proxy: {PROXY_URL}    region: {os.environ.get('AWS_REGION', 'us-west-2')}")
    print()
    print(" Starting Copium proxy (in production this is a long-lived service) ...")
    proxy = _start_proxy()
    try:
        _wait_for_proxy()
        print("  → proxy ready.")
        ...
        _print_stats_panel()
        print("\n" + "=" * 72)
        print(" Done. If requests_compressed > 0 above, Copium shrunk the")
