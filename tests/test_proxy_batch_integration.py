...
class TestOpenAIBatchCreate:
    ...
    def test_batch_create_with_compression(self, openai_batch_client, openai_api_key):
        ...
        try:
            ...
            if "x-copium-tokens-saved" in response.headers:
                tokens_saved = int(response.headers["x-copium-tokens-saved"])
                assert tokens_saved >= 0

            if "x-copium-savings-percent" in response.headers:
                savings_percent = float(response.headers["x-copium-savings-percent"])
                assert 0 <= savings_percent <= 100

            # Verify compression metadata was added
            metadata = batch_data.get("metadata", {})
            if metadata.get("copium_compressed") == "true":
                # Compression was applied
                assert "copium_tokens_saved" in metadata
                assert "copium_original_tokens" in metadata
                assert "copium_compressed_tokens" in metadata
                tokens_saved = int(metadata["copium_tokens_saved"])
                assert tokens_saved >= 0
