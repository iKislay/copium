...
from copium.cli import init as init_cli
from copium.install.models import ConfigScope, DeploymentManifest
from copium.providers.codex import build_launch_env, proxy_base_url
from copium.providers.codex.install import apply_provider_scope, build_install_env
...
def codex_proxy_stack(tmp_path_factory: pytest.TempPathFactory) -> Iterator[_CodexProxyStack]:
    ...
    previous_env = {name: os.environ.get(name) for name in ("COPIUM_REQUIRE_RUST_CORE", "HOME")}
    os.environ["COPIUM_REQUIRE_RUST_CORE"] = "false"
...
def _assert_delivery(
    ...
    *,
    ...
    while time.time() < deadline:
        ...
        raise AssertionError(f"Copium never recorded model {model!r} in /stats")
...
def _codex_base_url_from_config(path: Path) -> str:
    ...
    return str(parsed["model_providers"]["copium"]["base_url"])
...
def test_codex_launch_env_routes_messages_through_copium(
    ...
    _assert_delivery(
        ...
        content="Verify temporary launch env traffic reaches Copium.",
    )


def test_codex_install_env_routes_messages_through_copium(
    codex_proxy_stack: _CodexProxyStack,
) -> None:
    ...
    _assert_delivery(
        ...
        content="Verify persistent install env traffic reaches Copium.",
    )


def test_init_codex_config_routes_messages_through_copium(
    ...
    _assert_delivery(
        ...
        content="Verify init-generated Codex config sends traffic to Copium.",
    )


def test_provider_scope_codex_config_routes_messages_through_copium(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    codex_proxy_stack: _CodexProxyStack,
) -> None:
    ...
    _assert_delivery(
        ...
        content="Verify persistent provider config sends traffic to Copium.",
    )
