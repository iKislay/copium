...
Background — Finding #2 of COPIUM_PROXY_LOG_FINDINGS_2026_05_03.md.
A customer's production proxy was silently running without the
`copium._core` PyO3 extension because the Docker image never built it
...
1. The PyO3 module exposes a `hello()` marker function returning
   ``"copium-core"`` so the deployment smoke test has something stable
...
2. The proxy lifespan refuses to start when ``copium._core`` is
...
3. An explicit opt-out — ``COPIUM_REQUIRE_RUST_CORE=false`` — keeps
...
def test_rust_core_imports() -> None:
    """`copium._core.hello()` returns the documented sentinel.

    The deployment smoke test (`copium.proxy.server._check_rust_core`)
    ...
    """
    from copium._core import hello

    assert hello() == "copium-core"
...
def test_proxy_refuses_to_start_when_rust_core_missing(
    ...
    """When `copium._core` raises ImportError on import and the opt-out
    ...
    monkeypatch.delenv("COPIUM_REQUIRE_RUST_CORE", raising=False)

    # Force the import to raise. Patching the symbol on the module is
    # not enough because `_check_rust_core` runs `from copium._core
    ...
    sys.modules.pop("copium._core", None)
    ...
    # sys.modules["copium._core"] to None forces a real ImportError.
    sys.modules["copium._core"] = None  # type: ignore[assignment]
...
# ──────────────────────────────────────────────────────────────────────────
# 3. Opt-out path: COPIUM_REQUIRE_RUST_CORE=false → degraded mode + /health.
# ──────────────────────────────────────────────────────────────────────────
def test_proxy_starts_in_degraded_mode_when_opt_out_set(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """With the opt-out env var set, the lifespan must start successfully
    even when `copium._core` is missing, and `/health` must surface
    ...
    # sys.modules["copium._core"] = None makes Python treat the module
...
def test_check_rust_core_returns_loaded_when_extension_present(
    ...
    """When `copium._core` is loadable and `hello()` returns the
