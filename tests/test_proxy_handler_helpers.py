...
from copium.proxy.helpers import _copium_bypass_enabled
...
def test_copium_bypass_helper_is_transport_neutral() -> None:
    assert _copium_bypass_enabled({"x-copium-bypass": "true"}) is True
    assert _copium_bypass_enabled({"x-copium-bypass": " TRUE "}) is True
    assert _copium_bypass_enabled({"x-copium-mode": "passthrough"}) is True
    assert _copium_bypass_enabled({"x-copium-mode": " PASSTHROUGH "}) is True
    assert _copium_bypass_enabled({"x-copium-bypass": "false"}) is False
    assert _copium_bypass_enabled({}) is False
    assert _copium_bypass_enabled(None) is False
    assert OpenAIHandlerMixin._copium_bypass_enabled({"x-copium-bypass": "true"}) is True
...
def test_vertex_passthrough_records_usage_metadata_for_dashboard() -> None:
    handler = object.__new__(CopiumProxy)
...
def test_vertex_gemini_non_text_generate_records_dashboard_outcome() -> None:
    ...
    assert response.headers["x-copium-tokens-before"] == "31"
    assert response.headers["x-copium-tokens-after"] == "31"
    assert response.headers["x-copium-tokens-saved"] == "0"
    assert len(outcomes) == 1
    outcome = outcomes[0]
    assert outcome.provider == "vertex:google"
    assert outcome.model == "gemini-2.0-flash"
...
def test_retry_request_retries_connect_timeout() -> None:
    proxy = object.__new__(CopiumProxy)
...
def test_proxy_helper_creates_fresh_image_compressors(monkeypatch) -> None:
    from copium.proxy import helpers

    monkeypatch.setattr(helpers, "_image_compressor_available", None)
    _FreshCompressor.instances = 0

    with patch("copium.image.ImageCompressor", _FreshCompressor):
...
# subsystem's ProjectResolver: x-copium-project-id → x-copium-cwd →
...
def test_resolve_ccr_workspace_explicit_project_id_wins() -> None:
    """x-copium-project-id is the highest-priority signal."""
    request = _fake_request({"x-copium-project-id": "my-cool-project"})
    body = {}
...
def test_resolve_ccr_workspace_cwd_header() -> None:
    """x-copium-cwd produces a stable per-cwd key + basename label."""
    request = _fake_request({"x-copium-cwd": "/home/user/code/daphni-rails"})
    body = {}
    key, label = AnthropicHandlerMixin._resolve_ccr_workspace(request, body)
...
def test_resolve_ccr_workspace_two_cwds_get_distinct_keys() -> None:
    ...
    key_a, _ = AnthropicHandlerMixin._resolve_ccr_workspace(
        _fake_request({"x-copium-cwd": "/home/user/code/daphni-rails"}), {}
    )
    key_b, _ = AnthropicHandlerMixin._resolve_ccr_workspace(
        _fake_request({"x-copium-cwd": "/home/user/code/tamag0"}), {}
    )
