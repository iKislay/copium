...
This module is new (no modifications to existing Python under `copium/`).
...
LOG = logging.getLogger("copium.parity.recorder")
...
def record_all(root: Path | None = None) -> dict[str, str]:
    ...
    try:
        from copium.transforms.cache_aligner import CacheAligner
    ...
    try:
        from copium.ccr.tool_injection import CCRToolInjector
    ...
    try:
        from copium.transforms import content_detector as _cd_mod
