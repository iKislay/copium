"""Tests for :class:`copium.proxy.outcome.RequestOutcome` and the
:meth:`CopiumProxy._record_request_outcome` funnel.
...
def test_cache_hit_pct_rounds_to_int() -> None:
    """PERF log line consumed by ``copium perf`` parses an integer here;
...
class _FunnelHarness:
    """Pulls just enough of CopiumProxy onto an object to exercise
    ...
    def __init__(self, *, with_cost_tracker: bool = True, with_logger: bool = True) -> None:
        ...
        self._record_request_outcome = CopiumProxy._record_request_outcome.__get__(
            self, type(self)
        )
...
async def test_funnel_emits_perf_log_with_canonical_shape(
    ...
    """``copium perf`` parses this exact ``key=value`` format. Changing
    ...
    # records (the proxy disables ``copium.*`` propagation once started).
...
async def test_funnel_appends_client_to_perf_log_when_set() -> None:
    """``copium perf --client X`` filtering relies on the ``client=X``
