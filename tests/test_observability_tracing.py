...
def test_transform_pipeline_emits_trace_spans() -> None:
    ...
    provider = TracerProvider(resource=Resource.create({"service.name": "copium-test"}))
    provider.add_span_processor(SimpleSpanProcessor(exporter))
    set_copium_tracer(CopiumTracer(tracer_provider=provider))
    ...
    try:
        ...
        assert len(spans) == 1
        span = spans[0]
        assert span.name == "copium.compression.pipeline"
        assert span.attributes["copium.model"] == "gpt-4o"
        assert span.attributes["copium.tokens.before"] >= 1
        assert span.attributes["copium.tokens.after"] >= 1
    finally:
        reset_copium_tracing()
...
def test_langfuse_tracing_requires_explicit_enable(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config = LangfuseTracingConfig.from_env(default_service_name="copium-proxy")
