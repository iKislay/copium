...
Verifies that ``CopiumProxy`` gates the pre-upstream phase of
...
- env var ``COPIUM_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY`` with flag override
"""
...
class _DummyAnthropicHandler(AnthropicHandlerMixin):
    ...
    def __init__(
        ...
        *,
        ...
        # ``CopiumProxy._run_compression_in_executor`` for bounded thread
    ...
    async def _run_compression_in_executor(self, fn, *, timeout):  # noqa: ANN001
        # Mirror of ``CopiumProxy._run_compression_in_executor`` for the
...
def _run_cli_capture(args: list[str], env: dict | None = None) -> ProxyConfig:
    ...
    via ``from copium.proxy.server import ... run_server``) short-
    circuits it and lets us inspect the ``ProxyConfig`` that was built.
    """
    import copium.proxy.server as server_mod
...
def test_env_var_sets_pre_upstream_concurrency():
    ...
    env = {"COPIUM_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY": "4"}
...
def test_cli_env_sets_pre_upstream_timeouts():
    env = {
        "COPIUM_ANTHROPIC_PRE_UPSTREAM_ACQUIRE_TIMEOUT_SECONDS": "9.5",
        "COPIUM_ANTHROPIC_PRE_UPSTREAM_MEMORY_CONTEXT_TIMEOUT_SECONDS": "3.25",
    }
    config = _run_cli_capture([], env=env)
...
# --------------------------------------------------------------------------- #
# Sanity: CopiumProxy auto-computes default when config value is None.       #
# --------------------------------------------------------------------------- #
