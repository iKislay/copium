...
from copium.transforms.tag_protector import (
    KNOWN_HTML_TAGS,
    _is_html_tag,
    protect_tags,
    restore_tags,
...
class TestRestoreTags:
    ...
    def test_lost_placeholder_discards_wrap(self):
        ...
        protected = [("{{COPIUM_TAG_0}}", "<tag>data</tag>")]
    ...
    def test_partial_loss_keeps_present_discards_lost(self):
        ...
        protected = [
            ("{{COPIUM_TAG_0}}", "<a>1</a>"),
            ("{{COPIUM_TAG_1}}", "<lost>x</lost>"),
        ]
        result = restore_tags("head {{COPIUM_TAG_0}} tail", protected)
