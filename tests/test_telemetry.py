...
class TestGlobalTelemetryCollector:
    ...
    def test_env_var_disables_telemetry(self, monkeypatch):
        """COPIUM_TELEMETRY_DISABLED environment variable disables telemetry."""
        reset_telemetry_collector()
        monkeypatch.setenv("COPIUM_TELEMETRY_DISABLED", "1")
    ...
    def test_copium_telemetry_off_disables_collector(self, monkeypatch, off_value):
        """COPIUM_TELEMETRY=off (and other documented opt-out values) disables
        the collector — closes #390.

        Pre-#390 the collector only honoured COPIUM_TELEMETRY_DISABLED, which
        is undocumented. Users following the docs set COPIUM_TELEMETRY=off and
        ...
        """
        reset_telemetry_collector()
        monkeypatch.delenv("COPIUM_TELEMETRY_DISABLED", raising=False)
        monkeypatch.setenv("COPIUM_TELEMETRY", off_value)

        collector = get_telemetry_collector()

        assert collector._config.enabled is False, (
            f"COPIUM_TELEMETRY={off_value!r} must disable the collector — "
    ...
    def test_copium_telemetry_on_keeps_collector_enabled(self, monkeypatch):
