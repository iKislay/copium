...
def test_install_apply_starts_service_supervisor(monkeypatch) -> None:
    ...
    monkeypatch.setattr("copium.cli.install.build_manifest", lambda **_: manifest)
    monkeypatch.setattr("copium.cli.install.load_manifest", lambda profile: None)
    monkeypatch.setattr("copium.cli.install.apply_mutations", lambda deployment: [])
    monkeypatch.setattr("copium.cli.install.install_supervisor", lambda deployment: [])
    monkeypatch.setattr(
        "copium.cli.install.save_manifest", lambda deployment: calls.append("save")
    )
    monkeypatch.setattr(
        "copium.cli.install.start_supervisor", lambda deployment: calls.append("start_service")
    )
    monkeypatch.setattr(
        "copium.cli.install.start_detached_agent", lambda profile: calls.append("start_agent")
    )
    monkeypatch.setattr(
        "copium.cli.install.start_persistent_docker",
        lambda deployment: calls.append("start_docker"),
    )
    monkeypatch.setattr(
        "copium.cli.install.wait_ready", lambda deployment, timeout_seconds=45: True
    )
...
def test_install_status_includes_backend_from_health_probe(monkeypatch) -> None:
    ...
    monkeypatch.setattr("copium.cli.install.load_manifest", lambda profile: Manifest())
    monkeypatch.setattr("copium.cli.install.runtime_status", lambda manifest: "running")
    monkeypatch.setattr("copium.cli.install.probe_ready", lambda url: True)
    monkeypatch.setattr(
        "copium.cli.install.probe_json",
...
def test_install_restart_uses_internal_helpers(monkeypatch) -> None:
    ...
    monkeypatch.setattr("copium.cli.install.load_manifest", lambda profile: Manifest())
    monkeypatch.setattr(
        "copium.cli.install.stop_supervisor", lambda manifest: calls.append("stop_supervisor")
    )
    monkeypatch.setattr(
        "copium.cli.install.stop_runtime", lambda manifest: calls.append("stop_runtime")
    )
    monkeypatch.setattr(
        "copium.cli.install.start_supervisor", lambda manifest: calls.append("start_supervisor")
    )
    monkeypatch.setattr(
        "copium.cli.install.wait_ready", lambda manifest, timeout_seconds=45: True
    )
...
def test_install_apply_restores_previous_deployment_after_failed_update(monkeypatch) -> None:
    ...
    monkeypatch.setattr("copium.cli.install._start_deployment", _start)
...
def test_install_start_rejects_task_lifecycle(monkeypatch) -> None:
    ...
    assert result.exit_code != 0
    assert "copium install start" in result.output
...
def test_install_apply_uses_docker_runtime_for_persistent_docker(monkeypatch) -> None:
    ...
    monkeypatch.setattr("copium.cli.install.apply_mutations", lambda deployment: [])
    monkeypatch.setattr("copium.cli.install.install_supervisor", lambda deployment: [])
    monkeypatch.setattr("copium.cli.install.save_manifest", lambda deployment: None)
    monkeypatch.setattr(
        "copium.cli.install.start_persistent_docker",
        lambda deployment: calls.append("start_docker"),
    )
...
def test_install_remove_continues_when_runtime_teardown_errors(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "copium.cli.install.remove_supervisor", lambda manifest: calls.append("remove_supervisor")
    )
    monkeypatch.setattr(
        "copium.cli.install.revert_mutations", lambda manifest: calls.append("revert")
    )
    monkeypatch.setattr(
        "copium.cli.install.delete_manifest", lambda profile: calls.append("delete")
    )
...
def test_install_agent_run_exits_with_foreground_status(monkeypatch) -> None:
    ...
    monkeypatch.setattr("copium.cli.install.load_manifest", lambda profile: Manifest())
    monkeypatch.setattr("copium.cli.install.run_foreground", lambda manifest: 7)
