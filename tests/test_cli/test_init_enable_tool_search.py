"""`copium init claude` must set ENABLE_TOOL_SEARCH (GH #746).
...
`copium wrap claude` already sets it; `init` (and the persistent install) must
...
from copium.providers.claude import install as claude_install
